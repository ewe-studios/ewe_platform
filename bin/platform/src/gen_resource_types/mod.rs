...
impl ResourceGenerator {
    ...
    /// Generate mod.rs for a resources directory with provider-level feature flag.
    fn generate_mod_rs_simple(&self, provider: &str, apis: &[String]) -> Result<(), GenResourceError> {
        ...
        writeln!(out, "//!").map_err(write_fmt_error)?;
        writeln!(out, "//! Feature flag: `{provider}`").map_err(write_fmt_error)?;
        writeln!(out).map_err(write_fmt_error)?;
        writeln!(out, "#![cfg(feature = \"{provider}\")]").map_err(write_fmt_error)?;
        writeln!(out).map_err(write_fmt_error)?;
