...
//
// Ported & adapted from `wasmbin` v0.9.2 (https://github.com/RReverser/wasmbin)
// for ewe_platform (feature 15); modifications are listed in src/wasm/README.md.

use crate::wasm::builtins::WasmbinCountable;
use crate::wasm::io::{Decode, DecodeError, DecodeErrorKind, Encode};
use crate::wasm::visit::{Visit, VisitError};
use custom_debug::Debug as CustomDebug;
use once_cell::sync::OnceCell;
#[cfg(feature = "wasm-serde")]
...
#[cfg_attr(feature = "wasm-serde", derive(Serialize, Deserialize))]
#[cfg_attr(feature = "wasm-serde", serde(transparent))]
pub struct UnparsedBytes {
    #[expect(missing_docs)]
    #[debug(with = "custom_debug::hexbuf_str")]
    #[cfg_attr(feature = "wasm-serde", serde(with = "serde_bytes"))]
    pub bytes: Vec<u8>,
}
...
/// [`Blob`](crate::wasm::builtins::Blob)). During decoding, this type will
...
/// raw bytes.
#[derive(Clone)]
#[cfg_attr(feature = "wasm-serde", derive(Deserialize))]
#[cfg_attr(feature = "wasm-serde", serde(from = "T"))]
pub struct Lazy<T> {
    status: LazyStatus<T>,
}
...
impl<T: Decode + PartialEq> PartialEq for Lazy<T> {
    fn eq(&self, other: &Self) -> bool {
        // (edition 2021: let-chain from upstream rewritten as a nested condition)
