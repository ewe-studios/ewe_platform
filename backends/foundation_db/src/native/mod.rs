...
pub fn create_state_store(
    ...
    if std::env::var("DEPLOYMENT_D1_DATABASE_ID").is_ok() {
        return Ok(Box::new(D1Store::from_env_state(project, stage)?));
    }

    #[cfg(feature = "r2")]
    if std::env::var("DEPLOYMENT_R2_BUCKET").is_ok() {
        return Ok(Box::new(R2Store::from_env_state(project, stage)?));
    }

    #[cfg(feature = "libsql")]
    {
        if std::env::var("TURSO_DATABASE_URL").is_ok() {
            return Ok(Box::new(LibsqlStore::from_env_state(project_dir, project, stage)?));
        }
