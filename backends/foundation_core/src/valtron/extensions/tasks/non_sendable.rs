...
use crate::valtron::{ExecutionAction, Stream, TaskIterator, TaskShortCircuit, TaskSpread, TaskStatus};
...
pub trait TaskIteratorExt: TaskIterator + Sized {
    ...
    where
        ...
        Self::Ready: Clone,
        Self::Pending: Clone,
        P: Fn(&Self::Ready) -> CollectionState + 'static;
    ...
    where
        ...
        D: Clone + 'static,
        Self::Pending: Clone,
        F: Fn(&Self::Ready) -> (CollectionState, Option<D>) + 'static;
    ...
    /// ## Type Requirements
    ///
    /// - `M` must be `Clone +  'static` (observer gets transformed copy)
    /// - `Pending` must be `Clone` (observer gets a copy)
    ///
    /// ## Arguments
    ///
    ...
    where
        ...
        M: Clone + 'static,
        Self::Pending: Clone,
        F: Fn(&Self::Ready) -> (bool, Option<M>) + 'static;
    ...
    where
        ...
        Self::Ready: 'static,
        Self::Pending: 'static,
        Self::Spawner: ExecutionAction + 'static,
    {
    ...
    where
        Self: Sized,
        F: Fn(Self::Ready) -> InnerIter + 'static,
        InnerIter: Iterator<Item = TaskStatus<InnerR, InnerP, InnerS>> + 'static,
        InnerR: 'static,
        InnerP: 'static,
        InnerS: ExecutionAction + 'static,
        Self::Pending: Into<InnerP> + 'static,
        Self::Spawner: Into<InnerS> + 'static;
    ...
    where
        ...
        <Self::Ready as IntoIterator>::Item: 'static;
    ...
    where
        ...
        <Self::Pending as IntoIterator>::Item: 'static;
    ...
    where
        F: Fn(&TaskStatus<Self::Ready, Self::Pending, Self::Spawner>) + 'static;
    ...
    where
        F: Fn(&TaskStatus<Self::Ready, Self::Pending, Self::Spawner>) -> bool + 'static;
...
where
    I: TaskIterator + 'static,
    I::Ready: 'static,
    I::Pending: 'static,
    I::Spawner: ExecutionAction + 'static,
{
    fn map_ready<F, R>(self, f: F) -> TMapReady<Self, R>
    where
        F: Fn(Self::Ready) -> R + 'static,
...
where
    ...
    InnerP: 'static,
    InnerS: ExecutionAction + 'static,
    P: Into<InnerP> + 'static,
    S: Into<InnerS> + ExecutionAction + 'static,
{
...
where
    I: TaskIterator,
    I::Ready: IntoIterator,
    <I::Ready as IntoIterator>::Item: 'static,
    I::Pending: 'static,
    I::Spawner: 'static,
{
...
where
    I: TaskIterator,
    I::Pending: IntoIterator,
    <I::Pending as IntoIterator>::Item: 'static,
    I::Ready: 'static,
    I::Spawner: 'static,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match status {
            ...
            // Pass through non-Pending states
...
where
    I: TaskIterator,
    F: Fn(&TaskStatus<I::Ready, I::Pending, I::Spawner>) + 'static,
{
    type Item = TaskStatus<I::Ready, I::Pending, I::Spawner>;
...
where
    I: TaskIterator,
    F: Fn(&TaskStatus<I::Ready, I::Pending, I::Spawner>) -> bool + 'static,
{
    type Item = TaskStatus<I::Ready, I::Pending, I::Spawner>;
