...
pub trait StreamIteratorExt: StreamIterator + Sized {
    ...
    where
        Self: Sized,
    {
        crate::valtron::StreamIter(self)
    }

    /// Transform `Next(D)` values through `Fn(D) -> Option<R>`.  When the
    /// closure returns `None`, the stream terminates (exhausts).  `Some(r)`
    /// becomes `Stream::Next(r)`.
    fn try_map_done<F, R>(self, f: F) -> TryMapDone<Self, R>
    where
        F: Fn(Self::D) -> Option<R> + Send + 'static,
        R: Send + 'static;

    /// Sync-boundary collector: drain the stream to completion, collecting
    /// every `Stream::Next(value)` into a `Vec<D>`. Non-`Next` items are
    /// discarded.
    ///
    /// This is the same operation as [`crate::valtron::collect_result`],
    /// exposed as a chainable combinator.
    fn collect_result(self) -> Vec<Self::D>;

    /// Sync-boundary collector: drain the stream until the first
    /// `Stream::Next(value)` and return it. Returns `None` if the stream
    /// exhausts without producing a `Next`.
    ///
    /// This is the same operation as [`crate::valtron::collect_one`],
    /// exposed as a chainable combinator.
...
/// Wrapper that maps `Next(D)` values through `Fn(D) -> Option<R>`.  When the
/// closure returns `None`, the stream exhausts.  `Some(r)` becomes
/// `Stream::Next(r)`.  All non-`Next` states pass through unchanged.
///
/// For `Stream::Spread`, each `Done` is mapped individually.  If any `Done`
/// maps to `None`, the stream exhausts on the next `next()` call
/// (the current Spread is emitted with the successfully-mapped items).
pub struct TryMapDone<I: StreamIterator, R> {
    inner: I,
    mapper: Box<dyn Fn(I::D) -> Option<R> + Send>,
    done: bool,
}
    ...
    R: Send + 'static,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.inner.next().map(|stream| match stream {
            Stream::Next(v) => Stream::Next((self.mapper)(Stream::Next(v))),
            Stream::Pending(v) => Stream::Next((self.mapper)(Stream::Pending(v))),
            Stream::Delayed(d) => Stream::Delayed(d),
            Stream::Init => Stream::Init,
            Stream::Ignore => Stream::Ignore,
            Stream::Wait => Stream::Wait,
