...
pub trait StreamIteratorExt: StreamIterator + Sized {
    ...
    where
        F: Fn(Stream<Self::D, Self::P>) -> R + 'static,
        R: 'static;
    ...
    where
        F: Fn(&Self::D) -> bool + 'static;

    /// Transform Delayed durations.
    fn map_delayed<F>(self, f: F) -> MapDelayed<Self>
    where
        F: Fn(std::time::Duration) -> std::time::Duration + 'static;
    ...
    where
        ...
        Self::D: Clone,
        Self::P: Clone,
        Pred: Fn(&Stream<Self::D, Self::P>) -> CollectionState + 'static;
    ...
    where
        Self: Sized,
        F: Fn(Stream<Self::D, Self::P>) -> ShortCircuit<Self::D, Self::P> + 'static;
    ...
    where
        ...
        Self::D: Clone,
        Self::P: Clone,
        F: Fn(&Stream<Self::D, Self::P>) -> (bool, Option<Stream<DM, PM>>) + 'static;
    ...
    where
        Self: Sized,
        F: Fn(Stream<Self::D, Self::P>) -> InnerIter + 'static,
        InnerIter: StreamIterator + 'static,
    {
        MapIter {
            outer: self,
            mapper,
    ...
    where
        Self: Sized,
        F: Fn(Self::D) -> InnerIter + 'static,
        InnerIter: StreamIterator<D = InnerD, P = Self::P> + 'static,
    {
        MapIterDone {
            outer: self,
            mapper,
    ...
    where
        Self: Sized,
        F: Fn(Self::P) -> InnerIter + 'static,
        InnerIter: StreamIterator<D = Self::D, P = InnerP> + 'static,
    {
        MapIterPending {
            outer: self,
            mapper,
    ...
    where
        Self: Sized,
        Self::D: IntoIterator,
        <Self::D as IntoIterator>::Item: 'static;
    ...
    where
        Self: Sized,
        Self::P: IntoIterator,
        <Self::P as IntoIterator>::Item: 'static;
    ...
    where
        F: Fn(Stream<Self::D, Self::P>) -> Option<Stream<Self::D, Self::P>> + 'static;
    ...
    R: 'static,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.inner.next()? {
            Stream::Next(v) => Some(Stream::Next((self.mapper)(v))),
            Stream::Pending(v) => Some(Stream::Pending(v)),
    ...
    R: 'static,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.inner.next()? {
            Stream::Next(v) => Some(Stream::Next(v)),
            Stream::Pending(v) => Some(Stream::Pending((self.mapper)(v))),
            Stream::Delayed(d) => Some(Stream::Delayed(d)),
            Stream::Init => Some(Stream::Init),
            Stream::Ignore => Some(Stream::Ignore),
            Stream::Wait => Some(Stream::Wait),
    ...
    R: 'static,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match stream {
            Stream::Next(_) | Stream::Pending(_) => Some(Stream::Next((self.mapper)(stream))),
            Stream::Delayed(d) => Some(Stream::Delayed(d)),
            Stream::Init => Some(Stream::Init),
            Stream::Ignore => Some(Stream::Ignore),
            Stream::Wait => Some(Stream::Wait),
            Stream::Spread(items) => {
                Some(Stream::Spread(
                    items
                        ...
                        .map(|item| match item {
                            StreamSpread::Done(d) => {
                                StreamSpread::Done((self.mapper)(Stream::Next(d)))
                            }
                            StreamSpread::Pending(p) => {
                                StreamSpread::Done((self.mapper)(Stream::Pending(p)))
                            }
                        })
                        .collect(),
                ))
...
where
    I: StreamIterator<D = D, P = P> + 'static,
    D: Clone + 'static,
    P: 'static,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        for _ in 0..self.sources.len() {
            ...
            match self.sources[idx].next() {
                ...
                Some(Stream::Spread(items)) => {
                    let has_pending = items.iter().any(|i| matches!(i, StreamSpread::Pending(_)));
                    for item in items {
...
where
    I: StreamIterator,
    I::D: IntoIterator,
    <I::D as IntoIterator>::Item: 'static,
    I::P: 'static,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match self.inner.next() {
            ...
            Some(Stream::Wait) => Some(Stream::Wait),
            Some(Stream::Spread(items)) => {
                let mut done_items: Vec<<I::D as IntoIterator>::Item> = Vec::new();
                let mut pending_items: Vec<StreamSpread<<I::D as IntoIterator>::Item, I::P>> = Vec::new();
                for item in items {
                    match item {
                        StreamSpread::Done(iterable) => {
                            done_items.extend(iterable);
                        }
                        StreamSpread::Pending(p) => {
                ...
                if !done_items.is_empty() {
                    // Original: returned Vec<M> as a single Done
                    // For consistency, return the flat-mapped result
                    Some(Stream::Spread(
                        done_items.into_iter().map(StreamSpread::Done).chain(pending_items).collect(),
                    ))
...
where
    I: StreamIterator,
    I::P: IntoIterator,
    <I::P as IntoIterator>::Item: 'static,
    I::D: 'static,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match self.inner.next() {
            ...
            Some(Stream::Wait) => Some(Stream::Wait),
            Some(Stream::Spread(items)) => {
                let mut done_items: Vec<StreamSpread<I::D, <I::P as IntoIterator>::Item>> = Vec::new();
                let mut pending_items: Vec<<I::P as IntoIterator>::Item> = Vec::new();
                for item in items {
                    match item {
                        StreamSpread::Done(d) => {
                            done_items.push(StreamSpread::Done(d));
                        }
                        StreamSpread::Pending(iterable) => {
                            pending_items.extend(iterable);
                        }
                    }
                }
                if !pending_items.is_empty() {
                    Some(Stream::Spread(
                        done_items.into_iter().chain(pending_items.into_iter().map(StreamSpread::Pending)).collect(),
                    ))
                } else if !done_items.is_empty() {
                    Some(Stream::Spread(done_items))
                } else {
                    Some(Stream::Ignore)
                }
            }
    ...
    I::P: 'static,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match self.inner.next() {
            ...
            Some(Stream::Wait) => Some(Stream::Wait),
            Some(Stream::Spread(items)) => {
                let mut done_items: Vec<U::Item> = Vec::new();
                let mut pending_items: Vec<StreamSpread<U::Item, I::P>> = Vec::new();
                for item in items {
                    match item {
                        StreamSpread::Done(d) => {
                            done_items.extend((self.mapper)(d));
                        }
                        StreamSpread::Pending(p) => {
                            pending_items.push(StreamSpread::Pending(p));
                        }
    ...
    I::D: 'static,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match self.inner.next() {
            ...
            Some(Stream::Wait) => Some(Stream::Wait),
            Some(Stream::Spread(items)) => {
                let mut done_items: Vec<StreamSpread<I::D, U::Item>> = Vec::new();
                let mut pending_items: Vec<U::Item> = Vec::new();
                for item in items {
                    match item {
                        StreamSpread::Done(d) => {
                            done_items.push(StreamSpread::Done(d));
                        ...
                        StreamSpread::Pending(p) => {
                            pending_items.extend((self.mapper)(p));
                        }
                    }
                }
                Some(Stream::Spread(
...
where
    I: StreamIterator,
    F: Fn(Stream<I::D, I::P>) -> Stream<R, I::P> + 'static,
    R: 'static,
{
    type Item = Stream<R, I::P>;
...
where
    I: StreamIterator,
    F: Fn(&Stream<I::D, I::P>) -> bool + 'static,
{
    type Item = Stream<I::D, I::P>;
...
where
    I: StreamIterator,
    F: Fn(Stream<I::D, I::P>) -> Option<Stream<I::D, I::P>> + 'static,
{
    type Item = Stream<I::D, I::P>;
    ...
    R: 'static,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match self.inner.next() {
            ...
            Some(Stream::Wait) => Some(Stream::Wait),
            Some(Stream::Spread(items)) => {
                let mut pending_items: Vec<StreamSpread<R, I::P>> = Vec::new();
                if let Some(acc) = self.acc.take() {
                    let folded = items.into_iter().fold(acc, |acc, item| {
                        match item {
                            StreamSpread::Done(d) => (self.folder)(acc, d),
                            StreamSpread::Pending(p) => {
                                pending_items.push(StreamSpread::Pending(p));
                                acc
                            }
