...
use foundation_core::wire::simple_http::{
    http_streams, HTTPStreams, HttpRequestReader, HttpResponseReader,
    IncomingRequestParts, IncomingResponseParts, SimpleHttpBody,
};
...
fn test_request_reader_one_request_then_eof() {
    ...
    let has_intro = parts.iter().any(|p| {
        matches!(p, Ok(IncomingRequestParts::Intro(_, _, _)))
    });
...
fn test_response_reader_one_response_then_eof() {
    ...
    let has_intro = parts.iter().any(|p| {
        matches!(p, Ok(IncomingResponseParts::Intro(_, _, _)))
    });
...
fn test_keepalive_then_eof() {
    ...
    let has_request = parts.iter().any(|p| {
        matches!(p, Ok(IncomingRequestParts::Intro(_, _, _)))
    });
