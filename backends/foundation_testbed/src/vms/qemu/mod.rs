...
use crate::vms::config::{ensure_dirs, monitor_dir, DisplayMode, GuestOs, Result, TestbedError, VmProfile};
use tracing::debug;
...
impl QemuConfig {
    ...
    pub fn launch(self) -> Result<QemuVm> {
        ...
        let (macos_boot_disk, macos_efi_disk) = if config.profile.os == GuestOs::MacOS {
            let boot = crate::vms::import::macos::ensure_macos_image(config.profile.name)?;
            let base_dir = boot.parent().unwrap().to_path_buf();
            let efi = base_dir.join(crate::vms::import::macos::OPENCORE_EFI_FILENAME);
            (Some(boot), Some(efi))
        } else {
            (None, None)
        };
        ...
        let work_dir = crate::vms::config::work_dir(&config.profile.name);
...
pub fn adopt(
    ...
    crate::vms::state::save(&crate::vms::state::VmState {
...
pub fn refresh_network(
    ...
    // Stop if running
    if let Ok(state) = crate::vms::state::load(profile.name)
...
pub fn build_qemu_args(
    ...
    if profile.os == GuestOs::Windows {
        let vars_path = crate::vms::config::state_dir().join(format!("{}.nvram", profile.name));
        if !vars_path.exists() {
