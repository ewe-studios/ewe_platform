...
use clap::ArgMatches;
use crate::vms::config::{get_profile, DisplayMode};
...
fn attach_running(profile: &crate::vms::config::VmProfile) -> std::result::Result<crate::vms::qemu::QemuVm, BoxedError> {
    let vm_state = crate::vms::state::load(&profile.name).map_err(|e| Box::new(e) as BoxedError)?;
    ...
    let resolved = crate::vms::qemu::ResolvedPorts {
    ...
    let vm = crate::vms::qemu::QemuVm::adopt_running(
        profile.clone(),
...
pub fn cmd_snapshot(args: &ArgMatches) -> std::result::Result<(), BoxedError> {
    match args.subcommand() {
        Some(("save", m)) => {
            ...
            crate::vms::qemu::snapshot::save(&mut vm, snap_name)?;
        ...
        Some(("load", m)) => {
            ...
            let profile = resolve_profile(name)?;
            let mut vm = attach_running(&profile)?;
            crate::vms::qemu::snapshot::load(&mut vm, snap_name)?;
        ...
        Some(("delete", m)) => {
            ...
            let profile = resolve_profile(name)?;
            let mut vm = attach_running(&profile)?;
            crate::vms::qemu::snapshot::delete(&mut vm, snap_name)?;
        ...
        Some(("list", m)) => {
            ...
            let profile = resolve_profile(name)?;
            let mut vm = attach_running(&profile)?;
            let snapshots = crate::vms::qemu::snapshot::list(&mut vm)?;
...
pub fn cmd_resize_disk(args: &ArgMatches) -> std::result::Result<(), BoxedError> {
    ...
    crate::vms::qemu::disk::resize(&disk_path, plus_gb)?;
...
pub fn cmd_mount(args: &ArgMatches) -> std::result::Result<(), BoxedError> {
    match args.subcommand() {
        Some(("status", m)) => {
            ...
            let disk_path = profile.image_cache_path();
            let vm_state = crate::vms::state::load(name).ok();
            ...
            println!("  Guest tag: {}", crate::vms::qemu::mount::DEFAULT_TAG);
            println!("  Guest path: {}", crate::vms::qemu::mount::DEFAULT_GUEST_PATH);
        ...
        Some(("verify", m)) => {
            ...
            let mut session = crate::vms::ssh::connect(&profile)?;
            let verify_cmd = crate::vms::qemu::mount::verify_mount_command(
                crate::vms::qemu::mount::DEFAULT_GUEST_PATH,
            );
            let output = crate::vms::ssh::exec(&mut session, &verify_cmd)?;
...
fn wait_for_ssh_ready(
    ...
    timeout_secs: u64,
) -> Result<crate::vms::ssh::VmSession, BoxedError> {
    let deadline = std::time::Instant::now() + std::time::Duration::from_secs(timeout_secs);

    while std::time::Instant::now() < deadline {
        match crate::vms::ssh::connect(profile) {
            Ok(session) => {
                return Ok(session);
            }
            Err(e) => {
...
fn verify_mount(
    session: &mut crate::vms::ssh::VmSession,
    mount_type: &str,
    guest_path: &str,
    profile: &crate::vms::config::VmProfile,
    ...
    match mount_type {
        "virtiofs" => {
            ...
            let check = crate::vms::ssh::exec(session,
            ...
            match check {
                ...
                _ => {
                    ...
                    let reg_check = crate::vms::ssh::exec(session,
            ...
            // Check mount point
            let mount_check = crate::vms::ssh::exec(session,
            ...
            match mount_check {
                Ok(output) if output.contains("MOUNT_OK") => {
                    ...
                    let file_check = crate::vms::ssh::exec(session,
        ...
        "smb" => {
            ...
            let smb_check = crate::vms::ssh::exec(session, "if (Test-Path 'Z:\\') { 'SMB_OK' } else { 'SMB_MISSING' }");
            match smb_check {
        ...
        "9p" => {
            ...
            match mount_check {
                ...
                _ => {
                    ...
                    match crate::vms::ssh::exec(session, &mount_cmd) {
                        Ok(_) => {
...
pub fn cmd_network(args: &ArgMatches) -> std::result::Result<(), BoxedError> {
    use crate::vms::config::{DisplayMode, GuestOs};
    use crate::vms::providers::default_provider;
    use crate::vms::state;
    use std::sync::mpsc;
    ...
    if daemonize {
        ...
        let (ready_tx, ready_rx) = mpsc::channel::<Result<crate::vms::ssh::VmSession, String>>();
...
pub fn cmd_adopt(args: &ArgMatches) -> std::result::Result<(), BoxedError> {
    ...
    let (adopted_profile, disk_path) = crate::vms::qemu::adopt(&profile, Path::new(disk))?;
...
pub fn cmd_refresh_network(args: &ArgMatches) -> std::result::Result<(), BoxedError> {
    ...
    let vm = crate::vms::qemu::refresh_network(&profile, DisplayMode::Headless)?;
...
pub fn cmd_export(args: &ArgMatches) -> std::result::Result<(), BoxedError> {
    ...
    let compression = match args.get_one::<String>("compress").map(|s| s.as_str()) {
        Some("xz") | Some("lzma") => crate::vms::export::Compression::Xz,
        Some("gzip") | Some("gz") => crate::vms::export::Compression::Gzip,
        Some("none") => crate::vms::export::Compression::None,
        Some(other) => return Err(format!("Unknown compression type: {other}. Use xz, gzip, or none.").into()),
        None => crate::vms::export::Compression::Xz, // default: best compression
    };

    let opts = crate::vms::export::ExportOptions {
        out,
        store,
        version,
        notes,
    ...
    crate::vms::export::export_vm(name, &opts)?;
    Ok(())
}
