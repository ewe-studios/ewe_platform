...
use crate::vms::ssh;
use crate::vms::runner;
use crate::vms::runner::logs;
...
pub fn cmd_exec(args: &ArgMatches) -> std::result::Result<(), BoxedError> {
    ...
    match method {
        "winrm" => {
            ...
            let winrm = crate::vms::winrm::WinRM::new("127.0.0.1", port, profile.user, profile.pass);
...
pub fn cmd_run(args: &ArgMatches) -> std::result::Result<(), BoxedError> {
    ...
    let winrm = if profile.os == crate::vms::config::GuestOs::Windows {
        let port = profile.winrm_port.unwrap_or(5985);
        Some(crate::vms::winrm::WinRM::new("127.0.0.1", port, profile.user, profile.pass))
    } else {
        None
    };
