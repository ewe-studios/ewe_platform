...
fn verify_mount(session: &mut VmSession, os: GuestOs) -> Result<()> {
    ...
    let result = crate::vms::ssh::exec(session, &check_cmd)?;
    ...
    let result = crate::vms::ssh::exec(session, &project_check)?;
...
fn install_project_tools(session: &mut VmSession, os: GuestOs) -> Result<()> {
    ...
    crate::vms::ssh::exec(session, &install_cmd)?;
    Ok(())
}
...
"#,
    ...
    let (_output, exit_code) = crate::vms::ssh::exec_with_exit(session, &build_cmd)?;
    Ok(exit_code)
}
...
fn retrieve_artifacts(session: &mut VmSession, profile: &VmProfile) -> Result<PathBuf> {
    ...
    crate::vms::ssh::exec(session, &format!(
    ...
    crate::vms::ssh::download(session, "/tmp/artifacts.tar.gz", &local_tar)?;
