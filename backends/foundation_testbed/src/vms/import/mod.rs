...
pub fn ensure_image(profile: &VmProfile) -> Result<std::path::PathBuf> {
    ...
    if let Ok(image_path) = std::env::var(&env_key) {
        ...
        if is_compressed_qcow2(&path) {
            ...
            crate::vms::export::decompress(&path, &dest)?;
            return Ok(dest);
        }
    ...
    if let Some(cached) = find_cached_image(&dest) {
        return crate::vms::export::ensure_decompressed(&cached);
    }
    ...
    if is_compressed_qcow2(&temp_dest) {
        ...
        crate::vms::export::decompress(&temp_dest, &dest)?;
...
fn try_download_from_store(profile: &VmProfile, dest: &Path) -> Result<Option<PathBuf>> {
    let stores = crate::vms::config::load_image_stores();
    if stores.is_empty() {
        return Ok(None);
    }
...
fn store_download_url_direct(store: &crate::vms::config::ImageStore, filename: &str) -> Option<String> {
    match store.store_type {
        crate::vms::config::StoreType::Http => {
            Some(format!("{}/{}", store.destination.trim_end_matches('/'), filename))
        }
        crate::vms::config::StoreType::Local => {
            Some(format!("{}/{}", store.destination.trim_end_matches('/'), filename))
        }
        crate::vms::config::StoreType::R2 | crate::vms::config::StoreType::S3 => {
...
pub fn is_cached(profile: &VmProfile) -> bool {
    ...
    if profile.name.starts_with("macos") {
        let macos_dir = crate::vms::config::image_cache_dir().join(format!("macos-{}", profile.name));
...
fn store_download_url(store: &crate::vms::config::ImageStore, filename: &str) -> Option<String> {
    match store.store_type {
        crate::vms::config::StoreType::Http => {
            Some(format!("{}/{}", store.destination.trim_end_matches('/'), filename))
        }
...
pub fn ensure_virtio_iso() -> Result<PathBuf> {
    let cache_path = crate::vms::config::image_cache_dir().join("virtio-win.iso");

    // 1. Check EweStore local store
    let store_path = Path::new(crate::vms::config::VIRTIO_ISO_STORE_PATH);
    ...
    download::download(crate::vms::config::VIRTIO_ISO_DOWNLOAD_URL, &temp_path)?;
