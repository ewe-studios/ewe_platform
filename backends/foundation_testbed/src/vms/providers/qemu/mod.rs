...
//!
//! Wraps the existing `crate::vms::qemu::QemuConfig` into the Provider trait.

use std::path::PathBuf;

use crate::vms::config::{DisplayMode, Result, VmProfile};
use crate::vms::providers::{Provider, ProviderId, ResolvedPorts, VmHandle};
...
impl Provider for QemuProvider {
    ...
    fn launch(&self, profile: &VmProfile, mode: DisplayMode) -> Result<VmHandle> {
        let mut config = crate::vms::qemu::QemuConfig::new(profile.clone(), mode);

        // Apply per-VM mount configuration from testbed.toml
        let cwd = std::env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
        if let Some((host_path, guest_path, readonly, _methods)) =
            crate::vms::config::get_mount_for_profile(profile.name, &cwd.to_string_lossy())
        {
    ...
    fn monitor_command(&self, _handle: &VmHandle, _cmd: &str) -> Result<String> {
        Err(crate::vms::config::TestbedError::MonitorFailed {
    ...
    fn ensure_image(&self, profile: &VmProfile) -> Result<PathBuf> {
        crate::vms::import::ensure_image(profile)
    }

    fn host_health(&self) -> crate::vms::doctor::HostHealth {
        crate::vms::doctor::check_host()
    }
}
...
pub fn run_startup_scripts_for(handle: &VmHandle) -> Result<()> {
    crate::vms::init::run_startup_scripts(
        handle.profile.name,
        handle.profile.os,
...
fn run_shutdown_scripts(handle: &VmHandle) {
    if let Err(e) = crate::vms::init::run_shutdown_scripts(
        handle.profile.name,
        handle.profile.os,
        handle.resolved_ports.ssh_port,
        handle.profile.user,
