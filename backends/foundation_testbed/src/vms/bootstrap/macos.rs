...
fn enable_remote_login(session: &mut VmSession) -> Result<()> {
    let check = crate::vms::ssh::exec(session, "systemsetup -getremotelogin 2>/dev/null")?;
    if check.to_lowercase().contains("on") {
        return Ok(());
    }
    crate::vms::ssh::exec(session, "sudo systemsetup -setremotelogin on")?;
    Ok(())
}
...
fn install_xcode_clt(session: &mut VmSession) -> Result<()> {
    let clang_check = crate::vms::ssh::exec(session, "command -v clang >/dev/null 2>&1 && echo present || echo missing")?;
    if clang_check.contains("present") {
        let sdk_check = crate::vms::ssh::exec(session, "xcrun --show-sdk-path 2>/dev/null && echo present || echo missing")?;
        if sdk_check.contains("present") {
            return Ok(());
        }
    }
    let output = crate::vms::ssh::exec(
        session,
        "softwareupdate --list 2>&1 | grep -i 'Command Line Tools' | head -1",
    )?;
    if output.trim().is_empty() {
        crate::vms::ssh::exec(session, "xcode-select --install 2>&1")?;
    ...
    crate::vms::ssh::exec(session, "softwareupdate --all --install --agree-to-license 2>&1")?;
    Ok(())
}

fn install_mise(session: &mut VmSession) -> Result<()> {
    let mise = crate::vms::ssh::exec(session, "~/.local/bin/mise --version 2>/dev/null || echo missing")?;
    if !mise.contains("missing") && !mise.is_empty() {
        return Ok(());
    }
    crate::vms::ssh::exec(session, INSTALL_MISE_SH)?;
    Ok(())
}

fn activate_mise_in_zprofile(session: &mut VmSession) -> Result<()> {
    crate::vms::ssh::exec(session, ACTIVATE_MISE_ZPROFILE_SH)?;
    Ok(())
}
...
fn configure_mise_cargo_binstall(session: &mut VmSession) -> Result<()> {
    crate::vms::ssh::exec(session, CONFIGURE_MISE_CARGO_BINSTALL_SH)?;
    Ok(())
}
...
fn set_nushell_default_shell(session: &mut VmSession) -> Result<()> {
    crate::vms::ssh::exec(session, SET_NUSHELL_DEFAULT_SHELL_SH)?;
    Ok(())
}
