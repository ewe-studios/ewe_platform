...
use crate::vms::config::{self, GuestOs, VmProfile};
...
pub fn check_host() -> HostHealth {
    ...
    let prereqs = crate::vms::host_bootstrap::check_host_prerequisites();
    checks.push(Check {
        name: "mise",
        ok: prereqs.mise.is_available(),
        message: match &prereqs.mise {
            crate::vms::host_bootstrap::ToolStatus::AlreadyPresent { version }
            | crate::vms::host_bootstrap::ToolStatus::Installed { version } => {
                format!("mise {version}")
            }
            crate::vms::host_bootstrap::ToolStatus::Missing { error } => error.clone(),
        },
...
pub fn check_vm(profile: &VmProfile) -> HostHealth {
    ...
    // State check
    let running = crate::vms::state::is_running(profile.name);
    checks.push(Check {
        name: "running",
        ok: running,
        message: if running {
    ...
    // SSH reachability
    match crate::vms::ssh::check(profile) {
    ...
    if profile.os == GuestOs::Windows {
        match crate::vms::winrm::WinRM::from_profile(profile) {
            Ok(winrm) => {
                if winrm.ping() {
                    checks.push(Check {
                        name: "winrm",
