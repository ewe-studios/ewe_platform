...
fn run_scripts_dir(dir: &Path, os: GuestOs, ssh_port: u16, user: &str) -> Result<()> {
    ...
    for entry in &entries {
        ...
        let (output, code) = match (os, ext) {
            (GuestOs::Windows, "ps1") => {
                let mut session = crate::vms::ssh::connect_from_port(ssh_port, user, os)?;
                crate::vms::ssh::exec_ps_windows(&mut session, &content)?
            }
            _ => {
                let mut session = crate::vms::ssh::connect_from_port(ssh_port, user, os)?;
                let wrapped = format!("nu -c {}", shell_quote(&content));
                crate::vms::ssh::exec_with_exit(&mut session, &wrapped)?
            }
        };
