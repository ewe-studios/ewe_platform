...
impl D1Delta {
    pub fn from_env() -> VfsResult<Self> {
        ...
        let database_id = std::env::var("CF_D1_DATABASE_ID").map_err(|e| VfsError::Backend { message: format!("CF_D1_DATABASE_ID not set: {e}") })?;
...
pub struct D1SeekableFile { file: D1File, pos: std::sync::Arc<std::sync::RwLock<u64>> }
...
pub struct D1Directory { path: String, delta: std::sync::Arc<D1Delta> }

impl crate::shared::vfs::traits::VfsDirectory for D1Directory {
    type File = D1File; type SeekableFile = D1SeekableFile;
    ...
    fn create_dir(&self, name: &str) -> VfsResult<Box<dyn crate::shared::vfs::traits::VfsDirectory<File = Self::File, SeekableFile = Self::SeekableFile>>> {
        let p = if self.path.ends_with('/') { format!("{}{}", self.path, name) } else { format!("{}/{}", self.path, name) };
        self.delta.mkdir(&p)?; Ok(Box::new(D1Directory { path: p, delta: self.delta.clone() }))
    }
