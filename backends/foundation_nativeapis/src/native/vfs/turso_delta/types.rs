//! Types for LibsqlDelta: TursoDentry, ChunkConfig, and mapping helpers.
...
/// Convert a turso error to a VfsError.
pub fn turso_err(e: turso::Error) -> VfsError {
...
/// Rust-side representation of a turso_dentry row.
#[derive(Debug, Clone)]
pub struct TursoDentry {
    pub ino: i64,
    pub name: String,
    pub parent_ino: i64,
...
impl TursoDentry {
    /// Extract a TursoDentry from a turso Row.
    pub fn from_row(row: &Row) -> VfsResult<Self> {
        let ino = row.get::<i64>(0).map_err(turso_err)?;
        let name = row.get::<String>(1).map_err(turso_err)?;
        let parent_ino = row.get::<i64>(2).map_err(turso_err)?;
        let file_type_str = row.get::<String>(3).map_err(turso_err)?;
        ...
        let size = row.get::<i64>(4).map_err(turso_err)? as u64;
        let permissions = row.get::<i64>(5).map_err(turso_err)? as u32;
        let owner_uid = row.get::<i64>(6).map_err(turso_err)? as u32;
        let owner_gid = row.get::<i64>(7).map_err(turso_err)? as u32;
        ...
        let created_at = row.get::<i64>(10).map_err(turso_err)? as u64;
        let updated_at = row.get::<i64>(11).map_err(turso_err)? as u64;

        let symlink_target = row.get::<String>(12).ok();

        let chunk_size = row.get::<i64>(13).map_err(turso_err)? as u32;
