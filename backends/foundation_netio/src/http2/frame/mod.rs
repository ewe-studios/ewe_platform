...
mod tests {
    ...
    fn frame_header_roundtrip() {
        ...
        let head = Head { kind: Kind::Headers, flag: headers_flags::END_HEADERS | headers_flags::END_STREAM, stream_id: 1 };
        head.encode(128, &mut buf);
        assert_eq!(buf.len(), 9);
        assert_eq!(buf[3], Kind::Headers as u8);
        assert_eq!(buf[4], 0x01 | 0x04);
        // Parse back
        let mut arr = [0u8; 9];
    ...
    fn data_roundtrip() {
        ...
        let (head, _plen) = Head::parse_with_len(&hdr);
        assert_eq!(head.kind, Kind::Data);

        let parsed = DataFrame::parse(&head, &buf[9..]).unwrap();
        assert_eq!(&parsed.data[..], b"hello");
        assert_eq!(parsed.flags & data_flags::END_STREAM, data_flags::END_STREAM);
    }
    ...
    fn all_ten_kinds_roundtrip() {
        // Encode and decode one of each frame type.
