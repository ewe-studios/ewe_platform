...
mod tests {
    ...
    const INITIAL_WINDOW: WindowSize = 65_535; // RFC 7540 default

    /// Helper: build a FlowControl initialised with `sz` for both windows.
    ...
    fn window_underflow_is_error() {
        ...
        // Drain to i32::MIN then try one more — checked_sub overflows.
        // MAX_WINDOW_SIZE = 2_147_483_647 = i32::MAX
        w.decrease_by(MAX_WINDOW_SIZE).unwrap(); // -1 - 2_147_483_647 = -2_147_483_648 = i32::MIN
        // One more should overflow
        assert!(w.decrease_by(1).is_err());
    }
    ...
    fn inc_window_grows_peer_side() {
        ...
        // available is still 0 — we haven't assigned anything yet
        assert_eq!(fc.available().as_size(), 0);
    }
    ...
    fn claim_capacity_reduces_available() {
        ...
        assert_eq!(fc.window_size(), INITIAL_WINDOW); // unchanged
    }

    #[test]
    fn claim_capacity_can_go_negative() {
        // Claiming more than available pushes the window negative — this is
        // allowed because a subsequent SETTINGS_INITIAL_WINDOW_SIZE change can
        // make the negative window valid again.
        let mut fc = fc_with(100);
        assert!(fc.claim_capacity(101).is_ok());
        assert_eq!(fc.available().raw(), -1);
    }
    ...
    fn unclaimed_none_when_no_capacity_returned() {
        let fc = fc_with(INITIAL_WINDOW);
        // available == window_size → nothing unclaimed
    ...
    fn unclaimed_none_below_threshold() {
        let mut fc = fc_with(INITIAL_WINDOW);
        fc.send_data(1000).unwrap(); // window_size drops
        // Return some but not enough to trigger threshold
    ...
    fn unclaimed_some_above_threshold() {
        let mut fc = fc_with(INITIAL_WINDOW);
        fc.send_data(INITIAL_WINDOW).unwrap(); // drain window
        // Return half the window → should trigger
    ...
    fn dec_send_window_reduces_peer_side() {
        let mut fc = fc_with(INITIAL_WINDOW);
        // Peer tells us to reduce by 1000
        fc.dec_send_window(1000).unwrap();
        assert_eq!(fc.window_size(), INITIAL_WINDOW - 1000);
        assert_eq!(fc.available().as_size(), INITIAL_WINDOW); // unchanged
    }

    #[test]
    fn dec_recv_window_reduces_both() {
        let mut fc = fc_with(INITIAL_WINDOW);
        // We reduce our advertised window
    ...
    fn window_can_go_negative_from_settings_change() {
        ...
        // We reduce INITIAL_WINDOW_SIZE from 65535 to 16384 (diff = 49151)
        fc.dec_send_window(49_151).unwrap();
        // window_size is now: 65535 - 32768 - 49151 = -16384
        assert!(fc.window_size.0 < 0);
        assert_eq!(fc.window_size(), 0); // clamped by as_size
    }
    ...
    fn has_unavailable_false_when_window_negative() {
        ...
        fc.dec_send_window(1).unwrap(); // -1
        assert!(!fc.has_unavailable());
    }

    #[test]
    fn property_window_size_never_exceeds_max() {
        // A quick property: after any sequence of valid operations,
        // window_size() never exceeds MAX_WINDOW_SIZE.
        let mut fc = FlowControl::new();
