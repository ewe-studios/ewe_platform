...
//! (via `foundation_netio::connectrpc_codegen`).
//!
...
//! [`crate::connectrpc_codegen::generate_services`], and writes the result to
...
impl Config {
    ...
    ///    descriptor set.
    /// 2. Passes the descriptor set to `connectrpc_codegen::generate_services`.
    ...
    pub fn compile(self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let connectrpc_code =
            crate::connectrpc_codegen::generate_services(&descriptor_set.file);
