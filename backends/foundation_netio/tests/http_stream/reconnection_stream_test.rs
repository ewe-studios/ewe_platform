...
fn successfully_connects_on_first_try() {
    ...
    let endpoint = foundation_netio::netcap::ClientEndpoint::Plain(panic_if_failed!(
        Endpoint::with_string("http://127.0.0.1:3799")
    ));
...
fn fails_reconnection_after_max_retries() {
    let _pool_guard: PoolGuard = foundation_core::valtron::initialize_pool(42, None);
    let endpoint = foundation_netio::netcap::ClientEndpoint::Plain(panic_if_failed!(
        Endpoint::with_string("http://127.0.0.1:8899")
    ));
