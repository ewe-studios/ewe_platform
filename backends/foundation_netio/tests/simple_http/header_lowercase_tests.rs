...
fn known_variants_render_lowercase() {
    ...
    assert_eq!(SimpleHeader::CONTENT_ENCODING.to_string(), "content-encoding");
    assert_eq!(SimpleHeader::TRANSFER_ENCODING.to_string(), "transfer-encoding");
    assert_eq!(SimpleHeader::SET_COOKIE.to_string(), "set-cookie");
    assert_eq!(SimpleHeader::WWW_AUTHENTICATE.to_string(), "www-authenticate");
    assert_eq!(SimpleHeader::X_CONTENT_TYPE_OPTIONS.to_string(), "x-content-type-options");
...
fn protocol_variants_render_expected_tokens() {
    ...
    assert_eq!(SimpleHeader::GRPC_ACCEPT_ENCODING.to_string(), "grpc-accept-encoding");
    assert_eq!(SimpleHeader::GRPC_TIMEOUT.to_string(), "grpc-timeout");
    assert_eq!(SimpleHeader::GRPC_STATUS_DETAILS_BIN.to_string(), "grpc-status-details-bin");
    assert_eq!(SimpleHeader::CONNECT_PROTOCOL_VERSION.to_string(), "connect-protocol-version");
    assert_eq!(SimpleHeader::CONNECT_TIMEOUT_MS.to_string(), "connect-timeout-ms");
    assert_eq!(SimpleHeader::CONNECT_CONTENT_ENCODING.to_string(), "connect-content-encoding");
    assert_eq!(SimpleHeader::CONNECT_ACCEPT_ENCODING.to_string(), "connect-accept-encoding");
}

/// `Custom` headers keep the caller's exact casing.
#[test]
fn custom_headers_preserve_case() {
    assert_eq!(SimpleHeader::Custom("X-Villa".into()).to_string(), "X-Villa");
...
fn inbound_parsing_is_case_insensitive() {
    for spelling in ["content-type", "Content-Type", "CONTENT-TYPE", "cOnTeNt-TyPe"] {
        assert_eq!(SimpleHeader::from(spelling.to_string()), SimpleHeader::CONTENT_TYPE);
    }
    // Protocol headers parse case-insensitively too.
    for spelling in ["grpc-status", "GRPC-STATUS", "Grpc-Status"] {
        assert_eq!(SimpleHeader::from(spelling.to_string()), SimpleHeader::GRPC_STATUS);
    }
    assert_eq!(
...
fn rendered_response_uses_lowercase_headers() {
    ...
    assert!(!wire.contains("CONTENT-TYPE"), "no uppercase leak: {wire:?}");
}
