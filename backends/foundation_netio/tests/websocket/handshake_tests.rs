...
fn first_header_value(
    headers: &foundation_netio::simple_http::SimpleHeaders,
    key: &SimpleHeader,
) -> Option<String> {
...
fn test_validate_upgrade_response_valid() {
    ...
    let mut headers = foundation_netio::simple_http::SimpleHeaders::new();
    headers.insert(
...
fn test_validate_upgrade_response_wrong_status() {
    ...
    let headers = foundation_netio::simple_http::SimpleHeaders::new();

    let result = validate_upgrade_response(&Status::OK, &headers, &expected_accept);
    assert!(matches!(
        result,
        Err(foundation_netio::websocket::WebSocketError::UpgradeFailed(
            200
        ))
    ));
}
...
fn test_validate_upgrade_response_missing_accept() {
    ...
    assert!(matches!(
        result,
        Err(foundation_netio::websocket::WebSocketError::MissingAcceptKey)
    ));
}
...
fn test_validate_upgrade_response_invalid_accept() {
    ...
    assert!(matches!(
        result,
        Err(foundation_netio::websocket::WebSocketError::InvalidAcceptKey)
    ));
}
