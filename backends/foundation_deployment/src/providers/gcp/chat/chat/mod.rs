//! Auto-generated API module for gcp/chat chat.
//!
//! Generated by `cargo run --bin ewe_platform gen_api`.
//! DO NOT EDIT MANUALLY.
//!
//! Feature flag: `gcp_chat_chat `

#![cfg(feature = "gcp_chat_chat")]
#![allow(clippy::too_many_arguments, clippy::type_complexity)]
#![allow(
    clippy::missing_errors_doc,
    clippy::doc_markdown,
    clippy::useless_format
)]
#![allow(unused_imports)]

use foundation_core::valtron::{TaskIterator, TaskIteratorExt};
use foundation_core::wire::simple_http::client::{ClientRequestBuilder, SimpleHttpClient};
use foundation_macros::JsonHash;
use serde::{Deserialize, Serialize};

// Import shared types used by this module
use super::shared::Empty;

use super::shared::ApiResponse;

// =============================================================================
// TYPE DECLARATIONS
// =============================================================================

/// `SpaceDetails` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct SpaceDetails {
    /// description property.
    pub description: Option<String>,
    /// guidelines property.
    pub guidelines: Option<String>,
}

/// `GoogleAppsCardV1SuggestionItem` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1SuggestionItem {
    /// text property.
    pub text: Option<String>,
}

/// `ChatClientDataSourceMarkup` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ChatClientDataSourceMarkup {
    /// spaceDataSource property.
    pub space_data_source: Option<SpaceDataSource>,
}

/// `DriveDataRef` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct DriveDataRef {
    /// driveFileId property.
    pub drive_file_id: Option<String>,
}

/// `ActionStatus` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ActionStatus {
    /// statusCode property.
    pub status_code: Option<String>,
    /// userFacingMessage property.
    pub user_facing_message: Option<String>,
}

/// `GoogleAppsCardV1CardAction` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1CardAction {
    /// actionLabel property.
    pub action_label: Option<String>,
    /// onClick property.
    pub on_click: Option<Box<GoogleAppsCardV1OnClick>>,
}

/// `CustomEmoji` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct CustomEmoji {
    /// emojiName property.
    pub emoji_name: Option<String>,
    /// name property.
    pub name: Option<String>,
    /// payload property.
    pub payload: Option<CustomEmojiPayload>,
    /// temporaryImageUri property.
    pub temporary_image_uri: Option<String>,
    /// uid property.
    pub uid: Option<String>,
}

/// `Group` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Group {
    /// name property.
    pub name: Option<String>,
}

/// `ReactionDeletedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ReactionDeletedEventData {
    /// reaction property.
    pub reaction: Option<Reaction>,
}

/// `AttachedGif` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct AttachedGif {
    /// uri property.
    pub uri: Option<String>,
}

/// `SpaceReadState` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct SpaceReadState {
    /// lastReadTime property.
    pub last_read_time: Option<String>,
    /// name property.
    pub name: Option<String>,
}

/// `GoogleAppsCardV1Divider` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Divider {}

/// `GoogleAppsCardV1ImageComponent` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1ImageComponent {
    /// altText property.
    pub alt_text: Option<String>,
    /// borderStyle property.
    pub border_style: Option<GoogleAppsCardV1BorderStyle>,
    /// cropStyle property.
    pub crop_style: Option<GoogleAppsCardV1ImageCropStyle>,
    /// imageUri property.
    pub image_uri: Option<String>,
}

/// `GoogleAppsCardV1SwitchControl` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1SwitchControl {
    /// controlType property.
    pub control_type: Option<String>,
    /// name property.
    pub name: Option<String>,
    /// onChangeAction property.
    pub on_change_action: Option<GoogleAppsCardV1Action>,
    /// selected property.
    pub selected: Option<bool>,
    /// value property.
    pub value: Option<String>,
}

/// `Attachment` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Attachment {
    /// attachmentDataRef property.
    pub attachment_data_ref: Option<AttachmentDataRef>,
    /// contentName property.
    pub content_name: Option<String>,
    /// contentType property.
    pub content_type: Option<String>,
    /// downloadUri property.
    pub download_uri: Option<String>,
    /// driveDataRef property.
    pub drive_data_ref: Option<DriveDataRef>,
    /// name property.
    pub name: Option<String>,
    /// source property.
    pub source: Option<String>,
    /// thumbnailUri property.
    pub thumbnail_uri: Option<String>,
}

/// `SpaceEvent` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct SpaceEvent {
    /// eventTime property.
    pub event_time: Option<String>,
    /// eventType property.
    pub event_type: Option<String>,
    /// membershipBatchCreatedEventData property.
    pub membership_batch_created_event_data: Option<MembershipBatchCreatedEventData>,
    /// membershipBatchDeletedEventData property.
    pub membership_batch_deleted_event_data: Option<MembershipBatchDeletedEventData>,
    /// membershipBatchUpdatedEventData property.
    pub membership_batch_updated_event_data: Option<MembershipBatchUpdatedEventData>,
    /// membershipCreatedEventData property.
    pub membership_created_event_data: Option<MembershipCreatedEventData>,
    /// membershipDeletedEventData property.
    pub membership_deleted_event_data: Option<MembershipDeletedEventData>,
    /// membershipUpdatedEventData property.
    pub membership_updated_event_data: Option<MembershipUpdatedEventData>,
    /// messageBatchCreatedEventData property.
    pub message_batch_created_event_data: Option<MessageBatchCreatedEventData>,
    /// messageBatchDeletedEventData property.
    pub message_batch_deleted_event_data: Option<MessageBatchDeletedEventData>,
    /// messageBatchUpdatedEventData property.
    pub message_batch_updated_event_data: Option<MessageBatchUpdatedEventData>,
    /// messageCreatedEventData property.
    pub message_created_event_data: Option<MessageCreatedEventData>,
    /// messageDeletedEventData property.
    pub message_deleted_event_data: Option<MessageDeletedEventData>,
    /// messageUpdatedEventData property.
    pub message_updated_event_data: Option<MessageUpdatedEventData>,
    /// name property.
    pub name: Option<String>,
    /// reactionBatchCreatedEventData property.
    pub reaction_batch_created_event_data: Option<ReactionBatchCreatedEventData>,
    /// reactionBatchDeletedEventData property.
    pub reaction_batch_deleted_event_data: Option<ReactionBatchDeletedEventData>,
    /// reactionCreatedEventData property.
    pub reaction_created_event_data: Option<ReactionCreatedEventData>,
    /// reactionDeletedEventData property.
    pub reaction_deleted_event_data: Option<ReactionDeletedEventData>,
    /// spaceBatchUpdatedEventData property.
    pub space_batch_updated_event_data: Option<SpaceBatchUpdatedEventData>,
    /// spaceUpdatedEventData property.
    pub space_updated_event_data: Option<SpaceUpdatedEventData>,
}

/// `MembershipBatchUpdatedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MembershipBatchUpdatedEventData {
    /// memberships property.
    pub memberships: Option<Vec<MembershipUpdatedEventData>>,
}

/// `PositionSectionResponse` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct PositionSectionResponse {
    /// section property.
    pub section: Option<GoogleChatV1Section>,
}

/// `ReactionBatchCreatedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ReactionBatchCreatedEventData {
    /// reactions property.
    pub reactions: Option<Vec<ReactionCreatedEventData>>,
}

/// `MatchedUrl` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MatchedUrl {
    /// url property.
    pub url: Option<String>,
}

/// `OnClick` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct OnClick {
    /// action property.
    pub action: Option<FormAction>,
    /// openLink property.
    pub open_link: Option<OpenLink>,
}

/// `GoogleAppsCardV1Chip` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Chip {
    /// altText property.
    pub alt_text: Option<String>,
    /// disabled property.
    pub disabled: Option<bool>,
    /// enabled property.
    pub enabled: Option<bool>,
    /// icon property.
    pub icon: Option<GoogleAppsCardV1Icon>,
    /// label property.
    pub label: Option<String>,
    /// onClick property.
    pub on_click: Option<Box<GoogleAppsCardV1OnClick>>,
}

/// `WidgetMarkup` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct WidgetMarkup {
    /// buttons property.
    pub buttons: Option<Vec<Button>>,
    /// image property.
    pub image: Option<Image>,
    /// keyValue property.
    pub key_value: Option<KeyValue>,
    /// textParagraph property.
    pub text_paragraph: Option<TextParagraph>,
}

/// `MessageBatchDeletedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MessageBatchDeletedEventData {
    /// messages property.
    pub messages: Option<Vec<MessageDeletedEventData>>,
}

/// `Card` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Card {
    /// cardActions property.
    pub card_actions: Option<Vec<CardAction>>,
    /// header property.
    pub header: Option<CardHeader>,
    /// name property.
    pub name: Option<String>,
    /// sections property.
    pub sections: Option<Vec<Section>>,
}

/// `FindGroupChatsResponse` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct FindGroupChatsResponse {
    /// nextPageToken property.
    pub next_page_token: Option<String>,
    /// spaces property.
    pub spaces: Option<Vec<Space>>,
}

/// `DeletionMetadata` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct DeletionMetadata {
    /// deletionType property.
    pub deletion_type: Option<String>,
}

/// `GoogleAppsCardV1EventAction` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1EventAction {
    /// actionRuleId property.
    pub action_rule_id: Option<String>,
    /// commonWidgetAction property.
    pub common_widget_action: Option<GoogleAppsCardV1CommonWidgetAction>,
    /// postEventTriggers property.
    pub post_event_triggers: Option<Vec<GoogleAppsCardV1Trigger>>,
}

/// `ListMessagesResponse` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ListMessagesResponse {
    /// messages property.
    pub messages: Option<Vec<Message>>,
    /// nextPageToken property.
    pub next_page_token: Option<String>,
}

/// `PermissionSettings` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct PermissionSettings {
    /// manageApps property.
    pub manage_apps: Option<PermissionSetting>,
    /// manageMembersAndGroups property.
    pub manage_members_and_groups: Option<PermissionSetting>,
    /// manageWebhooks property.
    pub manage_webhooks: Option<PermissionSetting>,
    /// modifySpaceDetails property.
    pub modify_space_details: Option<PermissionSetting>,
    /// postMessages property.
    pub post_messages: Option<PermissionSetting>,
    /// replyMessages property.
    pub reply_messages: Option<PermissionSetting>,
    /// toggleHistory property.
    pub toggle_history: Option<PermissionSetting>,
    /// useAtMentionAll property.
    pub use_at_mention_all: Option<PermissionSetting>,
}

/// `WorkflowDataSourceMarkup` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct WorkflowDataSourceMarkup {
    /// includeVariables property.
    pub include_variables: Option<bool>,
    /// type property.
    pub r#type: Option<String>,
}

/// `GoogleAppsCardV1CarouselCard` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1CarouselCard {
    /// footerWidgets property.
    pub footer_widgets: Option<Vec<Box<GoogleAppsCardV1NestedWidget>>>,
    /// widgets property.
    pub widgets: Option<Vec<Box<GoogleAppsCardV1NestedWidget>>>,
}

/// `GoogleAppsCardV1OnClick` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1OnClick {
    /// action property.
    pub action: Option<GoogleAppsCardV1Action>,
    /// card property.
    pub card: Option<Box<GoogleAppsCardV1Card>>,
    /// openDynamicLinkAction property.
    pub open_dynamic_link_action: Option<GoogleAppsCardV1Action>,
    /// openLink property.
    pub open_link: Option<GoogleAppsCardV1OpenLink>,
    /// overflowMenu property.
    pub overflow_menu: Option<Box<GoogleAppsCardV1OverflowMenu>>,
}

/// `GoogleAppsCardV1OverflowMenuItem` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1OverflowMenuItem {
    /// disabled property.
    pub disabled: Option<bool>,
    /// onClick property.
    pub on_click: Option<Box<GoogleAppsCardV1OnClick>>,
    /// startIcon property.
    pub start_icon: Option<GoogleAppsCardV1Icon>,
    /// text property.
    pub text: Option<String>,
}

/// `QuotedMessageSnapshot` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct QuotedMessageSnapshot {
    /// annotations property.
    pub annotations: Option<Vec<Annotation>>,
    /// attachments property.
    pub attachments: Option<Vec<Attachment>>,
    /// formattedText property.
    pub formatted_text: Option<String>,
    /// sender property.
    pub sender: Option<String>,
    /// text property.
    pub text: Option<String>,
}

/// `GoogleAppsCardV1CardFixedFooter` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1CardFixedFooter {
    /// primaryButton property.
    pub primary_button: Option<Box<GoogleAppsCardV1Button>>,
    /// secondaryButton property.
    pub secondary_button: Option<Box<GoogleAppsCardV1Button>>,
}

/// `AttachmentDataRef` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct AttachmentDataRef {
    /// attachmentUploadToken property.
    pub attachment_upload_token: Option<String>,
    /// resourceName property.
    pub resource_name: Option<String>,
}

/// `GoogleAppsCardV1Column` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Column {
    /// horizontalAlignment property.
    pub horizontal_alignment: Option<String>,
    /// horizontalSizeStyle property.
    pub horizontal_size_style: Option<String>,
    /// verticalAlignment property.
    pub vertical_alignment: Option<String>,
    /// widgets property.
    pub widgets: Option<Vec<Box<GoogleAppsCardV1Widgets>>>,
}

/// `ListSectionsResponse` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ListSectionsResponse {
    /// nextPageToken property.
    pub next_page_token: Option<String>,
    /// sections property.
    pub sections: Option<Vec<GoogleChatV1Section>>,
}

/// `TextParagraph` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct TextParagraph {
    /// text property.
    pub text: Option<String>,
}

/// `KeyValue` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct KeyValue {
    /// bottomLabel property.
    pub bottom_label: Option<String>,
    /// button property.
    pub button: Option<Button>,
    /// content property.
    pub content: Option<String>,
    /// contentMultiline property.
    pub content_multiline: Option<bool>,
    /// icon property.
    pub icon: Option<String>,
    /// iconUrl property.
    pub icon_url: Option<String>,
    /// onClick property.
    pub on_click: Option<OnClick>,
    /// topLabel property.
    pub top_label: Option<String>,
}

/// `GoogleAppsCardV1Action` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Action {
    /// allWidgetsAreRequired property.
    pub all_widgets_are_required: Option<bool>,
    /// function property.
    pub function: Option<String>,
    /// interaction property.
    pub interaction: Option<String>,
    /// loadIndicator property.
    pub load_indicator: Option<String>,
    /// parameters property.
    pub parameters: Option<Vec<GoogleAppsCardV1ActionParameter>>,
    /// persistValues property.
    pub persist_values: Option<bool>,
    /// requiredWidgets property.
    pub required_widgets: Option<Vec<String>>,
}

/// `GoogleAppsCardV1Validation` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Validation {
    /// characterLimit property.
    pub character_limit: Option<i64>,
    /// inputType property.
    pub input_type: Option<String>,
}

/// `Annotation` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Annotation {
    /// customEmojiMetadata property.
    pub custom_emoji_metadata: Option<CustomEmojiMetadata>,
    /// length property.
    pub length: Option<i64>,
    /// richLinkMetadata property.
    pub rich_link_metadata: Option<RichLinkMetadata>,
    /// slashCommand property.
    pub slash_command: Option<SlashCommandMetadata>,
    /// startIndex property.
    pub start_index: Option<i64>,
    /// type property.
    pub r#type: Option<String>,
    /// userMention property.
    pub user_mention: Option<UserMentionMetadata>,
}

/// `UserMentionMetadata` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct UserMentionMetadata {
    /// type property.
    pub r#type: Option<String>,
    /// user property.
    pub user: Option<User>,
}

/// `MeetSpaceLinkData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MeetSpaceLinkData {
    /// huddleStatus property.
    pub huddle_status: Option<String>,
    /// meetingCode property.
    pub meeting_code: Option<String>,
    /// type property.
    pub r#type: Option<String>,
}

/// `ActionResponse` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ActionResponse {
    /// dialogAction property.
    pub dialog_action: Option<DialogAction>,
    /// type property.
    pub r#type: Option<String>,
    /// updatedWidget property.
    pub updated_widget: Option<UpdatedWidget>,
    /// url property.
    pub url: Option<String>,
}

/// `SpaceUpdatedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct SpaceUpdatedEventData {
    /// space property.
    pub space: Option<Space>,
}

/// `MessageBatchUpdatedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MessageBatchUpdatedEventData {
    /// messages property.
    pub messages: Option<Vec<MessageUpdatedEventData>>,
}

/// `ImageButton` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ImageButton {
    /// icon property.
    pub icon: Option<String>,
    /// iconUrl property.
    pub icon_url: Option<String>,
    /// name property.
    pub name: Option<String>,
    /// onClick property.
    pub on_click: Option<OnClick>,
}

/// `GoogleAppsCardV1Image` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Image {
    /// altText property.
    pub alt_text: Option<String>,
    /// imageUrl property.
    pub image_url: Option<String>,
    /// onClick property.
    pub on_click: Option<Box<GoogleAppsCardV1OnClick>>,
}

/// `CardWithId` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct CardWithId {
    /// card property.
    pub card: Option<Box<GoogleAppsCardV1Card>>,
    /// cardId property.
    pub card_id: Option<String>,
}

/// `GoogleAppsCardV1OverflowMenu` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1OverflowMenu {
    /// items property.
    pub items: Option<Vec<Box<GoogleAppsCardV1OverflowMenuItem>>>,
}

/// `OpenLink` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct OpenLink {
    /// url property.
    pub url: Option<String>,
}

/// `PermissionSetting` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct PermissionSetting {
    /// assistantManagersAllowed property.
    pub assistant_managers_allowed: Option<bool>,
    /// managersAllowed property.
    pub managers_allowed: Option<bool>,
    /// membersAllowed property.
    pub members_allowed: Option<bool>,
}

/// `GoogleAppsCardV1CardHeader` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1CardHeader {
    /// imageAltText property.
    pub image_alt_text: Option<String>,
    /// imageType property.
    pub image_type: Option<String>,
    /// imageUrl property.
    pub image_url: Option<String>,
    /// subtitle property.
    pub subtitle: Option<String>,
    /// title property.
    pub title: Option<String>,
}

/// `GoogleAppsCardV1ChipList` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1ChipList {
    /// chips property.
    pub chips: Option<Vec<Box<GoogleAppsCardV1Chip>>>,
    /// layout property.
    pub layout: Option<String>,
}

/// `GoogleAppsCardV1SelectionInput` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1SelectionInput {
    /// dataSourceConfigs property.
    pub data_source_configs: Option<Vec<GoogleAppsCardV1DataSourceConfig>>,
    /// externalDataSource property.
    pub external_data_source: Option<GoogleAppsCardV1Action>,
    /// hintText property.
    pub hint_text: Option<String>,
    /// items property.
    pub items: Option<Vec<GoogleAppsCardV1SelectionItem>>,
    /// label property.
    pub label: Option<String>,
    /// multiSelectMaxSelectedItems property.
    pub multi_select_max_selected_items: Option<i64>,
    /// multiSelectMinQueryLength property.
    pub multi_select_min_query_length: Option<i64>,
    /// name property.
    pub name: Option<String>,
    /// onChangeAction property.
    pub on_change_action: Option<GoogleAppsCardV1Action>,
    /// platformDataSource property.
    pub platform_data_source: Option<GoogleAppsCardV1PlatformDataSource>,
    /// type property.
    pub r#type: Option<String>,
}

/// `GoogleAppsCardV1TextParagraph` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1TextParagraph {
    /// maxLines property.
    pub max_lines: Option<i64>,
    /// text property.
    pub text: Option<String>,
    /// textSyntax property.
    pub text_syntax: Option<String>,
}

/// `SelectionItems` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct SelectionItems {
    /// items property.
    pub items: Option<Vec<GoogleAppsCardV1SelectionItem>>,
}

/// `Dialog` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Dialog {
    /// body property.
    pub body: Option<Box<GoogleAppsCardV1Card>>,
}

/// `GoogleAppsCardV1Columns` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Columns {
    /// columnItems property.
    pub column_items: Option<Vec<Box<GoogleAppsCardV1Column>>>,
}

/// `GoogleAppsCardV1GridItem` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1GridItem {
    /// id property.
    pub id: Option<String>,
    /// image property.
    pub image: Option<GoogleAppsCardV1ImageComponent>,
    /// layout property.
    pub layout: Option<String>,
    /// subtitle property.
    pub subtitle: Option<String>,
    /// title property.
    pub title: Option<String>,
}

/// `GoogleAppsCardV1Card` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Card {
    /// cardActions property.
    pub card_actions: Option<Vec<Box<GoogleAppsCardV1CardAction>>>,
    /// displayStyle property.
    pub display_style: Option<String>,
    /// expressionData property.
    pub expression_data: Option<Vec<GoogleAppsCardV1ExpressionData>>,
    /// fixedFooter property.
    pub fixed_footer: Option<Box<GoogleAppsCardV1CardFixedFooter>>,
    /// header property.
    pub header: Option<GoogleAppsCardV1CardHeader>,
    /// name property.
    pub name: Option<String>,
    /// peekCardHeader property.
    pub peek_card_header: Option<GoogleAppsCardV1CardHeader>,
    /// sectionDividerStyle property.
    pub section_divider_style: Option<String>,
    /// sections property.
    pub sections: Option<Vec<Box<GoogleAppsCardV1Section>>>,
}

/// `GoogleAppsCardV1Suggestions` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Suggestions {
    /// items property.
    pub items: Option<Vec<GoogleAppsCardV1SuggestionItem>>,
}

/// `GoogleAppsCardV1Button` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Button {
    /// altText property.
    pub alt_text: Option<String>,
    /// color property.
    pub color: Option<Color>,
    /// disabled property.
    pub disabled: Option<bool>,
    /// icon property.
    pub icon: Option<GoogleAppsCardV1Icon>,
    /// onClick property.
    pub on_click: Option<Box<GoogleAppsCardV1OnClick>>,
    /// text property.
    pub text: Option<String>,
    /// type property.
    pub r#type: Option<String>,
}

/// `GoogleAppsCardV1SelectionItem` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1SelectionItem {
    /// bottomText property.
    pub bottom_text: Option<String>,
    /// selected property.
    pub selected: Option<bool>,
    /// startIconUri property.
    pub start_icon_uri: Option<String>,
    /// text property.
    pub text: Option<String>,
    /// value property.
    pub value: Option<String>,
}

/// `SpaceDataSource` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct SpaceDataSource {
    /// defaultToCurrentSpace property.
    pub default_to_current_space: Option<bool>,
}

/// `MoveSectionItemResponse` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MoveSectionItemResponse {
    /// sectionItem property.
    pub section_item: Option<SectionItem>,
}

/// `GoogleChatV1Section` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleChatV1Section {
    /// displayName property.
    pub display_name: Option<String>,
    /// name property.
    pub name: Option<String>,
    /// sortOrder property.
    pub sort_order: Option<i64>,
    /// type property.
    pub r#type: Option<String>,
}

/// `Space` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Space {
    /// accessSettings property.
    pub access_settings: Option<AccessSettings>,
    /// adminInstalled property.
    pub admin_installed: Option<bool>,
    /// createTime property.
    pub create_time: Option<String>,
    /// customer property.
    pub customer: Option<String>,
    /// displayName property.
    pub display_name: Option<String>,
    /// externalUserAllowed property.
    pub external_user_allowed: Option<bool>,
    /// importMode property.
    pub import_mode: Option<bool>,
    /// importModeExpireTime property.
    pub import_mode_expire_time: Option<String>,
    /// lastActiveTime property.
    pub last_active_time: Option<String>,
    /// membershipCount property.
    pub membership_count: Option<MembershipCount>,
    /// name property.
    pub name: Option<String>,
    /// permissionSettings property.
    pub permission_settings: Option<PermissionSettings>,
    /// predefinedPermissionSettings property.
    pub predefined_permission_settings: Option<String>,
    /// singleUserBotDm property.
    pub single_user_bot_dm: Option<bool>,
    /// spaceDetails property.
    pub space_details: Option<SpaceDetails>,
    /// spaceHistoryState property.
    pub space_history_state: Option<String>,
    /// spaceThreadingState property.
    pub space_threading_state: Option<String>,
    /// spaceType property.
    pub space_type: Option<String>,
    /// spaceUri property.
    pub space_uri: Option<String>,
    /// threaded property.
    pub threaded: Option<bool>,
    /// type property.
    pub r#type: Option<String>,
}

/// `MessageBatchCreatedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MessageBatchCreatedEventData {
    /// messages property.
    pub messages: Option<Vec<MessageCreatedEventData>>,
}

/// `GoogleAppsCardV1Icon` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Icon {
    /// altText property.
    pub alt_text: Option<String>,
    /// iconUrl property.
    pub icon_url: Option<String>,
    /// imageType property.
    pub image_type: Option<String>,
    /// knownIcon property.
    pub known_icon: Option<String>,
    /// materialIcon property.
    pub material_icon: Option<GoogleAppsCardV1MaterialIcon>,
}

/// `GoogleAppsCardV1ExpressionDataCondition` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1ExpressionDataCondition {
    /// conditionType property.
    pub condition_type: Option<String>,
}

/// `ActionParameter` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ActionParameter {
    /// key property.
    pub key: Option<String>,
    /// value property.
    pub value: Option<String>,
}

/// `UploadAttachmentResponse` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct UploadAttachmentResponse {
    /// attachmentDataRef property.
    pub attachment_data_ref: Option<AttachmentDataRef>,
}

/// `MembershipBatchCreatedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MembershipBatchCreatedEventData {
    /// memberships property.
    pub memberships: Option<Vec<MembershipCreatedEventData>>,
}

/// `MembershipUpdatedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MembershipUpdatedEventData {
    /// membership property.
    pub membership: Option<Membership>,
}

/// `GoogleAppsCardV1UpdateVisibilityAction` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1UpdateVisibilityAction {
    /// visibility property.
    pub visibility: Option<String>,
}

/// `CompleteImportSpaceRequest` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct CompleteImportSpaceRequest {}

/// `DriveLinkData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct DriveLinkData {
    /// driveDataRef property.
    pub drive_data_ref: Option<DriveDataRef>,
    /// mimeType property.
    pub mime_type: Option<String>,
}

/// `MessageUpdatedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MessageUpdatedEventData {
    /// message property.
    pub message: Option<Message>,
}

/// `SpaceNotificationSetting` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct SpaceNotificationSetting {
    /// muteSetting property.
    pub mute_setting: Option<String>,
    /// name property.
    pub name: Option<String>,
    /// notificationSetting property.
    pub notification_setting: Option<String>,
}

/// `EmojiReactionSummary` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct EmojiReactionSummary {
    /// emoji property.
    pub emoji: Option<Emoji>,
    /// reactionCount property.
    pub reaction_count: Option<i64>,
}

/// `GoogleAppsCardV1OpenLink` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1OpenLink {
    /// onClose property.
    pub on_close: Option<String>,
    /// openAs property.
    pub open_as: Option<String>,
    /// url property.
    pub url: Option<String>,
}

/// `User` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct User {
    /// displayName property.
    pub display_name: Option<String>,
    /// domainId property.
    pub domain_id: Option<String>,
    /// isAnonymous property.
    pub is_anonymous: Option<bool>,
    /// name property.
    pub name: Option<String>,
    /// type property.
    pub r#type: Option<String>,
}

/// `Message` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Message {
    /// accessoryWidgets property.
    pub accessory_widgets: Option<Vec<AccessoryWidget>>,
    /// actionResponse property.
    pub action_response: Option<ActionResponse>,
    /// annotations property.
    pub annotations: Option<Vec<Annotation>>,
    /// argumentText property.
    pub argument_text: Option<String>,
    /// attachedGifs property.
    pub attached_gifs: Option<Vec<AttachedGif>>,
    /// attachment property.
    pub attachment: Option<Vec<Attachment>>,
    /// cards property.
    pub cards: Option<Vec<Card>>,
    /// cardsV2 property.
    pub cards_v2: Option<Vec<CardWithId>>,
    /// clientAssignedMessageId property.
    pub client_assigned_message_id: Option<String>,
    /// createTime property.
    pub create_time: Option<String>,
    /// deleteTime property.
    pub delete_time: Option<String>,
    /// deletionMetadata property.
    pub deletion_metadata: Option<DeletionMetadata>,
    /// emojiReactionSummaries property.
    pub emoji_reaction_summaries: Option<Vec<EmojiReactionSummary>>,
    /// fallbackText property.
    pub fallback_text: Option<String>,
    /// formattedText property.
    pub formatted_text: Option<String>,
    /// lastUpdateTime property.
    pub last_update_time: Option<String>,
    /// matchedUrl property.
    pub matched_url: Option<MatchedUrl>,
    /// name property.
    pub name: Option<String>,
    /// privateMessageViewer property.
    pub private_message_viewer: Option<User>,
    /// quotedMessageMetadata property.
    pub quoted_message_metadata: Option<QuotedMessageMetadata>,
    /// sender property.
    pub sender: Option<User>,
    /// slashCommand property.
    pub slash_command: Option<SlashCommand>,
    /// space property.
    pub space: Option<Space>,
    /// text property.
    pub text: Option<String>,
    /// thread property.
    pub thread: Option<Thread>,
    /// threadReply property.
    pub thread_reply: Option<bool>,
}

/// `UpdatedWidget` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct UpdatedWidget {
    /// suggestions property.
    pub suggestions: Option<SelectionItems>,
    /// widget property.
    pub widget: Option<String>,
}

/// `GoogleAppsCardV1PlatformDataSource` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1PlatformDataSource {
    /// commonDataSource property.
    pub common_data_source: Option<String>,
    /// hostAppDataSource property.
    pub host_app_data_source: Option<HostAppDataSourceMarkup>,
}

/// `GoogleAppsCardV1Widgets` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Widgets {
    /// buttonList property.
    pub button_list: Option<Box<GoogleAppsCardV1ButtonList>>,
    /// chipList property.
    pub chip_list: Option<Box<GoogleAppsCardV1ChipList>>,
    /// dateTimePicker property.
    pub date_time_picker: Option<GoogleAppsCardV1DateTimePicker>,
    /// decoratedText property.
    pub decorated_text: Option<Box<GoogleAppsCardV1DecoratedText>>,
    /// image property.
    pub image: Option<Box<GoogleAppsCardV1Image>>,
    /// selectionInput property.
    pub selection_input: Option<GoogleAppsCardV1SelectionInput>,
    /// textInput property.
    pub text_input: Option<GoogleAppsCardV1TextInput>,
    /// textParagraph property.
    pub text_paragraph: Option<GoogleAppsCardV1TextParagraph>,
}

/// `MoveSectionItemRequest` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MoveSectionItemRequest {
    /// targetSection property.
    pub target_section: Option<String>,
}

/// `HostAppDataSourceMarkup` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct HostAppDataSourceMarkup {
    /// chatDataSource property.
    pub chat_data_source: Option<ChatClientDataSourceMarkup>,
    /// workflowDataSource property.
    pub workflow_data_source: Option<WorkflowDataSourceMarkup>,
}

/// `GoogleAppsCardV1Condition` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Condition {
    /// actionRuleId property.
    pub action_rule_id: Option<String>,
    /// expressionDataCondition property.
    pub expression_data_condition: Option<GoogleAppsCardV1ExpressionDataCondition>,
}

/// `FormAction` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct FormAction {
    /// actionMethodName property.
    pub action_method_name: Option<String>,
    /// parameters property.
    pub parameters: Option<Vec<ActionParameter>>,
}

/// `Reaction` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Reaction {
    /// emoji property.
    pub emoji: Option<Emoji>,
    /// name property.
    pub name: Option<String>,
    /// user property.
    pub user: Option<User>,
}

/// `SlashCommand` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct SlashCommand {
    /// commandId property.
    pub command_id: Option<String>,
}

/// `SlashCommandMetadata` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct SlashCommandMetadata {
    /// bot property.
    pub bot: Option<User>,
    /// commandId property.
    pub command_id: Option<String>,
    /// commandName property.
    pub command_name: Option<String>,
    /// triggersDialog property.
    pub triggers_dialog: Option<bool>,
    /// type property.
    pub r#type: Option<String>,
}

/// `ThreadReadState` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ThreadReadState {
    /// lastReadTime property.
    pub last_read_time: Option<String>,
    /// name property.
    pub name: Option<String>,
}

/// `ListSpaceEventsResponse` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ListSpaceEventsResponse {
    /// nextPageToken property.
    pub next_page_token: Option<String>,
    /// spaceEvents property.
    pub space_events: Option<Vec<SpaceEvent>>,
}

/// `Emoji` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Emoji {
    /// customEmoji property.
    pub custom_emoji: Option<CustomEmoji>,
    /// unicode property.
    pub unicode: Option<String>,
}

/// `PositionSectionRequest` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct PositionSectionRequest {
    /// relativePosition property.
    pub relative_position: Option<String>,
    /// sortOrder property.
    pub sort_order: Option<i64>,
}

/// `AccessSettings` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct AccessSettings {
    /// accessState property.
    pub access_state: Option<String>,
    /// audience property.
    pub audience: Option<String>,
}

/// `GoogleAppsCardV1BorderStyle` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1BorderStyle {
    /// cornerRadius property.
    pub corner_radius: Option<i64>,
    /// strokeColor property.
    pub stroke_color: Option<Color>,
    /// type property.
    pub r#type: Option<String>,
}

/// `GoogleAppsCardV1ImageCropStyle` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1ImageCropStyle {
    /// aspectRatio property.
    pub aspect_ratio: Option<f64>,
    /// type property.
    pub r#type: Option<String>,
}

/// `ForwardedMetadata` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ForwardedMetadata {
    /// space property.
    pub space: Option<String>,
    /// spaceDisplayName property.
    pub space_display_name: Option<String>,
}

/// `Membership` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Membership {
    /// createTime property.
    pub create_time: Option<String>,
    /// deleteTime property.
    pub delete_time: Option<String>,
    /// groupMember property.
    pub group_member: Option<Group>,
    /// member property.
    pub member: Option<User>,
    /// name property.
    pub name: Option<String>,
    /// role property.
    pub role: Option<String>,
    /// state property.
    pub state: Option<String>,
}

/// `GoogleAppsCardV1MaterialIcon` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1MaterialIcon {
    /// fill property.
    pub fill: Option<bool>,
    /// grade property.
    pub grade: Option<i64>,
    /// name property.
    pub name: Option<String>,
    /// weight property.
    pub weight: Option<i64>,
}

/// `Section` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Section {
    /// header property.
    pub header: Option<String>,
    /// widgets property.
    pub widgets: Option<Vec<WidgetMarkup>>,
}

/// `Image` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Image {
    /// aspectRatio property.
    pub aspect_ratio: Option<f64>,
    /// imageUrl property.
    pub image_url: Option<String>,
    /// onClick property.
    pub on_click: Option<OnClick>,
}

/// `ListMembershipsResponse` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ListMembershipsResponse {
    /// memberships property.
    pub memberships: Option<Vec<Membership>>,
    /// nextPageToken property.
    pub next_page_token: Option<String>,
}

/// `GoogleAppsCardV1CollapseControl` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1CollapseControl {
    /// collapseButton property.
    pub collapse_button: Option<Box<GoogleAppsCardV1Button>>,
    /// expandButton property.
    pub expand_button: Option<Box<GoogleAppsCardV1Button>>,
    /// horizontalAlignment property.
    pub horizontal_alignment: Option<String>,
}

/// `ChatSpaceLinkData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ChatSpaceLinkData {
    /// message property.
    pub message: Option<String>,
    /// space property.
    pub space: Option<String>,
    /// thread property.
    pub thread: Option<String>,
}

/// `ListSpacesResponse` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ListSpacesResponse {
    /// nextPageToken property.
    pub next_page_token: Option<String>,
    /// spaces property.
    pub spaces: Option<Vec<Space>>,
}

/// `CardAction` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct CardAction {
    /// actionLabel property.
    pub action_label: Option<String>,
    /// onClick property.
    pub on_click: Option<OnClick>,
}

/// `GoogleAppsCardV1ExpressionData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1ExpressionData {
    /// conditions property.
    pub conditions: Option<Vec<GoogleAppsCardV1Condition>>,
    /// eventActions property.
    pub event_actions: Option<Vec<GoogleAppsCardV1EventAction>>,
    /// expression property.
    pub expression: Option<String>,
    /// id property.
    pub id: Option<String>,
}

/// `SectionItem` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct SectionItem {
    /// name property.
    pub name: Option<String>,
    /// space property.
    pub space: Option<String>,
}

/// `GoogleAppsCardV1ButtonList` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1ButtonList {
    /// buttons property.
    pub buttons: Option<Vec<Box<GoogleAppsCardV1Button>>>,
}

/// `MembershipCreatedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MembershipCreatedEventData {
    /// membership property.
    pub membership: Option<Membership>,
}

/// `SearchSpacesResponse` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct SearchSpacesResponse {
    /// nextPageToken property.
    pub next_page_token: Option<String>,
    /// spaces property.
    pub spaces: Option<Vec<Space>>,
    /// totalSize property.
    pub total_size: Option<i64>,
}

/// `GoogleAppsCardV1Widget` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Widget {
    /// buttonList property.
    pub button_list: Option<Box<GoogleAppsCardV1ButtonList>>,
    /// carousel property.
    pub carousel: Option<Box<GoogleAppsCardV1Carousel>>,
    /// chipList property.
    pub chip_list: Option<Box<GoogleAppsCardV1ChipList>>,
    /// columns property.
    pub columns: Option<Box<GoogleAppsCardV1Columns>>,
    /// dateTimePicker property.
    pub date_time_picker: Option<GoogleAppsCardV1DateTimePicker>,
    /// decoratedText property.
    pub decorated_text: Option<Box<GoogleAppsCardV1DecoratedText>>,
    /// divider property.
    pub divider: Option<GoogleAppsCardV1Divider>,
    /// eventActions property.
    pub event_actions: Option<Vec<GoogleAppsCardV1EventAction>>,
    /// grid property.
    pub grid: Option<Box<GoogleAppsCardV1Grid>>,
    /// horizontalAlignment property.
    pub horizontal_alignment: Option<String>,
    /// id property.
    pub id: Option<String>,
    /// image property.
    pub image: Option<Box<GoogleAppsCardV1Image>>,
    /// selectionInput property.
    pub selection_input: Option<GoogleAppsCardV1SelectionInput>,
    /// textInput property.
    pub text_input: Option<GoogleAppsCardV1TextInput>,
    /// textParagraph property.
    pub text_paragraph: Option<GoogleAppsCardV1TextParagraph>,
    /// visibility property.
    pub visibility: Option<String>,
}

/// `CustomEmojiPayload` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct CustomEmojiPayload {
    /// fileContent property.
    pub file_content: Option<String>,
    /// filename property.
    pub filename: Option<String>,
}

/// `Color` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Color {
    /// alpha property.
    pub alpha: Option<f64>,
    /// blue property.
    pub blue: Option<f64>,
    /// green property.
    pub green: Option<f64>,
    /// red property.
    pub red: Option<f64>,
}

/// `ListReactionsResponse` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ListReactionsResponse {
    /// nextPageToken property.
    pub next_page_token: Option<String>,
    /// reactions property.
    pub reactions: Option<Vec<Reaction>>,
}

/// `ReactionCreatedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ReactionCreatedEventData {
    /// reaction property.
    pub reaction: Option<Reaction>,
}

/// `GoogleAppsCardV1Trigger` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Trigger {
    /// actionRuleId property.
    pub action_rule_id: Option<String>,
}

/// `ListCustomEmojisResponse` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ListCustomEmojisResponse {
    /// customEmojis property.
    pub custom_emojis: Option<Vec<CustomEmoji>>,
    /// nextPageToken property.
    pub next_page_token: Option<String>,
}

/// `Button` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Button {
    /// imageButton property.
    pub image_button: Option<ImageButton>,
    /// textButton property.
    pub text_button: Option<TextButton>,
}

/// `ListSectionItemsResponse` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ListSectionItemsResponse {
    /// nextPageToken property.
    pub next_page_token: Option<String>,
    /// sectionItems property.
    pub section_items: Option<Vec<SectionItem>>,
}

/// `MembershipCount` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MembershipCount {
    /// joinedDirectHumanUserCount property.
    pub joined_direct_human_user_count: Option<i64>,
    /// joinedGroupCount property.
    pub joined_group_count: Option<i64>,
}

/// `SetUpSpaceRequest` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct SetUpSpaceRequest {
    /// memberships property.
    pub memberships: Option<Vec<Membership>>,
    /// requestId property.
    pub request_id: Option<String>,
    /// space property.
    pub space: Option<Space>,
}

/// `Thread` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Thread {
    /// name property.
    pub name: Option<String>,
    /// threadKey property.
    pub thread_key: Option<String>,
}

/// `GoogleAppsCardV1Section` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Section {
    /// collapseControl property.
    pub collapse_control: Option<Box<GoogleAppsCardV1CollapseControl>>,
    /// collapsible property.
    pub collapsible: Option<bool>,
    /// header property.
    pub header: Option<String>,
    /// id property.
    pub id: Option<String>,
    /// uncollapsibleWidgetsCount property.
    pub uncollapsible_widgets_count: Option<i64>,
    /// widgets property.
    pub widgets: Option<Vec<Box<GoogleAppsCardV1Widget>>>,
}

/// `Media` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Media {
    /// resourceName property.
    pub resource_name: Option<String>,
}

/// `GoogleAppsCardV1DateTimePicker` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1DateTimePicker {
    /// hostAppDataSource property.
    pub host_app_data_source: Option<HostAppDataSourceMarkup>,
    /// label property.
    pub label: Option<String>,
    /// name property.
    pub name: Option<String>,
    /// onChangeAction property.
    pub on_change_action: Option<GoogleAppsCardV1Action>,
    /// timezoneOffsetDate property.
    pub timezone_offset_date: Option<i64>,
    /// type property.
    pub r#type: Option<String>,
    /// valueMsEpoch property.
    pub value_ms_epoch: Option<String>,
}

/// `CalendarEventLinkData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct CalendarEventLinkData {
    /// calendarId property.
    pub calendar_id: Option<String>,
    /// eventId property.
    pub event_id: Option<String>,
}

/// `MessageCreatedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MessageCreatedEventData {
    /// message property.
    pub message: Option<Message>,
}

/// `UploadAttachmentRequest` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct UploadAttachmentRequest {
    /// filename property.
    pub filename: Option<String>,
}

/// `GoogleAppsCardV1TextInput` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1TextInput {
    /// autoCompleteAction property.
    pub auto_complete_action: Option<GoogleAppsCardV1Action>,
    /// hintText property.
    pub hint_text: Option<String>,
    /// hostAppDataSource property.
    pub host_app_data_source: Option<HostAppDataSourceMarkup>,
    /// initialSuggestions property.
    pub initial_suggestions: Option<GoogleAppsCardV1Suggestions>,
    /// label property.
    pub label: Option<String>,
    /// name property.
    pub name: Option<String>,
    /// onChangeAction property.
    pub on_change_action: Option<GoogleAppsCardV1Action>,
    /// placeholderText property.
    pub placeholder_text: Option<String>,
    /// type property.
    pub r#type: Option<String>,
    /// validation property.
    pub validation: Option<GoogleAppsCardV1Validation>,
    /// value property.
    pub value: Option<String>,
}

/// `QuotedMessageMetadata` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct QuotedMessageMetadata {
    /// forwardedMetadata property.
    pub forwarded_metadata: Option<ForwardedMetadata>,
    /// lastUpdateTime property.
    pub last_update_time: Option<String>,
    /// name property.
    pub name: Option<String>,
    /// quoteType property.
    pub quote_type: Option<String>,
    /// quotedMessageSnapshot property.
    pub quoted_message_snapshot: Option<QuotedMessageSnapshot>,
}

/// `GoogleAppsCardV1CommonWidgetAction` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1CommonWidgetAction {
    /// updateVisibilityAction property.
    pub update_visibility_action: Option<GoogleAppsCardV1UpdateVisibilityAction>,
}

/// `CompleteImportSpaceResponse` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct CompleteImportSpaceResponse {
    /// space property.
    pub space: Option<Space>,
}

/// `GoogleAppsCardV1Carousel` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Carousel {
    /// carouselCards property.
    pub carousel_cards: Option<Vec<Box<GoogleAppsCardV1CarouselCard>>>,
}

/// `GoogleAppsCardV1NestedWidget` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1NestedWidget {
    /// buttonList property.
    pub button_list: Option<Box<GoogleAppsCardV1ButtonList>>,
    /// image property.
    pub image: Option<Box<GoogleAppsCardV1Image>>,
    /// textParagraph property.
    pub text_paragraph: Option<GoogleAppsCardV1TextParagraph>,
}

/// `GoogleAppsCardV1DecoratedText` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1DecoratedText {
    /// bottomLabel property.
    pub bottom_label: Option<String>,
    /// bottomLabelText property.
    pub bottom_label_text: Option<GoogleAppsCardV1TextParagraph>,
    /// button property.
    pub button: Option<Box<GoogleAppsCardV1Button>>,
    /// contentText property.
    pub content_text: Option<GoogleAppsCardV1TextParagraph>,
    /// endIcon property.
    pub end_icon: Option<GoogleAppsCardV1Icon>,
    /// icon property.
    pub icon: Option<GoogleAppsCardV1Icon>,
    /// onClick property.
    pub on_click: Option<Box<GoogleAppsCardV1OnClick>>,
    /// startIcon property.
    pub start_icon: Option<GoogleAppsCardV1Icon>,
    /// startIconVerticalAlignment property.
    pub start_icon_vertical_alignment: Option<String>,
    /// switchControl property.
    pub switch_control: Option<GoogleAppsCardV1SwitchControl>,
    /// text property.
    pub text: Option<String>,
    /// topLabel property.
    pub top_label: Option<String>,
    /// topLabelText property.
    pub top_label_text: Option<GoogleAppsCardV1TextParagraph>,
    /// wrapText property.
    pub wrap_text: Option<bool>,
}

/// `SpaceBatchUpdatedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct SpaceBatchUpdatedEventData {
    /// spaces property.
    pub spaces: Option<Vec<SpaceUpdatedEventData>>,
}

/// `AccessoryWidget` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct AccessoryWidget {
    /// buttonList property.
    pub button_list: Option<Box<GoogleAppsCardV1ButtonList>>,
}

/// `GoogleAppsCardV1Grid` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1Grid {
    /// borderStyle property.
    pub border_style: Option<GoogleAppsCardV1BorderStyle>,
    /// columnCount property.
    pub column_count: Option<i64>,
    /// items property.
    pub items: Option<Vec<GoogleAppsCardV1GridItem>>,
    /// onClick property.
    pub on_click: Option<Box<GoogleAppsCardV1OnClick>>,
    /// title property.
    pub title: Option<String>,
}

/// `GoogleAppsCardV1DataSourceConfig` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1DataSourceConfig {
    /// minCharactersTrigger property.
    pub min_characters_trigger: Option<i64>,
    /// platformDataSource property.
    pub platform_data_source: Option<GoogleAppsCardV1PlatformDataSource>,
    /// remoteDataSource property.
    pub remote_data_source: Option<GoogleAppsCardV1Action>,
}

/// `CardHeader` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct CardHeader {
    /// imageStyle property.
    pub image_style: Option<String>,
    /// imageUrl property.
    pub image_url: Option<String>,
    /// subtitle property.
    pub subtitle: Option<String>,
    /// title property.
    pub title: Option<String>,
}

/// `MessageDeletedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MessageDeletedEventData {
    /// message property.
    pub message: Option<Message>,
}

/// `MembershipBatchDeletedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MembershipBatchDeletedEventData {
    /// memberships property.
    pub memberships: Option<Vec<MembershipDeletedEventData>>,
}

/// `GoogleAppsCardV1ActionParameter` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GoogleAppsCardV1ActionParameter {
    /// key property.
    pub key: Option<String>,
    /// value property.
    pub value: Option<String>,
}

/// `ReactionBatchDeletedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ReactionBatchDeletedEventData {
    /// reactions property.
    pub reactions: Option<Vec<ReactionDeletedEventData>>,
}

/// `CustomEmojiMetadata` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct CustomEmojiMetadata {
    /// customEmoji property.
    pub custom_emoji: Option<CustomEmoji>,
}

/// `MembershipDeletedEventData` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct MembershipDeletedEventData {
    /// membership property.
    pub membership: Option<Membership>,
}

/// `TextButton` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct TextButton {
    /// onClick property.
    pub on_click: Option<OnClick>,
    /// text property.
    pub text: Option<String>,
}

/// `RichLinkMetadata` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct RichLinkMetadata {
    /// calendarEventLinkData property.
    pub calendar_event_link_data: Option<CalendarEventLinkData>,
    /// chatSpaceLinkData property.
    pub chat_space_link_data: Option<ChatSpaceLinkData>,
    /// driveLinkData property.
    pub drive_link_data: Option<DriveLinkData>,
    /// meetSpaceLinkData property.
    pub meet_space_link_data: Option<MeetSpaceLinkData>,
    /// richLinkType property.
    pub rich_link_type: Option<String>,
    /// uri property.
    pub uri: Option<String>,
}

/// `DialogAction` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct DialogAction {
    /// actionStatus property.
    pub action_status: Option<ActionStatus>,
    /// dialog property.
    pub dialog: Option<Dialog>,
}

// =============================================================================
// ARGS TYPES (per-endpoint)
// =============================================================================

/// Arguments for [`chat.customEmojis.create_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatCustomEmojisCreateArgs {
    /// Request body.
    pub body: CustomEmoji,
}

/// Arguments for [`chat.customEmojis.delete_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatCustomEmojisDeleteArgs {
    /// Path parameter: `name`.
    pub name: String,
}

/// Arguments for [`chat.customEmojis.get_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatCustomEmojisGetArgs {
    /// Path parameter: `name`.
    pub name: String,
}

/// Arguments for [`chat.customEmojis.list_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatCustomEmojisListArgs {
    /// Query parameter: `filter`.
    pub filter: Option<String>,
    /// Query parameter: `pageSize`.
    pub page_size: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
}

/// Arguments for [`chat.media.download_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatMediaDownloadArgs {
    /// Path parameter: `resourceName`.
    pub resource_name: String,
}

/// Arguments for [`chat.media.upload_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatMediaUploadArgs {
    /// Path parameter: `parent`.
    pub parent: String,
    /// Request body.
    pub body: UploadAttachmentRequest,
}

/// Arguments for [`chat.spaces.completeImport_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesCompleteImportArgs {
    /// Path parameter: `name`.
    pub name: String,
    /// Request body.
    pub body: CompleteImportSpaceRequest,
}

/// Arguments for [`chat.spaces.create_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesCreateArgs {
    /// Query parameter: `requestId`.
    pub request_id: Option<String>,
    /// Request body.
    pub body: Space,
}

/// Arguments for [`chat.spaces.delete_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesDeleteArgs {
    /// Path parameter: `name`.
    pub name: String,
    /// Query parameter: `useAdminAccess`.
    pub use_admin_access: Option<String>,
}

/// Arguments for [`chat.spaces.findDirectMessage_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesFindDirectMessageArgs {
    /// Query parameter: `name`.
    pub name: Option<String>,
}

/// Arguments for [`chat.spaces.findGroupChats_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesFindGroupChatsArgs {
    /// Query parameter: `pageSize`.
    pub page_size: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
    /// Query parameter: `spaceView`.
    pub space_view: Option<String>,
    /// Query parameter: `users`.
    pub users: Option<String>,
}

/// Arguments for [`chat.spaces.get_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesGetArgs {
    /// Path parameter: `name`.
    pub name: String,
    /// Query parameter: `useAdminAccess`.
    pub use_admin_access: Option<String>,
}

/// Arguments for [`chat.spaces.list_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesListArgs {
    /// Query parameter: `filter`.
    pub filter: Option<String>,
    /// Query parameter: `pageSize`.
    pub page_size: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
}

/// Arguments for [`chat.spaces.patch_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesPatchArgs {
    /// Path parameter: `name`.
    pub name: String,
    /// Query parameter: `updateMask`.
    pub update_mask: Option<String>,
    /// Query parameter: `useAdminAccess`.
    pub use_admin_access: Option<String>,
    /// Request body.
    pub body: Space,
}

/// Arguments for [`chat.spaces.search_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesSearchArgs {
    /// Query parameter: `orderBy`.
    pub order_by: Option<String>,
    /// Query parameter: `pageSize`.
    pub page_size: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
    /// Query parameter: `query`.
    pub query: Option<String>,
    /// Query parameter: `useAdminAccess`.
    pub use_admin_access: Option<String>,
}

/// Arguments for [`chat.spaces.setup_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesSetupArgs {
    /// Request body.
    pub body: SetUpSpaceRequest,
}

/// Arguments for [`chat.spaces.members.create_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesMembersCreateArgs {
    /// Path parameter: `parent`.
    pub parent: String,
    /// Query parameter: `useAdminAccess`.
    pub use_admin_access: Option<String>,
    /// Request body.
    pub body: Membership,
}

/// Arguments for [`chat.spaces.members.delete_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesMembersDeleteArgs {
    /// Path parameter: `name`.
    pub name: String,
    /// Query parameter: `useAdminAccess`.
    pub use_admin_access: Option<String>,
}

/// Arguments for [`chat.spaces.members.get_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesMembersGetArgs {
    /// Path parameter: `name`.
    pub name: String,
    /// Query parameter: `useAdminAccess`.
    pub use_admin_access: Option<String>,
}

/// Arguments for [`chat.spaces.members.list_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesMembersListArgs {
    /// Path parameter: `parent`.
    pub parent: String,
    /// Query parameter: `filter`.
    pub filter: Option<String>,
    /// Query parameter: `pageSize`.
    pub page_size: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
    /// Query parameter: `showGroups`.
    pub show_groups: Option<String>,
    /// Query parameter: `showInvited`.
    pub show_invited: Option<String>,
    /// Query parameter: `useAdminAccess`.
    pub use_admin_access: Option<String>,
}

/// Arguments for [`chat.spaces.members.patch_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesMembersPatchArgs {
    /// Path parameter: `name`.
    pub name: String,
    /// Query parameter: `updateMask`.
    pub update_mask: Option<String>,
    /// Query parameter: `useAdminAccess`.
    pub use_admin_access: Option<String>,
    /// Request body.
    pub body: Membership,
}

/// Arguments for [`chat.spaces.messages.create_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesMessagesCreateArgs {
    /// Path parameter: `parent`.
    pub parent: String,
    /// Query parameter: `messageId`.
    pub message_id: Option<String>,
    /// Query parameter: `messageReplyOption`.
    pub message_reply_option: Option<String>,
    /// Query parameter: `requestId`.
    pub request_id: Option<String>,
    /// Query parameter: `threadKey`.
    pub thread_key: Option<String>,
    /// Request body.
    pub body: Message,
}

/// Arguments for [`chat.spaces.messages.delete_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesMessagesDeleteArgs {
    /// Path parameter: `name`.
    pub name: String,
    /// Query parameter: `force`.
    pub force: Option<String>,
}

/// Arguments for [`chat.spaces.messages.get_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesMessagesGetArgs {
    /// Path parameter: `name`.
    pub name: String,
}

/// Arguments for [`chat.spaces.messages.list_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesMessagesListArgs {
    /// Path parameter: `parent`.
    pub parent: String,
    /// Query parameter: `filter`.
    pub filter: Option<String>,
    /// Query parameter: `orderBy`.
    pub order_by: Option<String>,
    /// Query parameter: `pageSize`.
    pub page_size: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
    /// Query parameter: `showDeleted`.
    pub show_deleted: Option<String>,
}

/// Arguments for [`chat.spaces.messages.patch_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesMessagesPatchArgs {
    /// Path parameter: `name`.
    pub name: String,
    /// Query parameter: `allowMissing`.
    pub allow_missing: Option<String>,
    /// Query parameter: `updateMask`.
    pub update_mask: Option<String>,
    /// Request body.
    pub body: Message,
}

/// Arguments for [`chat.spaces.messages.update_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesMessagesUpdateArgs {
    /// Path parameter: `name`.
    pub name: String,
    /// Query parameter: `allowMissing`.
    pub allow_missing: Option<String>,
    /// Query parameter: `updateMask`.
    pub update_mask: Option<String>,
    /// Request body.
    pub body: Message,
}

/// Arguments for [`chat.spaces.messages.attachments.get_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesMessagesAttachmentsGetArgs {
    /// Path parameter: `name`.
    pub name: String,
}

/// Arguments for [`chat.spaces.messages.reactions.create_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesMessagesReactionsCreateArgs {
    /// Path parameter: `parent`.
    pub parent: String,
    /// Request body.
    pub body: Reaction,
}

/// Arguments for [`chat.spaces.messages.reactions.delete_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesMessagesReactionsDeleteArgs {
    /// Path parameter: `name`.
    pub name: String,
}

/// Arguments for [`chat.spaces.messages.reactions.list_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesMessagesReactionsListArgs {
    /// Path parameter: `parent`.
    pub parent: String,
    /// Query parameter: `filter`.
    pub filter: Option<String>,
    /// Query parameter: `pageSize`.
    pub page_size: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
}

/// Arguments for [`chat.spaces.spaceEvents.get_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesSpaceEventsGetArgs {
    /// Path parameter: `name`.
    pub name: String,
}

/// Arguments for [`chat.spaces.spaceEvents.list_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatSpacesSpaceEventsListArgs {
    /// Path parameter: `parent`.
    pub parent: String,
    /// Query parameter: `filter`.
    pub filter: Option<String>,
    /// Query parameter: `pageSize`.
    pub page_size: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
}

/// Arguments for [`chat.users.sections.create_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatUsersSectionsCreateArgs {
    /// Path parameter: `parent`.
    pub parent: String,
    /// Request body.
    pub body: GoogleChatV1Section,
}

/// Arguments for [`chat.users.sections.delete_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatUsersSectionsDeleteArgs {
    /// Path parameter: `name`.
    pub name: String,
}

/// Arguments for [`chat.users.sections.list_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatUsersSectionsListArgs {
    /// Path parameter: `parent`.
    pub parent: String,
    /// Query parameter: `pageSize`.
    pub page_size: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
}

/// Arguments for [`chat.users.sections.patch_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatUsersSectionsPatchArgs {
    /// Path parameter: `name`.
    pub name: String,
    /// Query parameter: `updateMask`.
    pub update_mask: Option<String>,
    /// Request body.
    pub body: GoogleChatV1Section,
}

/// Arguments for [`chat.users.sections.position_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatUsersSectionsPositionArgs {
    /// Path parameter: `name`.
    pub name: String,
    /// Request body.
    pub body: PositionSectionRequest,
}

/// Arguments for [`chat.users.sections.items.list_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatUsersSectionsItemsListArgs {
    /// Path parameter: `parent`.
    pub parent: String,
    /// Query parameter: `filter`.
    pub filter: Option<String>,
    /// Query parameter: `pageSize`.
    pub page_size: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
}

/// Arguments for [`chat.users.sections.items.move_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatUsersSectionsItemsMoveArgs {
    /// Path parameter: `name`.
    pub name: String,
    /// Request body.
    pub body: MoveSectionItemRequest,
}

/// Arguments for [`chat.users.spaces.getSpaceReadState_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatUsersSpacesGetSpaceReadStateArgs {
    /// Path parameter: `name`.
    pub name: String,
}

/// Arguments for [`chat.users.spaces.updateSpaceReadState_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatUsersSpacesUpdateSpaceReadStateArgs {
    /// Path parameter: `name`.
    pub name: String,
    /// Query parameter: `updateMask`.
    pub update_mask: Option<String>,
    /// Request body.
    pub body: SpaceReadState,
}

/// Arguments for [`chat.users.spaces.spaceNotificationSetting.get_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatUsersSpacesSpaceNotificationSettingGetArgs {
    /// Path parameter: `name`.
    pub name: String,
}

/// Arguments for [`chat.users.spaces.spaceNotificationSetting.patch_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatUsersSpacesSpaceNotificationSettingPatchArgs {
    /// Path parameter: `name`.
    pub name: String,
    /// Query parameter: `updateMask`.
    pub update_mask: Option<String>,
    /// Request body.
    pub body: SpaceNotificationSetting,
}

/// Arguments for [`chat.users.spaces.threads.getThreadReadState_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ChatUsersSpacesThreadsGetThreadReadStateArgs {
    /// Path parameter: `name`.
    pub name: String,
}

// =============================================================================
// CLIENT FUNCTIONS (per-endpoint)
// =============================================================================

// -----------------------------------------------------------------------------
// POST v1/customEmojis
// -----------------------------------------------------------------------------

/// POST v1/customEmojis.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_custom_emojis_create_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_custom_emojis_create_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatCustomEmojisCreateArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<CustomEmoji>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/customEmojis",);

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: CustomEmoji =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// DELETE v1/{+name}
// -----------------------------------------------------------------------------

/// DELETE v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_custom_emojis_delete_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_custom_emojis_delete_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatCustomEmojisDeleteArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Empty>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let mut builder = client
        .delete(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Empty =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/{+name}
// -----------------------------------------------------------------------------

/// GET v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_custom_emojis_get_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_custom_emojis_get_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatCustomEmojisGetArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<CustomEmoji>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: CustomEmoji =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/customEmojis
// -----------------------------------------------------------------------------

/// GET v1/customEmojis.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_custom_emojis_list_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_custom_emojis_list_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatCustomEmojisListArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<ListCustomEmojisResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/customEmojis",);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.filter {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("filter=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_size {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageSize=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_token {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageToken=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: ListCustomEmojisResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/media/{+resourceName}
// -----------------------------------------------------------------------------

/// GET v1/media/{+resourceName}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_media_download_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_media_download_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatMediaDownloadArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Media>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://chat.googleapis.com/v1/media/{}",
        args.resource_name,
    );

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Media =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST v1/{+parent}/attachments:upload
// -----------------------------------------------------------------------------

/// POST v1/{+parent}/attachments:upload.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_media_upload_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_media_upload_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatMediaUploadArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<UploadAttachmentResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://chat.googleapis.com/v1/{}/attachments:upload",
        args.parent,
    );

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: UploadAttachmentResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST v1/{+name}:completeImport
// -----------------------------------------------------------------------------

/// POST v1/{+name}:completeImport.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_complete_import_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_complete_import_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesCompleteImportArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<CompleteImportSpaceResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://chat.googleapis.com/v1/{}:completeImport",
        args.name,
    );

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: CompleteImportSpaceResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST v1/spaces
// -----------------------------------------------------------------------------

/// POST v1/spaces.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_create_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_create_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesCreateArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Space>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/spaces",);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.request_id {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("requestId=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Space =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// DELETE v1/{+name}
// -----------------------------------------------------------------------------

/// DELETE v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_delete_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_delete_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesDeleteArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Empty>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.use_admin_access {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("useAdminAccess=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .delete(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Empty =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/spaces:findDirectMessage
// -----------------------------------------------------------------------------

/// GET v1/spaces:findDirectMessage.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_find_direct_message_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_find_direct_message_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesFindDirectMessageArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Space>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/spaces:findDirectMessage",);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.name {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("name=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Space =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/spaces:findGroupChats
// -----------------------------------------------------------------------------

/// GET v1/spaces:findGroupChats.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_find_group_chats_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_find_group_chats_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesFindGroupChatsArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<FindGroupChatsResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/spaces:findGroupChats",);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.page_size {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageSize=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_token {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageToken=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.space_view {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("spaceView=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.users {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("users=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: FindGroupChatsResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/{+name}
// -----------------------------------------------------------------------------

/// GET v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_get_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_get_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesGetArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Space>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.use_admin_access {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("useAdminAccess=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Space =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/spaces
// -----------------------------------------------------------------------------

/// GET v1/spaces.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_list_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_list_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesListArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<ListSpacesResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/spaces",);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.filter {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("filter=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_size {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageSize=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_token {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageToken=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: ListSpacesResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// PATCH v1/{+name}
// -----------------------------------------------------------------------------

/// PATCH v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_patch_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_patch_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesPatchArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Space>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.update_mask {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("updateMask=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.use_admin_access {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("useAdminAccess=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .patch(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Space =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/spaces:search
// -----------------------------------------------------------------------------

/// GET v1/spaces:search.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_search_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_search_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesSearchArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<SearchSpacesResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/spaces:search",);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.order_by {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("orderBy=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_size {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageSize=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_token {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageToken=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.query {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("query=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.use_admin_access {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("useAdminAccess=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: SearchSpacesResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST v1/spaces:setup
// -----------------------------------------------------------------------------

/// POST v1/spaces:setup.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_setup_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_setup_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesSetupArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Space>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/spaces:setup",);

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Space =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST v1/{+parent}/members
// -----------------------------------------------------------------------------

/// POST v1/{+parent}/members.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_members_create_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_members_create_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesMembersCreateArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Membership>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}/members", args.parent,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.use_admin_access {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("useAdminAccess=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Membership =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// DELETE v1/{+name}
// -----------------------------------------------------------------------------

/// DELETE v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_members_delete_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_members_delete_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesMembersDeleteArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Membership>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.use_admin_access {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("useAdminAccess=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .delete(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Membership =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/{+name}
// -----------------------------------------------------------------------------

/// GET v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_members_get_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_members_get_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesMembersGetArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Membership>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.use_admin_access {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("useAdminAccess=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Membership =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/{+parent}/members
// -----------------------------------------------------------------------------

/// GET v1/{+parent}/members.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_members_list_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_members_list_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesMembersListArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<ListMembershipsResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}/members", args.parent,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.filter {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("filter=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_size {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageSize=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_token {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageToken=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.show_groups {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("showGroups=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.show_invited {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("showInvited=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.use_admin_access {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("useAdminAccess=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: ListMembershipsResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// PATCH v1/{+name}
// -----------------------------------------------------------------------------

/// PATCH v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_members_patch_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_members_patch_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesMembersPatchArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Membership>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.update_mask {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("updateMask=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.use_admin_access {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("useAdminAccess=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .patch(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Membership =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST v1/{+parent}/messages
// -----------------------------------------------------------------------------

/// POST v1/{+parent}/messages.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_messages_create_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_messages_create_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesMessagesCreateArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Message>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}/messages", args.parent,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.message_id {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("messageId=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.message_reply_option {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("messageReplyOption=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.request_id {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("requestId=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.thread_key {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("threadKey=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Message =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// DELETE v1/{+name}
// -----------------------------------------------------------------------------

/// DELETE v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_messages_delete_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_messages_delete_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesMessagesDeleteArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Empty>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.force {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("force=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .delete(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Empty =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/{+name}
// -----------------------------------------------------------------------------

/// GET v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_messages_get_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_messages_get_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesMessagesGetArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Message>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Message =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/{+parent}/messages
// -----------------------------------------------------------------------------

/// GET v1/{+parent}/messages.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_messages_list_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_messages_list_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesMessagesListArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<ListMessagesResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}/messages", args.parent,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.filter {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("filter=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.order_by {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("orderBy=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_size {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageSize=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_token {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageToken=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.show_deleted {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("showDeleted=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: ListMessagesResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// PATCH v1/{+name}
// -----------------------------------------------------------------------------

/// PATCH v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_messages_patch_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_messages_patch_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesMessagesPatchArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Message>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.allow_missing {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("allowMissing=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.update_mask {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("updateMask=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .patch(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Message =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// PUT v1/{+name}
// -----------------------------------------------------------------------------

/// PUT v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_messages_update_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_messages_update_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesMessagesUpdateArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Message>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.allow_missing {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("allowMissing=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.update_mask {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("updateMask=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .put(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Message =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/{+name}
// -----------------------------------------------------------------------------

/// GET v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_messages_attachments_get_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_messages_attachments_get_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesMessagesAttachmentsGetArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Attachment>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Attachment =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST v1/{+parent}/reactions
// -----------------------------------------------------------------------------

/// POST v1/{+parent}/reactions.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_messages_reactions_create_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_messages_reactions_create_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesMessagesReactionsCreateArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Reaction>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}/reactions", args.parent,);

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Reaction =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// DELETE v1/{+name}
// -----------------------------------------------------------------------------

/// DELETE v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_messages_reactions_delete_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_messages_reactions_delete_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesMessagesReactionsDeleteArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Empty>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let mut builder = client
        .delete(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Empty =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/{+parent}/reactions
// -----------------------------------------------------------------------------

/// GET v1/{+parent}/reactions.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_messages_reactions_list_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_messages_reactions_list_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesMessagesReactionsListArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<ListReactionsResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}/reactions", args.parent,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.filter {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("filter=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_size {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageSize=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_token {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageToken=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: ListReactionsResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/{+name}
// -----------------------------------------------------------------------------

/// GET v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_space_events_get_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_space_events_get_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesSpaceEventsGetArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<SpaceEvent>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: SpaceEvent =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/{+parent}/spaceEvents
// -----------------------------------------------------------------------------

/// GET v1/{+parent}/spaceEvents.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_spaces_space_events_list_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_spaces_space_events_list_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatSpacesSpaceEventsListArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<ListSpaceEventsResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}/spaceEvents", args.parent,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.filter {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("filter=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_size {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageSize=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_token {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageToken=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: ListSpaceEventsResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST v1/{+parent}/sections
// -----------------------------------------------------------------------------

/// POST v1/{+parent}/sections.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_users_sections_create_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_users_sections_create_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatUsersSectionsCreateArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<GoogleChatV1Section>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}/sections", args.parent,);

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: GoogleChatV1Section =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// DELETE v1/{+name}
// -----------------------------------------------------------------------------

/// DELETE v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_users_sections_delete_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_users_sections_delete_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatUsersSectionsDeleteArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Empty>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let mut builder = client
        .delete(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Empty =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/{+parent}/sections
// -----------------------------------------------------------------------------

/// GET v1/{+parent}/sections.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_users_sections_list_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_users_sections_list_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatUsersSectionsListArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<ListSectionsResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}/sections", args.parent,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.page_size {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageSize=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_token {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageToken=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: ListSectionsResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// PATCH v1/{+name}
// -----------------------------------------------------------------------------

/// PATCH v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_users_sections_patch_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_users_sections_patch_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatUsersSectionsPatchArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<GoogleChatV1Section>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.update_mask {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("updateMask=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .patch(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: GoogleChatV1Section =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST v1/{+name}:position
// -----------------------------------------------------------------------------

/// POST v1/{+name}:position.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_users_sections_position_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_users_sections_position_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatUsersSectionsPositionArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<PositionSectionResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}:position", args.name,);

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: PositionSectionResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/{+parent}/items
// -----------------------------------------------------------------------------

/// GET v1/{+parent}/items.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_users_sections_items_list_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_users_sections_items_list_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatUsersSectionsItemsListArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<ListSectionItemsResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}/items", args.parent,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.filter {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("filter=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_size {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageSize=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_token {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageToken=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: ListSectionItemsResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST v1/{+name}:move
// -----------------------------------------------------------------------------

/// POST v1/{+name}:move.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_users_sections_items_move_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_users_sections_items_move_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatUsersSectionsItemsMoveArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<MoveSectionItemResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}:move", args.name,);

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: MoveSectionItemResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/{+name}
// -----------------------------------------------------------------------------

/// GET v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_users_spaces_get_space_read_state_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_users_spaces_get_space_read_state_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatUsersSpacesGetSpaceReadStateArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<SpaceReadState>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: SpaceReadState =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// PATCH v1/{+name}
// -----------------------------------------------------------------------------

/// PATCH v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_users_spaces_update_space_read_state_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_users_spaces_update_space_read_state_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatUsersSpacesUpdateSpaceReadStateArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<SpaceReadState>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.update_mask {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("updateMask=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .patch(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: SpaceReadState =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/{+name}
// -----------------------------------------------------------------------------

/// GET v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_users_spaces_space_notification_setting_get_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_users_spaces_space_notification_setting_get_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatUsersSpacesSpaceNotificationSettingGetArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<SpaceNotificationSetting>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: SpaceNotificationSetting =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// PATCH v1/{+name}
// -----------------------------------------------------------------------------

/// PATCH v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_users_spaces_space_notification_setting_patch_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_users_spaces_space_notification_setting_patch_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatUsersSpacesSpaceNotificationSettingPatchArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<SpaceNotificationSetting>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.update_mask {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("updateMask=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .patch(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: SpaceNotificationSetting =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET v1/{+name}
// -----------------------------------------------------------------------------

/// GET v1/{+name}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = chat_users_spaces_threads_get_thread_read_state_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn chat_users_spaces_threads_get_thread_read_state_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ChatUsersSpacesThreadsGetThreadReadStateArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<ThreadReadState>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://chat.googleapis.com/v1/{}", args.name,);

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: ThreadReadState =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}
