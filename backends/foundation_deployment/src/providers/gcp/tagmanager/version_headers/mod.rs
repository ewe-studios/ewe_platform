//! Auto-generated API module for gcp/tagmanager version_headers.
//!
//! Generated by `cargo run --bin ewe_platform gen_api`.
//! DO NOT EDIT MANUALLY.
//!
//! Feature flag: `gcp_tagmanager_version_headers `

#![cfg(feature = "gcp_tagmanager_version_headers")]
#![allow(clippy::too_many_arguments, clippy::type_complexity)]
#![allow(
    clippy::missing_errors_doc,
    clippy::doc_markdown,
    clippy::useless_format
)]

use foundation_core::valtron::{execute, StreamIterator, TaskIterator, TaskIteratorExt};
use foundation_core::wire::simple_http::client::{ClientRequestBuilder, SimpleHttpClient};
use foundation_macros::JsonHash;
use serde::{Deserialize, Serialize};

use super::shared::{ApiError, ApiPending, ApiResponse};

// =============================================================================
// TYPE DECLARATIONS
// =============================================================================

/// `ListContainerVersionsResponse` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct ListContainerVersionsResponse {
    /// containerVersionHeader property.
    pub container_version_header: Option<Vec<ContainerVersionHeader>>,
    /// nextPageToken property.
    pub next_page_token: Option<String>,
}

/// `ContainerVersionHeader` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct ContainerVersionHeader {
    /// accountId property.
    pub account_id: Option<String>,
    /// containerId property.
    pub container_id: Option<String>,
    /// containerVersionId property.
    pub container_version_id: Option<String>,
    /// deleted property.
    pub deleted: Option<bool>,
    /// name property.
    pub name: Option<String>,
    /// numClients property.
    pub num_clients: Option<String>,
    /// numCustomTemplates property.
    pub num_custom_templates: Option<String>,
    /// numGtagConfigs property.
    pub num_gtag_configs: Option<String>,
    /// numTags property.
    pub num_tags: Option<String>,
    /// numTransformations property.
    pub num_transformations: Option<String>,
    /// numTriggers property.
    pub num_triggers: Option<String>,
    /// numVariables property.
    pub num_variables: Option<String>,
    /// numZones property.
    pub num_zones: Option<String>,
    /// path property.
    pub path: Option<String>,
}

// =============================================================================
// ARGS TYPES (per-endpoint)
// =============================================================================

/// Arguments for [`tagmanager.accounts.containers.version_headers.list_builder`].
#[derive(Debug, Clone, Serialize, JsonHash)]
pub struct TagmanagerAccountsContainersVersionHeadersListArgs {
    /// Path parameter: `parent`.
    pub parent: String,
    /// Query parameter: `includeDeleted`.
    pub include_deleted: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
}

// =============================================================================
// CLIENT FUNCTIONS (per-endpoint)
// =============================================================================

// -----------------------------------------------------------------------------
// GET tagmanager/v2/accounts/{accountsId}/containers/{containersId}/version_headers
// -----------------------------------------------------------------------------

/// GET tagmanager/v2/accounts/{accountsId}/containers/{containersId}/version_headers.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = tagmanager_accounts_containers_version_headers_list_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn tagmanager_accounts_containers_version_headers_list_request<R, F>(
    client: &SimpleHttpClient<R>,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<ListContainerVersionsResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://tagmanager.googleapis.com/tagmanager/v2/accounts/{{accountsId}}/containers/{{containersId}}/version_headers",
    );

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: ListContainerVersionsResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}
