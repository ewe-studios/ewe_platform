//! Auto-generated API module for gcp/tagmanager workspaces.
//!
//! Generated by `cargo run --bin ewe_platform gen_api`.
//! DO NOT EDIT MANUALLY.
//!
//! Feature flag: `gcp_tagmanager_workspaces `

#![cfg(feature = "gcp_tagmanager_workspaces")]
#![allow(clippy::too_many_arguments, clippy::type_complexity)]
#![allow(
    clippy::missing_errors_doc,
    clippy::doc_markdown,
    clippy::useless_format
)]

use foundation_core::valtron::{execute, StreamIterator, TaskIterator, TaskIteratorExt};
use foundation_core::wire::simple_http::client::{ClientRequestBuilder, SimpleHttpClient};
use foundation_macros::JsonHash;
use serde::{Deserialize, Serialize};

use super::shared::{ApiError, ApiPending, ApiResponse};

// =============================================================================
// TYPE DECLARATIONS
// =============================================================================

/// `Folder` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct Folder {
    /// accountId property.
    pub account_id: Option<String>,
    /// containerId property.
    pub container_id: Option<String>,
    /// fingerprint property.
    pub fingerprint: Option<String>,
    /// folderId property.
    pub folder_id: Option<String>,
    /// name property.
    pub name: Option<String>,
    /// notes property.
    pub notes: Option<String>,
    /// path property.
    pub path: Option<String>,
    /// tagManagerUrl property.
    pub tag_manager_url: Option<String>,
    /// workspaceId property.
    pub workspace_id: Option<String>,
}

/// `ContainerFeatures` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct ContainerFeatures {
    /// supportBuiltInVariables property.
    pub support_built_in_variables: Option<bool>,
    /// supportClients property.
    pub support_clients: Option<bool>,
    /// supportEnvironments property.
    pub support_environments: Option<bool>,
    /// supportFolders property.
    pub support_folders: Option<bool>,
    /// supportGtagConfigs property.
    pub support_gtag_configs: Option<bool>,
    /// supportTags property.
    pub support_tags: Option<bool>,
    /// supportTemplates property.
    pub support_templates: Option<bool>,
    /// supportTransformations property.
    pub support_transformations: Option<bool>,
    /// supportTriggers property.
    pub support_triggers: Option<bool>,
    /// supportUserPermissions property.
    pub support_user_permissions: Option<bool>,
    /// supportVariables property.
    pub support_variables: Option<bool>,
    /// supportVersions property.
    pub support_versions: Option<bool>,
    /// supportWorkspaces property.
    pub support_workspaces: Option<bool>,
    /// supportZones property.
    pub support_zones: Option<bool>,
}

/// `Condition` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct Condition {
    /// parameter property.
    pub parameter: Option<Vec<Parameter>>,
    /// type property.
    pub r#type: Option<String>,
}

/// `BuiltInVariable` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct BuiltInVariable {
    /// accountId property.
    pub account_id: Option<String>,
    /// containerId property.
    pub container_id: Option<String>,
    /// name property.
    pub name: Option<String>,
    /// path property.
    pub path: Option<String>,
    /// type property.
    pub r#type: Option<String>,
    /// workspaceId property.
    pub workspace_id: Option<String>,
}

/// `TeardownTag` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct TeardownTag {
    /// stopTeardownOnFailure property.
    pub stop_teardown_on_failure: Option<bool>,
    /// tagName property.
    pub tag_name: Option<String>,
}

/// `Variable` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct Variable {
    /// accountId property.
    pub account_id: Option<String>,
    /// containerId property.
    pub container_id: Option<String>,
    /// disablingTriggerId property.
    pub disabling_trigger_id: Option<Vec<String>>,
    /// enablingTriggerId property.
    pub enabling_trigger_id: Option<Vec<String>>,
    /// fingerprint property.
    pub fingerprint: Option<String>,
    /// formatValue property.
    pub format_value: Option<VariableFormatValue>,
    /// name property.
    pub name: Option<String>,
    /// notes property.
    pub notes: Option<String>,
    /// parameter property.
    pub parameter: Option<Vec<Parameter>>,
    /// parentFolderId property.
    pub parent_folder_id: Option<String>,
    /// path property.
    pub path: Option<String>,
    /// scheduleEndMs property.
    pub schedule_end_ms: Option<String>,
    /// scheduleStartMs property.
    pub schedule_start_ms: Option<String>,
    /// tagManagerUrl property.
    pub tag_manager_url: Option<String>,
    /// type property.
    pub r#type: Option<String>,
    /// variableId property.
    pub variable_id: Option<String>,
    /// workspaceId property.
    pub workspace_id: Option<String>,
}

/// `CreateContainerVersionResponse` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct CreateContainerVersionResponse {
    /// compilerError property.
    pub compiler_error: Option<bool>,
    /// containerVersion property.
    pub container_version: Option<ContainerVersion>,
    /// newWorkspacePath property.
    pub new_workspace_path: Option<String>,
    /// syncStatus property.
    pub sync_status: Option<SyncStatus>,
}

/// `GalleryReference` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct GalleryReference {
    /// galleryTemplateId property.
    pub gallery_template_id: Option<String>,
    /// host property.
    pub host: Option<String>,
    /// isModified property.
    pub is_modified: Option<bool>,
    /// owner property.
    pub owner: Option<String>,
    /// repository property.
    pub repository: Option<String>,
    /// signature property.
    pub signature: Option<String>,
    /// templateDeveloperId property.
    pub template_developer_id: Option<String>,
    /// version property.
    pub version: Option<String>,
}

/// `ListWorkspacesResponse` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct ListWorkspacesResponse {
    /// nextPageToken property.
    pub next_page_token: Option<String>,
    /// workspace property.
    pub workspace: Option<Vec<Workspace>>,
}

/// `MergeConflict` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct MergeConflict {
    /// entityInBaseVersion property.
    pub entity_in_base_version: Option<Entity>,
    /// entityInWorkspace property.
    pub entity_in_workspace: Option<Entity>,
}

/// `ZoneBoundary` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct ZoneBoundary {
    /// condition property.
    pub condition: Option<Vec<Condition>>,
    /// customEvaluationTriggerId property.
    pub custom_evaluation_trigger_id: Option<Vec<String>>,
}

/// `Entity` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct Entity {
    /// builtInVariable property.
    pub built_in_variable: Option<BuiltInVariable>,
    /// changeStatus property.
    pub change_status: Option<String>,
    /// client property.
    pub client: Option<Client>,
    /// customTemplate property.
    pub custom_template: Option<CustomTemplate>,
    /// folder property.
    pub folder: Option<Folder>,
    /// gtagConfig property.
    pub gtag_config: Option<GtagConfig>,
    /// tag property.
    pub tag: Option<Tag>,
    /// transformation property.
    pub transformation: Option<Transformation>,
    /// trigger property.
    pub trigger: Option<Trigger>,
    /// variable property.
    pub variable: Option<Variable>,
    /// zone property.
    pub zone: Option<Zone>,
}

/// `Transformation` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct Transformation {
    /// accountId property.
    pub account_id: Option<String>,
    /// containerId property.
    pub container_id: Option<String>,
    /// fingerprint property.
    pub fingerprint: Option<String>,
    /// name property.
    pub name: Option<String>,
    /// notes property.
    pub notes: Option<String>,
    /// parameter property.
    pub parameter: Option<Vec<Parameter>>,
    /// parentFolderId property.
    pub parent_folder_id: Option<String>,
    /// path property.
    pub path: Option<String>,
    /// tagManagerUrl property.
    pub tag_manager_url: Option<String>,
    /// transformationId property.
    pub transformation_id: Option<String>,
    /// type property.
    pub r#type: Option<String>,
    /// workspaceId property.
    pub workspace_id: Option<String>,
}

/// `Parameter` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct Parameter {
    /// isWeakReference property.
    pub is_weak_reference: Option<bool>,
    /// key property.
    pub key: Option<String>,
    /// list property.
    pub list: Option<Vec<Parameter>>,
    /// map property.
    pub map: Option<Vec<Parameter>>,
    /// type property.
    pub r#type: Option<String>,
    /// value property.
    pub value: Option<String>,
}

/// `ZoneChildContainer` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct ZoneChildContainer {
    /// nickname property.
    pub nickname: Option<String>,
    /// publicId property.
    pub public_id: Option<String>,
}

/// `GtagConfig` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct GtagConfig {
    /// accountId property.
    pub account_id: Option<String>,
    /// containerId property.
    pub container_id: Option<String>,
    /// fingerprint property.
    pub fingerprint: Option<String>,
    /// gtagConfigId property.
    pub gtag_config_id: Option<String>,
    /// parameter property.
    pub parameter: Option<Vec<Parameter>>,
    /// path property.
    pub path: Option<String>,
    /// tagManagerUrl property.
    pub tag_manager_url: Option<String>,
    /// type property.
    pub r#type: Option<String>,
    /// workspaceId property.
    pub workspace_id: Option<String>,
}

/// `SetupTag` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct SetupTag {
    /// stopOnSetupFailure property.
    pub stop_on_setup_failure: Option<bool>,
    /// tagName property.
    pub tag_name: Option<String>,
}

/// `SyncWorkspaceResponse` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct SyncWorkspaceResponse {
    /// mergeConflict property.
    pub merge_conflict: Option<Vec<MergeConflict>>,
    /// syncStatus property.
    pub sync_status: Option<SyncStatus>,
}

/// `SyncStatus` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct SyncStatus {
    /// mergeConflict property.
    pub merge_conflict: Option<bool>,
    /// syncError property.
    pub sync_error: Option<bool>,
}

/// `Client` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct Client {
    /// accountId property.
    pub account_id: Option<String>,
    /// clientId property.
    pub client_id: Option<String>,
    /// containerId property.
    pub container_id: Option<String>,
    /// fingerprint property.
    pub fingerprint: Option<String>,
    /// name property.
    pub name: Option<String>,
    /// notes property.
    pub notes: Option<String>,
    /// parameter property.
    pub parameter: Option<Vec<Parameter>>,
    /// parentFolderId property.
    pub parent_folder_id: Option<String>,
    /// path property.
    pub path: Option<String>,
    /// priority property.
    pub priority: Option<i64>,
    /// tagManagerUrl property.
    pub tag_manager_url: Option<String>,
    /// type property.
    pub r#type: Option<String>,
    /// workspaceId property.
    pub workspace_id: Option<String>,
}

/// `Tag` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct Tag {
    /// accountId property.
    pub account_id: Option<String>,
    /// blockingTriggerId property.
    pub blocking_trigger_id: Option<Vec<String>>,
    /// consentSettings property.
    pub consent_settings: Option<TagConsentSetting>,
    /// containerId property.
    pub container_id: Option<String>,
    /// fingerprint property.
    pub fingerprint: Option<String>,
    /// firingTriggerId property.
    pub firing_trigger_id: Option<Vec<String>>,
    /// liveOnly property.
    pub live_only: Option<bool>,
    /// monitoringMetadata property.
    pub monitoring_metadata: Option<Parameter>,
    /// monitoringMetadataTagNameKey property.
    pub monitoring_metadata_tag_name_key: Option<String>,
    /// name property.
    pub name: Option<String>,
    /// notes property.
    pub notes: Option<String>,
    /// parameter property.
    pub parameter: Option<Vec<Parameter>>,
    /// parentFolderId property.
    pub parent_folder_id: Option<String>,
    /// path property.
    pub path: Option<String>,
    /// paused property.
    pub paused: Option<bool>,
    /// priority property.
    pub priority: Option<Parameter>,
    /// scheduleEndMs property.
    pub schedule_end_ms: Option<String>,
    /// scheduleStartMs property.
    pub schedule_start_ms: Option<String>,
    /// setupTag property.
    pub setup_tag: Option<Vec<SetupTag>>,
    /// tagFiringOption property.
    pub tag_firing_option: Option<String>,
    /// tagId property.
    pub tag_id: Option<String>,
    /// tagManagerUrl property.
    pub tag_manager_url: Option<String>,
    /// teardownTag property.
    pub teardown_tag: Option<Vec<TeardownTag>>,
    /// type property.
    pub r#type: Option<String>,
    /// workspaceId property.
    pub workspace_id: Option<String>,
}

/// `Zone` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct Zone {
    /// accountId property.
    pub account_id: Option<String>,
    /// boundary property.
    pub boundary: Option<ZoneBoundary>,
    /// childContainer property.
    pub child_container: Option<Vec<ZoneChildContainer>>,
    /// containerId property.
    pub container_id: Option<String>,
    /// fingerprint property.
    pub fingerprint: Option<String>,
    /// name property.
    pub name: Option<String>,
    /// notes property.
    pub notes: Option<String>,
    /// path property.
    pub path: Option<String>,
    /// tagManagerUrl property.
    pub tag_manager_url: Option<String>,
    /// typeRestriction property.
    pub type_restriction: Option<ZoneTypeRestriction>,
    /// workspaceId property.
    pub workspace_id: Option<String>,
    /// zoneId property.
    pub zone_id: Option<String>,
}

/// `ZoneTypeRestriction` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct ZoneTypeRestriction {
    /// enable property.
    pub enable: Option<bool>,
    /// whitelistedTypeId property.
    pub whitelisted_type_id: Option<Vec<String>>,
}

/// `Trigger` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct Trigger {
    /// accountId property.
    pub account_id: Option<String>,
    /// autoEventFilter property.
    pub auto_event_filter: Option<Vec<Condition>>,
    /// checkValidation property.
    pub check_validation: Option<Parameter>,
    /// containerId property.
    pub container_id: Option<String>,
    /// continuousTimeMinMilliseconds property.
    pub continuous_time_min_milliseconds: Option<Parameter>,
    /// customEventFilter property.
    pub custom_event_filter: Option<Vec<Condition>>,
    /// eventName property.
    pub event_name: Option<Parameter>,
    /// filter property.
    pub filter: Option<Vec<Condition>>,
    /// fingerprint property.
    pub fingerprint: Option<String>,
    /// horizontalScrollPercentageList property.
    pub horizontal_scroll_percentage_list: Option<Parameter>,
    /// interval property.
    pub interval: Option<Parameter>,
    /// intervalSeconds property.
    pub interval_seconds: Option<Parameter>,
    /// limit property.
    pub limit: Option<Parameter>,
    /// maxTimerLengthSeconds property.
    pub max_timer_length_seconds: Option<Parameter>,
    /// name property.
    pub name: Option<String>,
    /// notes property.
    pub notes: Option<String>,
    /// parameter property.
    pub parameter: Option<Vec<Parameter>>,
    /// parentFolderId property.
    pub parent_folder_id: Option<String>,
    /// path property.
    pub path: Option<String>,
    /// selector property.
    pub selector: Option<Parameter>,
    /// tagManagerUrl property.
    pub tag_manager_url: Option<String>,
    /// totalTimeMinMilliseconds property.
    pub total_time_min_milliseconds: Option<Parameter>,
    /// triggerId property.
    pub trigger_id: Option<String>,
    /// type property.
    pub r#type: Option<String>,
    /// uniqueTriggerId property.
    pub unique_trigger_id: Option<Parameter>,
    /// verticalScrollPercentageList property.
    pub vertical_scroll_percentage_list: Option<Parameter>,
    /// visibilitySelector property.
    pub visibility_selector: Option<Parameter>,
    /// visiblePercentageMax property.
    pub visible_percentage_max: Option<Parameter>,
    /// visiblePercentageMin property.
    pub visible_percentage_min: Option<Parameter>,
    /// waitForTags property.
    pub wait_for_tags: Option<Parameter>,
    /// waitForTagsTimeout property.
    pub wait_for_tags_timeout: Option<Parameter>,
    /// workspaceId property.
    pub workspace_id: Option<String>,
}

/// `Workspace` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct Workspace {
    /// accountId property.
    pub account_id: Option<String>,
    /// containerId property.
    pub container_id: Option<String>,
    /// description property.
    pub description: Option<String>,
    /// fingerprint property.
    pub fingerprint: Option<String>,
    /// name property.
    pub name: Option<String>,
    /// path property.
    pub path: Option<String>,
    /// tagManagerUrl property.
    pub tag_manager_url: Option<String>,
    /// workspaceId property.
    pub workspace_id: Option<String>,
}

/// `QuickPreviewResponse` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct QuickPreviewResponse {
    /// compilerError property.
    pub compiler_error: Option<bool>,
    /// containerVersion property.
    pub container_version: Option<ContainerVersion>,
    /// syncStatus property.
    pub sync_status: Option<SyncStatus>,
}

/// `VariableFormatValue` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct VariableFormatValue {
    /// caseConversionType property.
    pub case_conversion_type: Option<String>,
    /// convertFalseToValue property.
    pub convert_false_to_value: Option<Parameter>,
    /// convertNullToValue property.
    pub convert_null_to_value: Option<Parameter>,
    /// convertToBoolean property.
    pub convert_to_boolean: Option<bool>,
    /// convertToNumber property.
    pub convert_to_number: Option<String>,
    /// convertTrueToValue property.
    pub convert_true_to_value: Option<Parameter>,
    /// convertUndefinedToValue property.
    pub convert_undefined_to_value: Option<Parameter>,
}

/// `TagConsentSetting` type.
#[derive(Debug, Clone, Serialize, Deserialize, JsonHash)]
pub struct TagConsentSetting {
    /// consentStatus property.
    pub consent_status: Option<String>,
    /// consentType property.
    pub consent_type: Option<Parameter>,
}

// =============================================================================
// ARGS TYPES (per-endpoint)
// =============================================================================

/// Arguments for [`tagmanager.accounts.containers.workspaces.create_builder`].
#[derive(Debug, Clone, Serialize, JsonHash)]
pub struct TagmanagerAccountsContainersWorkspacesCreateArgs {
    /// Path parameter: `parent`.
    pub parent: String,
}

/// Arguments for [`tagmanager.accounts.containers.workspaces.create_version_builder`].
#[derive(Debug, Clone, Serialize, JsonHash)]
pub struct TagmanagerAccountsContainersWorkspacesCreateVersionArgs {
    /// Path parameter: `path`.
    pub path: String,
}

/// Arguments for [`tagmanager.accounts.containers.workspaces.delete_builder`].
#[derive(Debug, Clone, Serialize, JsonHash)]
pub struct TagmanagerAccountsContainersWorkspacesDeleteArgs {
    /// Path parameter: `path`.
    pub path: String,
}

/// Arguments for [`tagmanager.accounts.containers.workspaces.get_builder`].
#[derive(Debug, Clone, Serialize, JsonHash)]
pub struct TagmanagerAccountsContainersWorkspacesGetArgs {
    /// Path parameter: `path`.
    pub path: String,
}

/// Arguments for [`tagmanager.accounts.containers.workspaces.list_builder`].
#[derive(Debug, Clone, Serialize, JsonHash)]
pub struct TagmanagerAccountsContainersWorkspacesListArgs {
    /// Path parameter: `parent`.
    pub parent: String,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
}

/// Arguments for [`tagmanager.accounts.containers.workspaces.quick_preview_builder`].
#[derive(Debug, Clone, Serialize, JsonHash)]
pub struct TagmanagerAccountsContainersWorkspacesQuickPreviewArgs {
    /// Path parameter: `path`.
    pub path: String,
}

/// Arguments for [`tagmanager.accounts.containers.workspaces.resolve_conflict_builder`].
#[derive(Debug, Clone, Serialize, JsonHash)]
pub struct TagmanagerAccountsContainersWorkspacesResolveConflictArgs {
    /// Path parameter: `path`.
    pub path: String,
    /// Query parameter: `fingerprint`.
    pub fingerprint: Option<String>,
}

/// Arguments for [`tagmanager.accounts.containers.workspaces.sync_builder`].
#[derive(Debug, Clone, Serialize, JsonHash)]
pub struct TagmanagerAccountsContainersWorkspacesSyncArgs {
    /// Path parameter: `path`.
    pub path: String,
}

/// Arguments for [`tagmanager.accounts.containers.workspaces.update_builder`].
#[derive(Debug, Clone, Serialize, JsonHash)]
pub struct TagmanagerAccountsContainersWorkspacesUpdateArgs {
    /// Path parameter: `path`.
    pub path: String,
    /// Query parameter: `fingerprint`.
    pub fingerprint: Option<String>,
}

// =============================================================================
// CLIENT FUNCTIONS (per-endpoint)
// =============================================================================

// -----------------------------------------------------------------------------
// POST tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces
// -----------------------------------------------------------------------------

/// POST tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = tagmanager_accounts_containers_workspaces_create_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn tagmanager_accounts_containers_workspaces_create_request<R, F>(
    client: &SimpleHttpClient<R>,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Workspace>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://tagmanager.googleapis.com/tagmanager/v2/accounts/{{accountsId}}/containers/{{containersId}}/workspaces",
    );

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Workspace =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces/{workspacesId}:create_version
// -----------------------------------------------------------------------------

/// POST tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces/{workspacesId}:create_version.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = tagmanager_accounts_containers_workspaces_create_version_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn tagmanager_accounts_containers_workspaces_create_version_request<R, F>(
    client: &SimpleHttpClient<R>,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<CreateContainerVersionResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://tagmanager.googleapis.com/tagmanager/v2/accounts/{{accountsId}}/containers/{{containersId}}/workspaces/{{workspacesId}}:create_version",
    );

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: CreateContainerVersionResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// DELETE tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces/{workspacesId}
// -----------------------------------------------------------------------------

/// DELETE tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces/{workspacesId}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = tagmanager_accounts_containers_workspaces_delete_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn tagmanager_accounts_containers_workspaces_delete_request<R, F>(
    client: &SimpleHttpClient<R>,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<()>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://tagmanager.googleapis.com/tagmanager/v2/accounts/{{accountsId}}/containers/{{containersId}}/workspaces/{{workspacesId}}",
    );

    let mut builder = client
        .delete(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream: _,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: (),
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces/{workspacesId}
// -----------------------------------------------------------------------------

/// GET tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces/{workspacesId}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = tagmanager_accounts_containers_workspaces_get_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn tagmanager_accounts_containers_workspaces_get_request<R, F>(
    client: &SimpleHttpClient<R>,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Workspace>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://tagmanager.googleapis.com/tagmanager/v2/accounts/{{accountsId}}/containers/{{containersId}}/workspaces/{{workspacesId}}",
    );

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Workspace =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces
// -----------------------------------------------------------------------------

/// GET tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = tagmanager_accounts_containers_workspaces_list_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn tagmanager_accounts_containers_workspaces_list_request<R, F>(
    client: &SimpleHttpClient<R>,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<ListWorkspacesResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://tagmanager.googleapis.com/tagmanager/v2/accounts/{{accountsId}}/containers/{{containersId}}/workspaces",
    );

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: ListWorkspacesResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces/{workspacesId}:quick_preview
// -----------------------------------------------------------------------------

/// POST tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces/{workspacesId}:quick_preview.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = tagmanager_accounts_containers_workspaces_quick_preview_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn tagmanager_accounts_containers_workspaces_quick_preview_request<R, F>(
    client: &SimpleHttpClient<R>,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<QuickPreviewResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://tagmanager.googleapis.com/tagmanager/v2/accounts/{{accountsId}}/containers/{{containersId}}/workspaces/{{workspacesId}}:quick_preview",
    );

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: QuickPreviewResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces/{workspacesId}:resolve_conflict
// -----------------------------------------------------------------------------

/// POST tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces/{workspacesId}:resolve_conflict.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = tagmanager_accounts_containers_workspaces_resolve_conflict_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn tagmanager_accounts_containers_workspaces_resolve_conflict_request<R, F>(
    client: &SimpleHttpClient<R>,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<()>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://tagmanager.googleapis.com/tagmanager/v2/accounts/{{accountsId}}/containers/{{containersId}}/workspaces/{{workspacesId}}:resolve_conflict",
    );

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream: _,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: (),
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces/{workspacesId}:sync
// -----------------------------------------------------------------------------

/// POST tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces/{workspacesId}:sync.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = tagmanager_accounts_containers_workspaces_sync_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn tagmanager_accounts_containers_workspaces_sync_request<R, F>(
    client: &SimpleHttpClient<R>,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<SyncWorkspaceResponse>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://tagmanager.googleapis.com/tagmanager/v2/accounts/{{accountsId}}/containers/{{containersId}}/workspaces/{{workspacesId}}:sync",
    );

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: SyncWorkspaceResponse =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// PUT tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces/{workspacesId}
// -----------------------------------------------------------------------------

/// PUT tagmanager/v2/accounts/{accountsId}/containers/{containersId}/workspaces/{workspacesId}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = tagmanager_accounts_containers_workspaces_update_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn tagmanager_accounts_containers_workspaces_update_request<R, F>(
    client: &SimpleHttpClient<R>,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Workspace>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://tagmanager.googleapis.com/tagmanager/v2/accounts/{{accountsId}}/containers/{{containersId}}/workspaces/{{workspacesId}}",
    );

    let mut builder = client
        .put(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Workspace =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}
