...
/// let provider = DriveactivityProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct DriveactivityProvider<S, R>
where
...
impl<S, R> DriveactivityProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new DriveactivityProvider from ProviderClient, extracting the HTTP client.
    ///
