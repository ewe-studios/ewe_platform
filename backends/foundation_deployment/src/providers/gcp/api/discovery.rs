...
/// let provider = DiscoveryProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct DiscoveryProvider<S, R>
where
...
impl<S, R> DiscoveryProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new DiscoveryProvider from ProviderClient, extracting the HTTP client.
    ///
