...
/// let provider = RealtimebiddingProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct RealtimebiddingProvider<S, R>
where
...
impl<S, R> RealtimebiddingProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new RealtimebiddingProvider from ProviderClient, extracting the HTTP client.
    ///
