...
/// let provider = FirebasedatabaseProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct FirebasedatabaseProvider<S, R>
where
...
impl<S, R> FirebasedatabaseProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new FirebasedatabaseProvider from ProviderClient, extracting the HTTP client.
    ///
