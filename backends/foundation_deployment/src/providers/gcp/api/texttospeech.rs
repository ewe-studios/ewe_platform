...
/// let provider = TexttospeechProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct TexttospeechProvider<S, R>
where
...
impl<S, R> TexttospeechProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new TexttospeechProvider from ProviderClient, extracting the HTTP client.
    ///
