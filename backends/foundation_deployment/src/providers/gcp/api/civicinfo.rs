...
/// let provider = CivicinfoProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct CivicinfoProvider<S, R>
where
...
impl<S, R> CivicinfoProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new CivicinfoProvider from ProviderClient, extracting the HTTP client.
    ///
