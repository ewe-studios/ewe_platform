...
/// let provider = DatalabelingProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct DatalabelingProvider<S, R>
where
...
impl<S, R> DatalabelingProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new DatalabelingProvider from ProviderClient, extracting the HTTP client.
    ///
