...
/// let provider = ServiceconsumermanagementProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct ServiceconsumermanagementProvider<S, R>
where
...
impl<S, R> ServiceconsumermanagementProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new ServiceconsumermanagementProvider from ProviderClient, extracting the HTTP client.
    ///
