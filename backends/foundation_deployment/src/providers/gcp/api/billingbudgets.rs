...
/// let provider = BillingbudgetsProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct BillingbudgetsProvider<S, R>
where
...
impl<S, R> BillingbudgetsProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new BillingbudgetsProvider from ProviderClient, extracting the HTTP client.
    ///
