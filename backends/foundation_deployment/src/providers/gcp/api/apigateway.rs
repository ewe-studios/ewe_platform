...
/// let provider = ApigatewayProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct ApigatewayProvider<S, R>
where
...
impl<S, R> ApigatewayProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new ApigatewayProvider from ProviderClient, extracting the HTTP client.
    ///
