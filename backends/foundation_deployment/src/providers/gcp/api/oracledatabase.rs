...
/// let provider = OracledatabaseProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct OracledatabaseProvider<S, R>
where
...
impl<S, R> OracledatabaseProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new OracledatabaseProvider from ProviderClient, extracting the HTTP client.
    ///
