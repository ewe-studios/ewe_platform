...
/// let provider = ObservabilityProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct ObservabilityProvider<S, R>
where
...
impl<S, R> ObservabilityProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new ObservabilityProvider from ProviderClient, extracting the HTTP client.
    ///
