...
/// let provider = BlockchainnodeengineProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct BlockchainnodeengineProvider<S, R>
where
...
impl<S, R> BlockchainnodeengineProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new BlockchainnodeengineProvider from ProviderClient, extracting the HTTP client.
    ///
