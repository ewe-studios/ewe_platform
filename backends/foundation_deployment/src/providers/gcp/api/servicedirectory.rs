...
/// let provider = ServicedirectoryProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct ServicedirectoryProvider<S, R>
where
...
impl<S, R> ServicedirectoryProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new ServicedirectoryProvider from ProviderClient, extracting the HTTP client.
    ///
