...
/// let provider = HypercomputeclusterProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct HypercomputeclusterProvider<S, R>
where
...
impl<S, R> HypercomputeclusterProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new HypercomputeclusterProvider from ProviderClient, extracting the HTTP client.
    ///
