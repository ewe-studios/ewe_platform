...
/// let provider = FirebaseapphostingProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct FirebaseapphostingProvider<S, R>
where
...
impl<S, R> FirebaseapphostingProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new FirebaseapphostingProvider from ProviderClient, extracting the HTTP client.
    ///
