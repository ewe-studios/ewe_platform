...
/// let provider = DataportabilityProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct DataportabilityProvider<S, R>
where
...
impl<S, R> DataportabilityProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new DataportabilityProvider from ProviderClient, extracting the HTTP client.
    ///
