...
/// let provider = AnalyticshubProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct AnalyticshubProvider<S, R>
where
...
impl<S, R> AnalyticshubProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new AnalyticshubProvider from ProviderClient, extracting the HTTP client.
    ///
