...
/// let provider = ThreatintelligenceProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct ThreatintelligenceProvider<S, R>
where
...
impl<S, R> ThreatintelligenceProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new ThreatintelligenceProvider from ProviderClient, extracting the HTTP client.
    ///
