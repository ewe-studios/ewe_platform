...
/// let provider = NotebooksProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct NotebooksProvider<S, R>
where
...
impl<S, R> NotebooksProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new NotebooksProvider from ProviderClient, extracting the HTTP client.
    ///
