...
/// let provider = CloudlocationfinderProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct CloudlocationfinderProvider<S, R>
where
...
impl<S, R> CloudlocationfinderProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new CloudlocationfinderProvider from ProviderClient, extracting the HTTP client.
    ///
