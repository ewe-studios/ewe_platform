...
/// let provider = WebsecurityscannerProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct WebsecurityscannerProvider<S, R>
where
...
impl<S, R> WebsecurityscannerProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new WebsecurityscannerProvider from ProviderClient, extracting the HTTP client.
    ///
