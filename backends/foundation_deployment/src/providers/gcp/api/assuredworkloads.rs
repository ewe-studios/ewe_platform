...
/// let provider = AssuredworkloadsProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct AssuredworkloadsProvider<S, R>
where
...
impl<S, R> AssuredworkloadsProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new AssuredworkloadsProvider from ProviderClient, extracting the HTTP client.
    ///
