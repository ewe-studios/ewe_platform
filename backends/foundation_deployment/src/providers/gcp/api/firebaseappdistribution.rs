...
/// let provider = FirebaseappdistributionProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct FirebaseappdistributionProvider<S, R>
where
...
impl<S, R> FirebaseappdistributionProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new FirebaseappdistributionProvider from ProviderClient, extracting the HTTP client.
    ///
