...
/// let provider = KmsinventoryProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct KmsinventoryProvider<S, R>
where
...
impl<S, R> KmsinventoryProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new KmsinventoryProvider from ProviderClient, extracting the HTTP client.
    ///
