...
/// let provider = YoutubereportingProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct YoutubereportingProvider<S, R>
where
...
impl<S, R> YoutubereportingProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new YoutubereportingProvider from ProviderClient, extracting the HTTP client.
    ///
