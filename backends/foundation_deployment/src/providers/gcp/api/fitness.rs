...
/// let provider = FitnessProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct FitnessProvider<S, R>
where
    ...
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new FitnessProvider from ProviderClient, extracting the HTTP client.
    ///
