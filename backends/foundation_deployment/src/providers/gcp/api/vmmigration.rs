...
/// let provider = VmmigrationProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct VmmigrationProvider<S, R>
where
...
impl<S, R> VmmigrationProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new VmmigrationProvider from ProviderClient, extracting the HTTP client.
    ///
