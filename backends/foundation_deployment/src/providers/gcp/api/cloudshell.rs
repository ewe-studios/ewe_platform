...
/// let provider = CloudshellProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct CloudshellProvider<S, R>
where
...
impl<S, R> CloudshellProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new CloudshellProvider from ProviderClient, extracting the HTTP client.
    ///
