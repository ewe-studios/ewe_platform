...
/// let provider = FirebaserulesProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct FirebaserulesProvider<S, R>
where
...
impl<S, R> FirebaserulesProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new FirebaserulesProvider from ProviderClient, extracting the HTTP client.
    ///
