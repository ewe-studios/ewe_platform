...
/// let provider = CloudfunctionsProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct CloudfunctionsProvider<S, R>
where
...
impl<S, R> CloudfunctionsProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new CloudfunctionsProvider from ProviderClient, extracting the HTTP client.
    ///
