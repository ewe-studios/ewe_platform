...
/// let provider = GmailpostmastertoolsProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct GmailpostmastertoolsProvider<S, R>
where
...
impl<S, R> GmailpostmastertoolsProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new GmailpostmastertoolsProvider from ProviderClient, extracting the HTTP client.
    ///
