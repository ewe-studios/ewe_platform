...
/// let provider = TravelimpactmodelProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct TravelimpactmodelProvider<S, R>
where
...
impl<S, R> TravelimpactmodelProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new TravelimpactmodelProvider from ProviderClient, extracting the HTTP client.
    ///
