...
/// let provider = ChromepolicyProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct ChromepolicyProvider<S, R>
where
...
impl<S, R> ChromepolicyProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new ChromepolicyProvider from ProviderClient, extracting the HTTP client.
    ///
