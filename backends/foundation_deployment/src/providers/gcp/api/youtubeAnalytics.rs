...
/// let provider = YoutubeAnalyticsProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct YoutubeAnalyticsProvider<S, R>
where
...
impl<S, R> YoutubeAnalyticsProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new YoutubeAnalyticsProvider from ProviderClient, extracting the HTTP client.
    ///
