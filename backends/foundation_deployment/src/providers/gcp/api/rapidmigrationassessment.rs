...
/// let provider = RapidmigrationassessmentProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct RapidmigrationassessmentProvider<S, R>
where
...
impl<S, R> RapidmigrationassessmentProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new RapidmigrationassessmentProvider from ProviderClient, extracting the HTTP client.
    ///
