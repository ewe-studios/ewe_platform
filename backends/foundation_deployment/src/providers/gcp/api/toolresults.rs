...
/// let provider = ToolresultsProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct ToolresultsProvider<S, R>
where
...
impl<S, R> ToolresultsProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new ToolresultsProvider from ProviderClient, extracting the HTTP client.
    ///
