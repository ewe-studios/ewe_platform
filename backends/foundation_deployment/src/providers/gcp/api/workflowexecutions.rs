...
/// let provider = WorkflowexecutionsProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct WorkflowexecutionsProvider<S, R>
where
...
impl<S, R> WorkflowexecutionsProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new WorkflowexecutionsProvider from ProviderClient, extracting the HTTP client.
    ///
