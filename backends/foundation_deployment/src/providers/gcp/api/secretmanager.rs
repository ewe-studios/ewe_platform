...
/// let provider = SecretmanagerProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct SecretmanagerProvider<S, R>
where
...
impl<S, R> SecretmanagerProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new SecretmanagerProvider from ProviderClient, extracting the HTTP client.
    ///
