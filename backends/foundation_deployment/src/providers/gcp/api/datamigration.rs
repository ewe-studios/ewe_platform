...
/// let provider = DatamigrationProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct DatamigrationProvider<S, R>
where
...
impl<S, R> DatamigrationProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new DatamigrationProvider from ProviderClient, extracting the HTTP client.
    ///
