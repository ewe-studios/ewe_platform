...
/// let provider = TagmanagerProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct TagmanagerProvider<S, R>
where
...
impl<S, R> TagmanagerProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new TagmanagerProvider from ProviderClient, extracting the HTTP client.
    ///
