...
/// let provider = CloudsupportProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct CloudsupportProvider<S, R>
where
...
impl<S, R> CloudsupportProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new CloudsupportProvider from ProviderClient, extracting the HTTP client.
    ///
