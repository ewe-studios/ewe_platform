...
/// let provider = CustomsearchProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct CustomsearchProvider<S, R>
where
...
impl<S, R> CustomsearchProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new CustomsearchProvider from ProviderClient, extracting the HTTP client.
    ///
