...
/// let provider = ServiceusageProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct ServiceusageProvider<S, R>
where
...
impl<S, R> ServiceusageProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new ServiceusageProvider from ProviderClient, extracting the HTTP client.
    ///
