...
/// let provider = AppengineProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct AppengineProvider<S, R>
where
...
impl<S, R> AppengineProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new AppengineProvider from ProviderClient, extracting the HTTP client.
    ///
