...
/// let provider = AdvisorynotificationsProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct AdvisorynotificationsProvider<S, R>
where
...
impl<S, R> AdvisorynotificationsProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new AdvisorynotificationsProvider from ProviderClient, extracting the HTTP client.
    ///
