...
/// let provider = ParametermanagerProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct ParametermanagerProvider<S, R>
where
...
impl<S, R> ParametermanagerProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new ParametermanagerProvider from ProviderClient, extracting the HTTP client.
    ///
