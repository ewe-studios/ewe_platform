...
/// let provider = BigqueryreservationProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct BigqueryreservationProvider<S, R>
where
...
impl<S, R> BigqueryreservationProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new BigqueryreservationProvider from ProviderClient, extracting the HTTP client.
    ///
