...
/// let provider = SmartdevicemanagementProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct SmartdevicemanagementProvider<S, R>
where
...
impl<S, R> SmartdevicemanagementProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new SmartdevicemanagementProvider from ProviderClient, extracting the HTTP client.
    ///
