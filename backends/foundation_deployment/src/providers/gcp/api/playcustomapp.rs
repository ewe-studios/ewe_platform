...
/// let provider = PlaycustomappProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct PlaycustomappProvider<S, R>
where
...
impl<S, R> PlaycustomappProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new PlaycustomappProvider from ProviderClient, extracting the HTTP client.
    ///
