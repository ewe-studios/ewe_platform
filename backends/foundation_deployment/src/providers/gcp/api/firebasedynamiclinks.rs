...
/// let provider = FirebasedynamiclinksProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct FirebasedynamiclinksProvider<S, R>
where
...
impl<S, R> FirebasedynamiclinksProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new FirebasedynamiclinksProvider from ProviderClient, extracting the HTTP client.
    ///
