...
/// let provider = GkeonpremProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct GkeonpremProvider<S, R>
where
...
impl<S, R> GkeonpremProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new GkeonpremProvider from ProviderClient, extracting the HTTP client.
    ///
