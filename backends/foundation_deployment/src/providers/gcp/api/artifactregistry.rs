...
/// let provider = ArtifactregistryProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct ArtifactregistryProvider<S, R>
where
...
impl<S, R> ArtifactregistryProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new ArtifactregistryProvider from ProviderClient, extracting the HTTP client.
    ///
