...
/// let provider = FirebasedataconnectProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct FirebasedataconnectProvider<S, R>
where
...
impl<S, R> FirebasedataconnectProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new FirebasedataconnectProvider from ProviderClient, extracting the HTTP client.
    ///
