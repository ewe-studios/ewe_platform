...
/// let provider = PubsubliteProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct PubsubliteProvider<S, R>
where
...
impl<S, R> PubsubliteProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new PubsubliteProvider from ProviderClient, extracting the HTTP client.
    ///
