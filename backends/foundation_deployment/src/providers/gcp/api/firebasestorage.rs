...
/// let provider = FirebasestorageProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct FirebasestorageProvider<S, R>
where
...
impl<S, R> FirebasestorageProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new FirebasestorageProvider from ProviderClient, extracting the HTTP client.
    ///
