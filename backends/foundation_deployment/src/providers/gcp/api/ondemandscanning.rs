...
/// let provider = OndemandscanningProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct OndemandscanningProvider<S, R>
where
...
impl<S, R> OndemandscanningProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new OndemandscanningProvider from ProviderClient, extracting the HTTP client.
    ///
