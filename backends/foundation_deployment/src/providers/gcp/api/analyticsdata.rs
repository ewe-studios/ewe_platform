...
/// let provider = AnalyticsdataProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct AnalyticsdataProvider<S, R>
where
...
impl<S, R> AnalyticsdataProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new AnalyticsdataProvider from ProviderClient, extracting the HTTP client.
    ///
