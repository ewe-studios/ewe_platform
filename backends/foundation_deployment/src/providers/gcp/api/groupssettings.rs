//! GroupssettingsProvider - State-aware groupssettings API client.
//!
//! WHY: Users need state-aware API clients that automatically track
//!      resource changes in the state store.
//!
//! WHAT: Provider wrapping ProviderClient<S> with methods for
//!       groupssettings API endpoints that auto-store results.
//!
...
/// GroupssettingsProvider with automatic state tracking.
///
/// # Type Parameters
///
...
/// let provider = GroupssettingsProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct GroupssettingsProvider<S, R>
where
...
impl<S, R> GroupssettingsProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    /// Create new GroupssettingsProvider.
    ...
    /// Create new GroupssettingsProvider from ProviderClient, extracting the HTTP client.
    ///
    ...
    pub fn groups_settings_groups_patch(
        ...
        ProviderError<ApiError>,
    > {
        let builder = groups_settings_groups_patch_builder(
            &self.http_client,
            &args.groupUniqueId,
        )
        .map_err(ProviderError::Api)?;
    ...
    pub fn groups_settings_groups_update(
        ...
        ProviderError<ApiError>,
    > {
        let builder = groups_settings_groups_update_builder(
            &self.http_client,
            &args.groupUniqueId,
        )
        .map_err(ProviderError::Api)?;
