...
/// let provider = ManagedkafkaProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct ManagedkafkaProvider<S, R>
where
...
impl<S, R> ManagedkafkaProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new ManagedkafkaProvider from ProviderClient, extracting the HTTP client.
    ///
