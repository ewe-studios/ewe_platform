...
/// let provider = ContaineranalysisProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct ContaineranalysisProvider<S, R>
where
...
impl<S, R> ContaineranalysisProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new ContaineranalysisProvider from ProviderClient, extracting the HTTP client.
    ///
