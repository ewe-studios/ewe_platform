//! FirebasehostingProvider - State-aware firebasehosting API client.
//!
//! WHY: Users need state-aware API clients that automatically track
//!      resource changes in the state store.
//!
//! WHAT: Provider wrapping ProviderClient<S> with methods for
//!       firebasehosting API endpoints that auto-store results.
//!
...
/// FirebasehostingProvider with automatic state tracking.
///
/// # Type Parameters
///
...
/// let provider = FirebasehostingProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct FirebasehostingProvider<S, R>
where
...
impl<S, R> FirebasehostingProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    /// Create new FirebasehostingProvider.
    ...
    /// Create new FirebasehostingProvider from ProviderClient, extracting the HTTP client.
    ///
    ...
    > {
        let builder = firebasehosting_operations_cancel_builder(
            &self.http_client,
            &args.name,
        )
    ...
    pub fn firebasehosting_operations_delete(
        ...
        ProviderError<ApiError>,
    > {
        let builder = firebasehosting_operations_delete_builder(
            &self.http_client,
            &args.name,
        )
        .map_err(ProviderError::Api)?;
    ...
    /// Firebasehosting projects sites custom domains operations delete.
    ///
    /// Automatically stores the result in the state store on success.
    ///
    /// # Arguments
