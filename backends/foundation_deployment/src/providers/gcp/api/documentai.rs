...
/// let provider = DocumentaiProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct DocumentaiProvider<S, R>
where
...
impl<S, R> DocumentaiProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new DocumentaiProvider from ProviderClient, extracting the HTTP client.
    ///
