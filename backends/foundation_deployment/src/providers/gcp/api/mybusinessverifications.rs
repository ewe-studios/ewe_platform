...
/// let provider = MybusinessverificationsProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct MybusinessverificationsProvider<S, R>
where
...
impl<S, R> MybusinessverificationsProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new MybusinessverificationsProvider from ProviderClient, extracting the HTTP client.
    ///
