...
/// let provider = AuthorizedbuyersmarketplaceProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct AuthorizedbuyersmarketplaceProvider<S, R>
where
...
impl<S, R> AuthorizedbuyersmarketplaceProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new AuthorizedbuyersmarketplaceProvider from ProviderClient, extracting the HTTP client.
    ///
