...
/// let provider = ReaderrevenuesubscriptionlinkingProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct ReaderrevenuesubscriptionlinkingProvider<S, R>
where
...
impl<S, R> ReaderrevenuesubscriptionlinkingProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new ReaderrevenuesubscriptionlinkingProvider from ProviderClient, extracting the HTTP client.
    ///
