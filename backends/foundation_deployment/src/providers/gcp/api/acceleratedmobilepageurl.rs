...
/// let provider = AcceleratedmobilepageurlProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct AcceleratedmobilepageurlProvider<S, R>
where
...
impl<S, R> AcceleratedmobilepageurlProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new AcceleratedmobilepageurlProvider from ProviderClient, extracting the HTTP client.
    ///
