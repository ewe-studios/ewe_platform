...
/// let provider = BigquerydatatransferProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct BigquerydatatransferProvider<S, R>
where
...
impl<S, R> BigquerydatatransferProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new BigquerydatatransferProvider from ProviderClient, extracting the HTTP client.
    ///
