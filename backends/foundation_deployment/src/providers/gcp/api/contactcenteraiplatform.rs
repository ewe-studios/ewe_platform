...
/// let provider = ContactcenteraiplatformProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct ContactcenteraiplatformProvider<S, R>
where
...
impl<S, R> ContactcenteraiplatformProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new ContactcenteraiplatformProvider from ProviderClient, extracting the HTTP client.
    ///
