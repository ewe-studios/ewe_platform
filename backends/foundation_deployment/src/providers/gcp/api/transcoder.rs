...
/// let provider = TranscoderProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct TranscoderProvider<S, R>
where
...
impl<S, R> TranscoderProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new TranscoderProvider from ProviderClient, extracting the HTTP client.
    ///
