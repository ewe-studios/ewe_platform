...
/// let provider = CloudcommerceprocurementProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct CloudcommerceprocurementProvider<S, R>
where
...
impl<S, R> CloudcommerceprocurementProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new CloudcommerceprocurementProvider from ProviderClient, extracting the HTTP client.
    ///
