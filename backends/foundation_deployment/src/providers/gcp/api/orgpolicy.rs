...
/// let provider = OrgpolicyProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct OrgpolicyProvider<S, R>
where
...
impl<S, R> OrgpolicyProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new OrgpolicyProvider from ProviderClient, extracting the HTTP client.
    ///
