...
/// let provider = DatafusionProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct DatafusionProvider<S, R>
where
...
impl<S, R> DatafusionProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new DatafusionProvider from ProviderClient, extracting the HTTP client.
    ///
