...
/// let provider = GamesConfigurationProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct GamesConfigurationProvider<S, R>
where
...
impl<S, R> GamesConfigurationProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new GamesConfigurationProvider from ProviderClient, extracting the HTTP client.
    ///
