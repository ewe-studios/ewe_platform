...
/// let provider = BeyondcorpProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct BeyondcorpProvider<S, R>
where
...
impl<S, R> BeyondcorpProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new BeyondcorpProvider from ProviderClient, extracting the HTTP client.
    ///
