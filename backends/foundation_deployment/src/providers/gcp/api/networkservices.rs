...
/// let provider = NetworkservicesProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct NetworkservicesProvider<S, R>
where
...
impl<S, R> NetworkservicesProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new NetworkservicesProvider from ProviderClient, extracting the HTTP client.
    ///
