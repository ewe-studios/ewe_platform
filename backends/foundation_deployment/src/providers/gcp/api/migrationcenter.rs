...
/// let provider = MigrationcenterProvider::from_provider_client(client);
/// ```
#[derive(Clone)]
pub struct MigrationcenterProvider<S, R>
where
...
impl<S, R> MigrationcenterProvider<S, R>
where
    S: foundation_db::state::traits::StateStore + Send + Sync + 'static,
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + 'static,
{
    ...
    /// Create new MigrationcenterProvider from ProviderClient, extracting the HTTP client.
    ///
