//! Auto-generated API clients for Google Cloud Platform - alertcenter.
//!
//! Generated by `cargo run --bin ewe_platform gen_provider_clients`.
//! DO NOT EDIT MANUALLY.
//!
//! Feature flag: `gcp`
//!

#![cfg(feature = "gcp")]

//! Feature flag: `gcp/alertcenter`

#![cfg(feature = "gcp/alertcenter")]

use foundation_core::valtron::{execute, StreamIterator, StreamIteratorExt};
use foundation_core::wire::simple_http::client::{
    body_reader, ClientRequestBuilder, RequestIntro, SimpleHttpClient, SystemDnsResolver,
};
use crate::providers::gcp::clients::types::*;
use crate::providers::gcp::resources::*;

/// GET v1beta1/alerts:batchDelete
/// Performs batch delete operation on alerts.
///
/// Returns `ClientRequestBuilder` for customization.
/// Use `alertcenter_alerts_batch_delete_execute()` to send, or `alertcenter_alerts_batch_delete` for simplest API.

pub fn alertcenter_alerts_batch_delete_builder(
    client: &SimpleHttpClient,
    body: &BatchDeleteAlertsRequest
) -> Result<ClientRequestBuilder<SystemDnsResolver>, ApiError> {

    // Build URL
    let url = format!(
        "https://api.example.comv1beta1/alerts:batchDelete",
    );

    // Build request
    client.get(&url)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))

        .body_json(body)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
}

/// GET v1beta1/alerts:batchDelete
/// Performs batch delete operation on alerts.
///
/// Takes a `ClientRequestBuilder`, builds and executes the request.
/// For customization, use `alertcenter_alerts_batch_delete_builder()` then pass to this function.
///
/// # Arguments
///
/// * `builder` - ClientRequestBuilder from alertcenter_alerts_batch_delete_builder()
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_batch_delete_execute(
    builder: ClientRequestBuilder<SystemDnsResolver>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<BatchDeleteAlertsResponse>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    // Get SendRequestTask from builder
    let task = builder
        .build_send_request()
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
        .map_ready(|intro| match intro {
            RequestIntro::Success { stream, status } => {
                let headers = status.headers().clone();

                if !status.is_success() {
                    return Err(ApiError::HttpStatus {
                        code: status.as_u16(),
                        headers,
                    });
                }

                let body = body_reader::collect_string(stream);
                let parsed: BatchDeleteAlertsResponse = serde_json::from_str(&body)
                    .map_err(|e| ApiError::ParseFailed(e.to_string()))?;

                Ok(ApiResponse {
                    status: status.as_u16(),
                    headers,
                    body: parsed,
                })
            }
            RequestIntro::Failed(e) => Err(ApiError::RequestSendFailed(e.to_string())),
        })
        .map_pending(|_| ApiPending::Sending);

    // Convert TaskIterator to StreamIterator
    execute(task, None)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))
}

/// GET v1beta1/alerts:batchDelete
/// Performs batch delete operation on alerts.
///
/// Simplest API - builds and executes the request in one call.
/// For customization, use `alertcenter_alerts_batch_delete_builder()` + `alertcenter_alerts_batch_delete_execute()`.
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_batch_delete(
    client: &SimpleHttpClient,
    body: &BatchDeleteAlertsRequest,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<BatchDeleteAlertsResponse>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    let builder = alertcenter_alerts_batch_delete_builder(client, body)?;
    alertcenter_alerts_batch_delete_execute(builder)
}

/// GET v1beta1/alerts:batchUndelete
/// Performs batch undelete operation on alerts.
///
/// Returns `ClientRequestBuilder` for customization.
/// Use `alertcenter_alerts_batch_undelete_execute()` to send, or `alertcenter_alerts_batch_undelete` for simplest API.

pub fn alertcenter_alerts_batch_undelete_builder(
    client: &SimpleHttpClient,
    body: &BatchUndeleteAlertsRequest
) -> Result<ClientRequestBuilder<SystemDnsResolver>, ApiError> {

    // Build URL
    let url = format!(
        "https://api.example.comv1beta1/alerts:batchUndelete",
    );

    // Build request
    client.get(&url)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))

        .body_json(body)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
}

/// GET v1beta1/alerts:batchUndelete
/// Performs batch undelete operation on alerts.
///
/// Takes a `ClientRequestBuilder`, builds and executes the request.
/// For customization, use `alertcenter_alerts_batch_undelete_builder()` then pass to this function.
///
/// # Arguments
///
/// * `builder` - ClientRequestBuilder from alertcenter_alerts_batch_undelete_builder()
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_batch_undelete_execute(
    builder: ClientRequestBuilder<SystemDnsResolver>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<BatchUndeleteAlertsResponse>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    // Get SendRequestTask from builder
    let task = builder
        .build_send_request()
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
        .map_ready(|intro| match intro {
            RequestIntro::Success { stream, status } => {
                let headers = status.headers().clone();

                if !status.is_success() {
                    return Err(ApiError::HttpStatus {
                        code: status.as_u16(),
                        headers,
                    });
                }

                let body = body_reader::collect_string(stream);
                let parsed: BatchUndeleteAlertsResponse = serde_json::from_str(&body)
                    .map_err(|e| ApiError::ParseFailed(e.to_string()))?;

                Ok(ApiResponse {
                    status: status.as_u16(),
                    headers,
                    body: parsed,
                })
            }
            RequestIntro::Failed(e) => Err(ApiError::RequestSendFailed(e.to_string())),
        })
        .map_pending(|_| ApiPending::Sending);

    // Convert TaskIterator to StreamIterator
    execute(task, None)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))
}

/// GET v1beta1/alerts:batchUndelete
/// Performs batch undelete operation on alerts.
///
/// Simplest API - builds and executes the request in one call.
/// For customization, use `alertcenter_alerts_batch_undelete_builder()` + `alertcenter_alerts_batch_undelete_execute()`.
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_batch_undelete(
    client: &SimpleHttpClient,
    body: &BatchUndeleteAlertsRequest,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<BatchUndeleteAlertsResponse>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    let builder = alertcenter_alerts_batch_undelete_builder(client, body)?;
    alertcenter_alerts_batch_undelete_execute(builder)
}

/// GET v1beta1/alerts/{alertId}
/// Marks the specified alert for deletion. An alert that has been marked for deletion is removed from Alert Center after 30 days. Marking an alert for deletion has no effect on an alert which has already been marked for deletion. Attempting to mark a nonexistent alert for deletion results in a NOT_FOUND error.
///
/// Returns `ClientRequestBuilder` for customization.
/// Use `alertcenter_alerts_delete_execute()` to send, or `alertcenter_alerts_delete` for simplest API.

pub fn alertcenter_alerts_delete_builder(
    client: &SimpleHttpClient,
    alertId: &str,
    customerId: Option<&str>,
) -> Result<ClientRequestBuilder<SystemDnsResolver>, ApiError> {

    // Build URL
    let url = format!(
        "https://api.example.comv1beta1/alerts/{}",
        alertId,
    );

    // Build request
    client.get(&url)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))

        .query(
            "customerId",
            customerId.as_ref().map(|s| s.to_string()).unwrap_or_default().as_str(),
        )
}

/// GET v1beta1/alerts/{alertId}
/// Marks the specified alert for deletion. An alert that has been marked for deletion is removed from Alert Center after 30 days. Marking an alert for deletion has no effect on an alert which has already been marked for deletion. Attempting to mark a nonexistent alert for deletion results in a NOT_FOUND error.
///
/// Takes a `ClientRequestBuilder`, builds and executes the request.
/// For customization, use `alertcenter_alerts_delete_builder()` then pass to this function.
///
/// # Arguments
///
/// * `builder` - ClientRequestBuilder from alertcenter_alerts_delete_builder()
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_delete_execute(
    builder: ClientRequestBuilder<SystemDnsResolver>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<Empty>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    // Get SendRequestTask from builder
    let task = builder
        .build_send_request()
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
        .map_ready(|intro| match intro {
            RequestIntro::Success { stream, status } => {
                let headers = status.headers().clone();

                if !status.is_success() {
                    return Err(ApiError::HttpStatus {
                        code: status.as_u16(),
                        headers,
                    });
                }

                let body = body_reader::collect_string(stream);
                let parsed: Empty = serde_json::from_str(&body)
                    .map_err(|e| ApiError::ParseFailed(e.to_string()))?;

                Ok(ApiResponse {
                    status: status.as_u16(),
                    headers,
                    body: parsed,
                })
            }
            RequestIntro::Failed(e) => Err(ApiError::RequestSendFailed(e.to_string())),
        })
        .map_pending(|_| ApiPending::Sending);

    // Convert TaskIterator to StreamIterator
    execute(task, None)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))
}

/// GET v1beta1/alerts/{alertId}
/// Marks the specified alert for deletion. An alert that has been marked for deletion is removed from Alert Center after 30 days. Marking an alert for deletion has no effect on an alert which has already been marked for deletion. Attempting to mark a nonexistent alert for deletion results in a NOT_FOUND error.
///
/// Simplest API - builds and executes the request in one call.
/// For customization, use `alertcenter_alerts_delete_builder()` + `alertcenter_alerts_delete_execute()`.
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_delete(
    client: &SimpleHttpClient,
    alertId: &str,
    customerId: Option<&str>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<Empty>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    let builder = alertcenter_alerts_delete_builder(client, alertId, customerId)?;
    alertcenter_alerts_delete_execute(builder)
}

/// GET v1beta1/alerts/{alertId}
/// Gets the specified alert. Attempting to get a nonexistent alert returns NOT_FOUND error.
///
/// Returns `ClientRequestBuilder` for customization.
/// Use `alertcenter_alerts_get_execute()` to send, or `alertcenter_alerts_get` for simplest API.

pub fn alertcenter_alerts_get_builder(
    client: &SimpleHttpClient,
    alertId: &str,
    customerId: Option<&str>,
) -> Result<ClientRequestBuilder<SystemDnsResolver>, ApiError> {

    // Build URL
    let url = format!(
        "https://api.example.comv1beta1/alerts/{}",
        alertId,
    );

    // Build request
    client.get(&url)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))

        .query(
            "customerId",
            customerId.as_ref().map(|s| s.to_string()).unwrap_or_default().as_str(),
        )
}

/// GET v1beta1/alerts/{alertId}
/// Gets the specified alert. Attempting to get a nonexistent alert returns NOT_FOUND error.
///
/// Takes a `ClientRequestBuilder`, builds and executes the request.
/// For customization, use `alertcenter_alerts_get_builder()` then pass to this function.
///
/// # Arguments
///
/// * `builder` - ClientRequestBuilder from alertcenter_alerts_get_builder()
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_get_execute(
    builder: ClientRequestBuilder<SystemDnsResolver>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<Alert>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    // Get SendRequestTask from builder
    let task = builder
        .build_send_request()
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
        .map_ready(|intro| match intro {
            RequestIntro::Success { stream, status } => {
                let headers = status.headers().clone();

                if !status.is_success() {
                    return Err(ApiError::HttpStatus {
                        code: status.as_u16(),
                        headers,
                    });
                }

                let body = body_reader::collect_string(stream);
                let parsed: Alert = serde_json::from_str(&body)
                    .map_err(|e| ApiError::ParseFailed(e.to_string()))?;

                Ok(ApiResponse {
                    status: status.as_u16(),
                    headers,
                    body: parsed,
                })
            }
            RequestIntro::Failed(e) => Err(ApiError::RequestSendFailed(e.to_string())),
        })
        .map_pending(|_| ApiPending::Sending);

    // Convert TaskIterator to StreamIterator
    execute(task, None)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))
}

/// GET v1beta1/alerts/{alertId}
/// Gets the specified alert. Attempting to get a nonexistent alert returns NOT_FOUND error.
///
/// Simplest API - builds and executes the request in one call.
/// For customization, use `alertcenter_alerts_get_builder()` + `alertcenter_alerts_get_execute()`.
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_get(
    client: &SimpleHttpClient,
    alertId: &str,
    customerId: Option<&str>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<Alert>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    let builder = alertcenter_alerts_get_builder(client, alertId, customerId)?;
    alertcenter_alerts_get_execute(builder)
}

/// GET v1beta1/alerts/{alertId}/metadata
/// Returns the metadata of an alert. Attempting to get metadata for a non-existent alert returns NOT_FOUND error.
///
/// Returns `ClientRequestBuilder` for customization.
/// Use `alertcenter_alerts_get_metadata_execute()` to send, or `alertcenter_alerts_get_metadata` for simplest API.

pub fn alertcenter_alerts_get_metadata_builder(
    client: &SimpleHttpClient,
    alertId: &str,
    customerId: Option<&str>,
) -> Result<ClientRequestBuilder<SystemDnsResolver>, ApiError> {

    // Build URL
    let url = format!(
        "https://api.example.comv1beta1/alerts/{}/metadata",
        alertId,
    );

    // Build request
    client.get(&url)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))

        .query(
            "customerId",
            customerId.as_ref().map(|s| s.to_string()).unwrap_or_default().as_str(),
        )
}

/// GET v1beta1/alerts/{alertId}/metadata
/// Returns the metadata of an alert. Attempting to get metadata for a non-existent alert returns NOT_FOUND error.
///
/// Takes a `ClientRequestBuilder`, builds and executes the request.
/// For customization, use `alertcenter_alerts_get_metadata_builder()` then pass to this function.
///
/// # Arguments
///
/// * `builder` - ClientRequestBuilder from alertcenter_alerts_get_metadata_builder()
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_get_metadata_execute(
    builder: ClientRequestBuilder<SystemDnsResolver>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<AlertMetadata>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    // Get SendRequestTask from builder
    let task = builder
        .build_send_request()
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
        .map_ready(|intro| match intro {
            RequestIntro::Success { stream, status } => {
                let headers = status.headers().clone();

                if !status.is_success() {
                    return Err(ApiError::HttpStatus {
                        code: status.as_u16(),
                        headers,
                    });
                }

                let body = body_reader::collect_string(stream);
                let parsed: AlertMetadata = serde_json::from_str(&body)
                    .map_err(|e| ApiError::ParseFailed(e.to_string()))?;

                Ok(ApiResponse {
                    status: status.as_u16(),
                    headers,
                    body: parsed,
                })
            }
            RequestIntro::Failed(e) => Err(ApiError::RequestSendFailed(e.to_string())),
        })
        .map_pending(|_| ApiPending::Sending);

    // Convert TaskIterator to StreamIterator
    execute(task, None)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))
}

/// GET v1beta1/alerts/{alertId}/metadata
/// Returns the metadata of an alert. Attempting to get metadata for a non-existent alert returns NOT_FOUND error.
///
/// Simplest API - builds and executes the request in one call.
/// For customization, use `alertcenter_alerts_get_metadata_builder()` + `alertcenter_alerts_get_metadata_execute()`.
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_get_metadata(
    client: &SimpleHttpClient,
    alertId: &str,
    customerId: Option<&str>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<AlertMetadata>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    let builder = alertcenter_alerts_get_metadata_builder(client, alertId, customerId)?;
    alertcenter_alerts_get_metadata_execute(builder)
}

/// GET v1beta1/alerts
/// Lists the alerts.
///
/// Returns `ClientRequestBuilder` for customization.
/// Use `alertcenter_alerts_list_execute()` to send, or `alertcenter_alerts_list` for simplest API.

pub fn alertcenter_alerts_list_builder(
    client: &SimpleHttpClient,
    customerId: Option<&str>,
    filter: Option<&str>,
    orderBy: Option<&str>,
    pageSize: Option<i32>,
    pageToken: Option<&str>,
) -> Result<ClientRequestBuilder<SystemDnsResolver>, ApiError> {

    // Build URL
    let url = format!(
        "https://api.example.comv1beta1/alerts",
    );

    // Build request
    client.get(&url)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))

        .query(
            "customerId",
            customerId.as_ref().map(|s| s.to_string()).unwrap_or_default().as_str(),
        )

        .query(
            "filter",
            filter.as_ref().map(|s| s.to_string()).unwrap_or_default().as_str(),
        )

        .query(
            "orderBy",
            orderBy.as_ref().map(|s| s.to_string()).unwrap_or_default().as_str(),
        )

        .query(
            "pageSize",
            pageSize.as_ref().map(|s| s.to_string()).unwrap_or_default().as_str(),
        )

        .query(
            "pageToken",
            pageToken.as_ref().map(|s| s.to_string()).unwrap_or_default().as_str(),
        )
}

/// GET v1beta1/alerts
/// Lists the alerts.
///
/// Takes a `ClientRequestBuilder`, builds and executes the request.
/// For customization, use `alertcenter_alerts_list_builder()` then pass to this function.
///
/// # Arguments
///
/// * `builder` - ClientRequestBuilder from alertcenter_alerts_list_builder()
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_list_execute(
    builder: ClientRequestBuilder<SystemDnsResolver>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<ListAlertsResponse>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    // Get SendRequestTask from builder
    let task = builder
        .build_send_request()
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
        .map_ready(|intro| match intro {
            RequestIntro::Success { stream, status } => {
                let headers = status.headers().clone();

                if !status.is_success() {
                    return Err(ApiError::HttpStatus {
                        code: status.as_u16(),
                        headers,
                    });
                }

                let body = body_reader::collect_string(stream);
                let parsed: ListAlertsResponse = serde_json::from_str(&body)
                    .map_err(|e| ApiError::ParseFailed(e.to_string()))?;

                Ok(ApiResponse {
                    status: status.as_u16(),
                    headers,
                    body: parsed,
                })
            }
            RequestIntro::Failed(e) => Err(ApiError::RequestSendFailed(e.to_string())),
        })
        .map_pending(|_| ApiPending::Sending);

    // Convert TaskIterator to StreamIterator
    execute(task, None)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))
}

/// GET v1beta1/alerts
/// Lists the alerts.
///
/// Simplest API - builds and executes the request in one call.
/// For customization, use `alertcenter_alerts_list_builder()` + `alertcenter_alerts_list_execute()`.
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_list(
    client: &SimpleHttpClient,
    customerId: Option<&str>,
    filter: Option<&str>,
    orderBy: Option<&str>,
    pageSize: Option<i32>,
    pageToken: Option<&str>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<ListAlertsResponse>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    let builder = alertcenter_alerts_list_builder(client, customerId, filter, orderBy, pageSize, pageToken)?;
    alertcenter_alerts_list_execute(builder)
}

/// GET v1beta1/alerts/{alertId}:undelete
/// Restores, or "undeletes", an alert that was marked for deletion within the past 30 days. Attempting to undelete an alert which was marked for deletion over 30 days ago (which has been removed from the Alert Center database) or a nonexistent alert returns a NOT_FOUND error. Attempting to undelete an alert which has not been marked for deletion has no effect.
///
/// Returns `ClientRequestBuilder` for customization.
/// Use `alertcenter_alerts_undelete_execute()` to send, or `alertcenter_alerts_undelete` for simplest API.

pub fn alertcenter_alerts_undelete_builder(
    client: &SimpleHttpClient,
    alertId: &str,
    body: &UndeleteAlertRequest
) -> Result<ClientRequestBuilder<SystemDnsResolver>, ApiError> {

    // Build URL
    let url = format!(
        "https://api.example.comv1beta1/alerts/{}:undelete",
        alertId,
    );

    // Build request
    client.get(&url)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))

        .body_json(body)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
}

/// GET v1beta1/alerts/{alertId}:undelete
/// Restores, or "undeletes", an alert that was marked for deletion within the past 30 days. Attempting to undelete an alert which was marked for deletion over 30 days ago (which has been removed from the Alert Center database) or a nonexistent alert returns a NOT_FOUND error. Attempting to undelete an alert which has not been marked for deletion has no effect.
///
/// Takes a `ClientRequestBuilder`, builds and executes the request.
/// For customization, use `alertcenter_alerts_undelete_builder()` then pass to this function.
///
/// # Arguments
///
/// * `builder` - ClientRequestBuilder from alertcenter_alerts_undelete_builder()
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_undelete_execute(
    builder: ClientRequestBuilder<SystemDnsResolver>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<Alert>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    // Get SendRequestTask from builder
    let task = builder
        .build_send_request()
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
        .map_ready(|intro| match intro {
            RequestIntro::Success { stream, status } => {
                let headers = status.headers().clone();

                if !status.is_success() {
                    return Err(ApiError::HttpStatus {
                        code: status.as_u16(),
                        headers,
                    });
                }

                let body = body_reader::collect_string(stream);
                let parsed: Alert = serde_json::from_str(&body)
                    .map_err(|e| ApiError::ParseFailed(e.to_string()))?;

                Ok(ApiResponse {
                    status: status.as_u16(),
                    headers,
                    body: parsed,
                })
            }
            RequestIntro::Failed(e) => Err(ApiError::RequestSendFailed(e.to_string())),
        })
        .map_pending(|_| ApiPending::Sending);

    // Convert TaskIterator to StreamIterator
    execute(task, None)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))
}

/// GET v1beta1/alerts/{alertId}:undelete
/// Restores, or "undeletes", an alert that was marked for deletion within the past 30 days. Attempting to undelete an alert which was marked for deletion over 30 days ago (which has been removed from the Alert Center database) or a nonexistent alert returns a NOT_FOUND error. Attempting to undelete an alert which has not been marked for deletion has no effect.
///
/// Simplest API - builds and executes the request in one call.
/// For customization, use `alertcenter_alerts_undelete_builder()` + `alertcenter_alerts_undelete_execute()`.
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_undelete(
    client: &SimpleHttpClient,
    alertId: &str,
    body: &UndeleteAlertRequest,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<Alert>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    let builder = alertcenter_alerts_undelete_builder(client, alertId, body)?;
    alertcenter_alerts_undelete_execute(builder)
}

/// GET v1beta1/alerts/{alertId}/feedback
/// Creates new feedback for an alert. Attempting to create a feedback for a non-existent alert returns NOT_FOUND error. Attempting to create a feedback for an alert that is marked for deletion returns FAILED_PRECONDITION' error.
///
/// Returns `ClientRequestBuilder` for customization.
/// Use `alertcenter_alerts_feedback_create_execute()` to send, or `alertcenter_alerts_feedback_create` for simplest API.

pub fn alertcenter_alerts_feedback_create_builder(
    client: &SimpleHttpClient,
    alertId: &str,
    customerId: Option<&str>,
    body: &AlertFeedback
) -> Result<ClientRequestBuilder<SystemDnsResolver>, ApiError> {

    // Build URL
    let url = format!(
        "https://api.example.comv1beta1/alerts/{}/feedback",
        alertId,
    );

    // Build request
    client.get(&url)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))

        .query(
            "customerId",
            customerId.as_ref().map(|s| s.to_string()).unwrap_or_default().as_str(),
        )

        .body_json(body)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
}

/// GET v1beta1/alerts/{alertId}/feedback
/// Creates new feedback for an alert. Attempting to create a feedback for a non-existent alert returns NOT_FOUND error. Attempting to create a feedback for an alert that is marked for deletion returns FAILED_PRECONDITION' error.
///
/// Takes a `ClientRequestBuilder`, builds and executes the request.
/// For customization, use `alertcenter_alerts_feedback_create_builder()` then pass to this function.
///
/// # Arguments
///
/// * `builder` - ClientRequestBuilder from alertcenter_alerts_feedback_create_builder()
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_feedback_create_execute(
    builder: ClientRequestBuilder<SystemDnsResolver>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<AlertFeedback>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    // Get SendRequestTask from builder
    let task = builder
        .build_send_request()
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
        .map_ready(|intro| match intro {
            RequestIntro::Success { stream, status } => {
                let headers = status.headers().clone();

                if !status.is_success() {
                    return Err(ApiError::HttpStatus {
                        code: status.as_u16(),
                        headers,
                    });
                }

                let body = body_reader::collect_string(stream);
                let parsed: AlertFeedback = serde_json::from_str(&body)
                    .map_err(|e| ApiError::ParseFailed(e.to_string()))?;

                Ok(ApiResponse {
                    status: status.as_u16(),
                    headers,
                    body: parsed,
                })
            }
            RequestIntro::Failed(e) => Err(ApiError::RequestSendFailed(e.to_string())),
        })
        .map_pending(|_| ApiPending::Sending);

    // Convert TaskIterator to StreamIterator
    execute(task, None)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))
}

/// GET v1beta1/alerts/{alertId}/feedback
/// Creates new feedback for an alert. Attempting to create a feedback for a non-existent alert returns NOT_FOUND error. Attempting to create a feedback for an alert that is marked for deletion returns FAILED_PRECONDITION' error.
///
/// Simplest API - builds and executes the request in one call.
/// For customization, use `alertcenter_alerts_feedback_create_builder()` + `alertcenter_alerts_feedback_create_execute()`.
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_feedback_create(
    client: &SimpleHttpClient,
    alertId: &str,
    customerId: Option<&str>,
    body: &AlertFeedback,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<AlertFeedback>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    let builder = alertcenter_alerts_feedback_create_builder(client, alertId, customerId, body)?;
    alertcenter_alerts_feedback_create_execute(builder)
}

/// GET v1beta1/alerts/{alertId}/feedback
/// Lists all the feedback for an alert. Attempting to list feedbacks for a non-existent alert returns NOT_FOUND error.
///
/// Returns `ClientRequestBuilder` for customization.
/// Use `alertcenter_alerts_feedback_list_execute()` to send, or `alertcenter_alerts_feedback_list` for simplest API.

pub fn alertcenter_alerts_feedback_list_builder(
    client: &SimpleHttpClient,
    alertId: &str,
    customerId: Option<&str>,
    filter: Option<&str>,
) -> Result<ClientRequestBuilder<SystemDnsResolver>, ApiError> {

    // Build URL
    let url = format!(
        "https://api.example.comv1beta1/alerts/{}/feedback",
        alertId,
    );

    // Build request
    client.get(&url)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))

        .query(
            "customerId",
            customerId.as_ref().map(|s| s.to_string()).unwrap_or_default().as_str(),
        )

        .query(
            "filter",
            filter.as_ref().map(|s| s.to_string()).unwrap_or_default().as_str(),
        )
}

/// GET v1beta1/alerts/{alertId}/feedback
/// Lists all the feedback for an alert. Attempting to list feedbacks for a non-existent alert returns NOT_FOUND error.
///
/// Takes a `ClientRequestBuilder`, builds and executes the request.
/// For customization, use `alertcenter_alerts_feedback_list_builder()` then pass to this function.
///
/// # Arguments
///
/// * `builder` - ClientRequestBuilder from alertcenter_alerts_feedback_list_builder()
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_feedback_list_execute(
    builder: ClientRequestBuilder<SystemDnsResolver>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<ListAlertFeedbackResponse>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    // Get SendRequestTask from builder
    let task = builder
        .build_send_request()
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
        .map_ready(|intro| match intro {
            RequestIntro::Success { stream, status } => {
                let headers = status.headers().clone();

                if !status.is_success() {
                    return Err(ApiError::HttpStatus {
                        code: status.as_u16(),
                        headers,
                    });
                }

                let body = body_reader::collect_string(stream);
                let parsed: ListAlertFeedbackResponse = serde_json::from_str(&body)
                    .map_err(|e| ApiError::ParseFailed(e.to_string()))?;

                Ok(ApiResponse {
                    status: status.as_u16(),
                    headers,
                    body: parsed,
                })
            }
            RequestIntro::Failed(e) => Err(ApiError::RequestSendFailed(e.to_string())),
        })
        .map_pending(|_| ApiPending::Sending);

    // Convert TaskIterator to StreamIterator
    execute(task, None)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))
}

/// GET v1beta1/alerts/{alertId}/feedback
/// Lists all the feedback for an alert. Attempting to list feedbacks for a non-existent alert returns NOT_FOUND error.
///
/// Simplest API - builds and executes the request in one call.
/// For customization, use `alertcenter_alerts_feedback_list_builder()` + `alertcenter_alerts_feedback_list_execute()`.
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_alerts_feedback_list(
    client: &SimpleHttpClient,
    alertId: &str,
    customerId: Option<&str>,
    filter: Option<&str>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<ListAlertFeedbackResponse>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    let builder = alertcenter_alerts_feedback_list_builder(client, alertId, customerId, filter)?;
    alertcenter_alerts_feedback_list_execute(builder)
}

/// GET v1beta1/settings
/// Returns customer-level settings.
///
/// Returns `ClientRequestBuilder` for customization.
/// Use `alertcenter_get_settings_execute()` to send, or `alertcenter_get_settings` for simplest API.

pub fn alertcenter_get_settings_builder(
    client: &SimpleHttpClient,
    customerId: Option<&str>,
) -> Result<ClientRequestBuilder<SystemDnsResolver>, ApiError> {

    // Build URL
    let url = format!(
        "https://api.example.comv1beta1/settings",
    );

    // Build request
    client.get(&url)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))

        .query(
            "customerId",
            customerId.as_ref().map(|s| s.to_string()).unwrap_or_default().as_str(),
        )
}

/// GET v1beta1/settings
/// Returns customer-level settings.
///
/// Takes a `ClientRequestBuilder`, builds and executes the request.
/// For customization, use `alertcenter_get_settings_builder()` then pass to this function.
///
/// # Arguments
///
/// * `builder` - ClientRequestBuilder from alertcenter_get_settings_builder()
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_get_settings_execute(
    builder: ClientRequestBuilder<SystemDnsResolver>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<Settings>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    // Get SendRequestTask from builder
    let task = builder
        .build_send_request()
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
        .map_ready(|intro| match intro {
            RequestIntro::Success { stream, status } => {
                let headers = status.headers().clone();

                if !status.is_success() {
                    return Err(ApiError::HttpStatus {
                        code: status.as_u16(),
                        headers,
                    });
                }

                let body = body_reader::collect_string(stream);
                let parsed: Settings = serde_json::from_str(&body)
                    .map_err(|e| ApiError::ParseFailed(e.to_string()))?;

                Ok(ApiResponse {
                    status: status.as_u16(),
                    headers,
                    body: parsed,
                })
            }
            RequestIntro::Failed(e) => Err(ApiError::RequestSendFailed(e.to_string())),
        })
        .map_pending(|_| ApiPending::Sending);

    // Convert TaskIterator to StreamIterator
    execute(task, None)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))
}

/// GET v1beta1/settings
/// Returns customer-level settings.
///
/// Simplest API - builds and executes the request in one call.
/// For customization, use `alertcenter_get_settings_builder()` + `alertcenter_get_settings_execute()`.
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_get_settings(
    client: &SimpleHttpClient,
    customerId: Option<&str>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<Settings>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    let builder = alertcenter_get_settings_builder(client, customerId)?;
    alertcenter_get_settings_execute(builder)
}

/// GET v1beta1/settings
/// Updates the customer-level settings.
///
/// Returns `ClientRequestBuilder` for customization.
/// Use `alertcenter_update_settings_execute()` to send, or `alertcenter_update_settings` for simplest API.

pub fn alertcenter_update_settings_builder(
    client: &SimpleHttpClient,
    customerId: Option<&str>,
    body: &Settings
) -> Result<ClientRequestBuilder<SystemDnsResolver>, ApiError> {

    // Build URL
    let url = format!(
        "https://api.example.comv1beta1/settings",
    );

    // Build request
    client.get(&url)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))

        .query(
            "customerId",
            customerId.as_ref().map(|s| s.to_string()).unwrap_or_default().as_str(),
        )

        .body_json(body)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
}

/// GET v1beta1/settings
/// Updates the customer-level settings.
///
/// Takes a `ClientRequestBuilder`, builds and executes the request.
/// For customization, use `alertcenter_update_settings_builder()` then pass to this function.
///
/// # Arguments
///
/// * `builder` - ClientRequestBuilder from alertcenter_update_settings_builder()
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_update_settings_execute(
    builder: ClientRequestBuilder<SystemDnsResolver>,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<Settings>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    // Get SendRequestTask from builder
    let task = builder
        .build_send_request()
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))?
        .map_ready(|intro| match intro {
            RequestIntro::Success { stream, status } => {
                let headers = status.headers().clone();

                if !status.is_success() {
                    return Err(ApiError::HttpStatus {
                        code: status.as_u16(),
                        headers,
                    });
                }

                let body = body_reader::collect_string(stream);
                let parsed: Settings = serde_json::from_str(&body)
                    .map_err(|e| ApiError::ParseFailed(e.to_string()))?;

                Ok(ApiResponse {
                    status: status.as_u16(),
                    headers,
                    body: parsed,
                })
            }
            RequestIntro::Failed(e) => Err(ApiError::RequestSendFailed(e.to_string())),
        })
        .map_pending(|_| ApiPending::Sending);

    // Convert TaskIterator to StreamIterator
    execute(task, None)
        .map_err(|e| ApiError::RequestBuildFailed(e.to_string()))
}

/// GET v1beta1/settings
/// Updates the customer-level settings.
///
/// Simplest API - builds and executes the request in one call.
/// For customization, use `alertcenter_update_settings_builder()` + `alertcenter_update_settings_execute()`.
///
/// # Errors
///
/// Returns an error if the request cannot be built.

pub fn alertcenter_update_settings(
    client: &SimpleHttpClient,
    customerId: Option<&str>,
    body: &Settings,
) -> Result<
    impl StreamIterator<
        D = Result<ApiResponse<Settings>, ApiError>,
        P = ApiPending
    > + Send + 'static,
    ApiError,
> {

    let builder = alertcenter_update_settings_builder(client, customerId, body)?;
    alertcenter_update_settings_execute(builder)
}

