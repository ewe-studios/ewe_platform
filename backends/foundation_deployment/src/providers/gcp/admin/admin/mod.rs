//! Auto-generated API module for gcp/admin admin.
//!
//! Generated by `cargo run --bin ewe_platform gen_api`.
//! DO NOT EDIT MANUALLY.
//!
//! Feature flag: `gcp_admin_admin `

#![cfg(feature = "gcp_admin_admin")]
#![allow(clippy::too_many_arguments, clippy::type_complexity)]
#![allow(
    clippy::missing_errors_doc,
    clippy::doc_markdown,
    clippy::useless_format
)]
#![allow(unused_imports)]

use foundation_core::valtron::{TaskIterator, TaskIteratorExt};
use foundation_core::wire::simple_http::client::{ClientRequestBuilder, SimpleHttpClient};
use foundation_macros::JsonHash;
use serde::{Deserialize, Serialize};

use super::shared::ApiResponse;

// =============================================================================
// TYPE DECLARATIONS
// =============================================================================

/// `UsageReports` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct UsageReports {
    /// etag property.
    pub etag: Option<String>,
    /// kind property.
    pub kind: Option<String>,
    /// nextPageToken property.
    pub next_page_token: Option<String>,
    /// usageReports property.
    pub usage_reports: Option<Vec<UsageReport>>,
    /// warnings property.
    pub warnings: Option<Vec<std::collections::HashMap<String, serde_json::Value>>>,
}

/// `GroupIdentity` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct GroupIdentity {
    /// groupEmail property.
    pub group_email: Option<String>,
    /// id property.
    pub id: Option<String>,
}

/// `UsageReport` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct UsageReport {
    /// date property.
    pub date: Option<String>,
    /// entity property.
    pub entity: Option<std::collections::HashMap<String, serde_json::Value>>,
    /// etag property.
    pub etag: Option<String>,
    /// kind property.
    pub kind: Option<String>,
    /// parameters property.
    pub parameters: Option<Vec<std::collections::HashMap<String, serde_json::Value>>>,
}

/// `NestedParameter` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct NestedParameter {
    /// boolValue property.
    pub bool_value: Option<bool>,
    /// intValue property.
    pub int_value: Option<String>,
    /// multiBoolValue property.
    pub multi_bool_value: Option<Vec<bool>>,
    /// multiIntValue property.
    pub multi_int_value: Option<Vec<String>>,
    /// multiValue property.
    pub multi_value: Option<Vec<String>>,
    /// name property.
    pub name: Option<String>,
    /// value property.
    pub value: Option<String>,
}

/// `ResourceDetails` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ResourceDetails {
    /// appliedLabels property.
    pub applied_labels: Option<Vec<AppliedLabel>>,
    /// id property.
    pub id: Option<String>,
    /// ownerDetails property.
    pub owner_details: Option<OwnerDetails>,
    /// relation property.
    pub relation: Option<String>,
    /// title property.
    pub title: Option<String>,
    /// type property.
    pub r#type: Option<String>,
}

/// `AppliedLabel` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct AppliedLabel {
    /// fieldValues property.
    pub field_values: Option<Vec<FieldValue>>,
    /// id property.
    pub id: Option<String>,
    /// reason property.
    pub reason: Option<Reason>,
    /// title property.
    pub title: Option<String>,
}

/// `FieldValueUserListValue` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct FieldValueUserListValue {
    /// values property.
    pub values: Option<Vec<FieldValueUserValue>>,
}

/// `OwnerIdentity` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct OwnerIdentity {
    /// customerIdentity property.
    pub customer_identity: Option<CustomerIdentity>,
    /// groupIdentity property.
    pub group_identity: Option<GroupIdentity>,
    /// userIdentity property.
    pub user_identity: Option<UserIdentity>,
}

/// `Channel` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Channel {
    /// address property.
    pub address: Option<String>,
    /// expiration property.
    pub expiration: Option<String>,
    /// id property.
    pub id: Option<String>,
    /// kind property.
    pub kind: Option<String>,
    /// params property.
    pub params: Option<serde_json::Value>,
    /// payload property.
    pub payload: Option<bool>,
    /// resourceId property.
    pub resource_id: Option<String>,
    /// resourceUri property.
    pub resource_uri: Option<String>,
    /// token property.
    pub token: Option<String>,
    /// type property.
    pub r#type: Option<String>,
}

/// `FieldValue` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct FieldValue {
    /// dateValue property.
    pub date_value: Option<Date>,
    /// displayName property.
    pub display_name: Option<String>,
    /// id property.
    pub id: Option<String>,
    /// integerValue property.
    pub integer_value: Option<String>,
    /// longTextValue property.
    pub long_text_value: Option<String>,
    /// reason property.
    pub reason: Option<Reason>,
    /// selectionListValue property.
    pub selection_list_value: Option<FieldValueSelectionListValue>,
    /// selectionValue property.
    pub selection_value: Option<FieldValueSelectionValue>,
    /// textListValue property.
    pub text_list_value: Option<FieldValueTextListValue>,
    /// textValue property.
    pub text_value: Option<String>,
    /// type property.
    pub r#type: Option<String>,
    /// unsetValue property.
    pub unset_value: Option<bool>,
    /// userListValue property.
    pub user_list_value: Option<FieldValueUserListValue>,
    /// userValue property.
    pub user_value: Option<FieldValueUserValue>,
}

/// `OwnerDetails` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct OwnerDetails {
    /// ownerIdentity property.
    pub owner_identity: Option<Vec<OwnerIdentity>>,
    /// ownerType property.
    pub owner_type: Option<String>,
}

/// `Date` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Date {
    /// day property.
    pub day: Option<i64>,
    /// month property.
    pub month: Option<i64>,
    /// year property.
    pub year: Option<i64>,
}

/// `FieldValueSelectionListValue` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct FieldValueSelectionListValue {
    /// values property.
    pub values: Option<Vec<FieldValueSelectionValue>>,
}

/// `ActivityEventsStatus` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ActivityEventsStatus {
    /// errorCode property.
    pub error_code: Option<String>,
    /// errorMessage property.
    pub error_message: Option<String>,
    /// eventStatus property.
    pub event_status: Option<String>,
    /// httpStatusCode property.
    pub http_status_code: Option<i64>,
}

/// `Activities` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Activities {
    /// etag property.
    pub etag: Option<String>,
    /// items property.
    pub items: Option<Vec<Activity>>,
    /// kind property.
    pub kind: Option<String>,
    /// nextPageToken property.
    pub next_page_token: Option<String>,
}

/// `FieldValueSelectionValue` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct FieldValueSelectionValue {
    /// badged property.
    pub badged: Option<bool>,
    /// displayName property.
    pub display_name: Option<String>,
    /// id property.
    pub id: Option<String>,
}

/// `Reason` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Reason {
    /// reasonType property.
    pub reason_type: Option<String>,
}

/// `Activity` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct Activity {
    /// actor property.
    pub actor: Option<std::collections::HashMap<String, serde_json::Value>>,
    /// etag property.
    pub etag: Option<String>,
    /// events property.
    pub events: Option<Vec<std::collections::HashMap<String, serde_json::Value>>>,
    /// id property.
    pub id: Option<std::collections::HashMap<String, serde_json::Value>>,
    /// ipAddress property.
    pub ip_address: Option<String>,
    /// kind property.
    pub kind: Option<String>,
    /// networkInfo property.
    pub network_info: Option<ActivityNetworkInfo>,
    /// ownerDomain property.
    pub owner_domain: Option<String>,
    /// resourceDetails property.
    pub resource_details: Option<Vec<ResourceDetails>>,
}

/// `ActivityNetworkInfo` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct ActivityNetworkInfo {
    /// ipAsn property.
    pub ip_asn: Option<Vec<i64>>,
    /// regionCode property.
    pub region_code: Option<String>,
    /// subdivisionCode property.
    pub subdivision_code: Option<String>,
}

/// `UserIdentity` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct UserIdentity {
    /// id property.
    pub id: Option<String>,
    /// userEmail property.
    pub user_email: Option<String>,
}

/// `FieldValueUserValue` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct FieldValueUserValue {
    /// email property.
    pub email: Option<String>,
}

/// `FieldValueTextListValue` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct FieldValueTextListValue {
    /// values property.
    pub values: Option<Vec<String>>,
}

/// `CustomerIdentity` type.
#[derive(Debug, Clone, Default, Serialize, Deserialize, JsonHash)]
pub struct CustomerIdentity {
    /// id property.
    pub id: Option<String>,
}

// =============================================================================
// ARGS TYPES (per-endpoint)
// =============================================================================

/// Arguments for [`reports.activities.list_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ReportsActivitiesListArgs {
    /// Path parameter: `userKey`.
    pub user_key: String,
    /// Path parameter: `applicationName`.
    pub application_name: String,
    /// Query parameter: `actorIpAddress`.
    pub actor_ip_address: Option<String>,
    /// Query parameter: `applicationInfoFilter`.
    pub application_info_filter: Option<String>,
    /// Query parameter: `customerId`.
    pub customer_id: Option<String>,
    /// Query parameter: `endTime`.
    pub end_time: Option<String>,
    /// Query parameter: `eventName`.
    pub event_name: Option<String>,
    /// Query parameter: `filters`.
    pub filters: Option<String>,
    /// Query parameter: `groupIdFilter`.
    pub group_id_filter: Option<String>,
    /// Query parameter: `includeSensitiveData`.
    pub include_sensitive_data: Option<String>,
    /// Query parameter: `maxResults`.
    pub max_results: Option<String>,
    /// Query parameter: `networkInfoFilter`.
    pub network_info_filter: Option<String>,
    /// Query parameter: `orgUnitID`.
    pub org_unit_id: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
    /// Query parameter: `resourceDetailsFilter`.
    pub resource_details_filter: Option<String>,
    /// Query parameter: `startTime`.
    pub start_time: Option<String>,
    /// Query parameter: `statusFilter`.
    pub status_filter: Option<String>,
}

/// Arguments for [`reports.activities.watch_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ReportsActivitiesWatchArgs {
    /// Path parameter: `userKey`.
    pub user_key: String,
    /// Path parameter: `applicationName`.
    pub application_name: String,
    /// Query parameter: `actorIpAddress`.
    pub actor_ip_address: Option<String>,
    /// Query parameter: `customerId`.
    pub customer_id: Option<String>,
    /// Query parameter: `endTime`.
    pub end_time: Option<String>,
    /// Query parameter: `eventName`.
    pub event_name: Option<String>,
    /// Query parameter: `filters`.
    pub filters: Option<String>,
    /// Query parameter: `groupIdFilter`.
    pub group_id_filter: Option<String>,
    /// Query parameter: `maxResults`.
    pub max_results: Option<String>,
    /// Query parameter: `orgUnitID`.
    pub org_unit_id: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
    /// Query parameter: `startTime`.
    pub start_time: Option<String>,
    /// Request body.
    pub body: Channel,
}

/// Arguments for [`admin.channels.stop_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct AdminChannelsStopArgs {
    /// Request body.
    pub body: Channel,
}

/// Arguments for [`reports.customerUsageReports.get_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ReportsCustomerUsageReportsGetArgs {
    /// Path parameter: `date`.
    pub date: String,
    /// Query parameter: `customerId`.
    pub customer_id: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
    /// Query parameter: `parameters`.
    pub parameters: Option<String>,
}

/// Arguments for [`reports.entityUsageReports.get_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ReportsEntityUsageReportsGetArgs {
    /// Path parameter: `entityType`.
    pub entity_type: String,
    /// Path parameter: `entityKey`.
    pub entity_key: String,
    /// Path parameter: `date`.
    pub date: String,
    /// Query parameter: `customerId`.
    pub customer_id: Option<String>,
    /// Query parameter: `filters`.
    pub filters: Option<String>,
    /// Query parameter: `maxResults`.
    pub max_results: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
    /// Query parameter: `parameters`.
    pub parameters: Option<String>,
}

/// Arguments for [`reports.userUsageReport.get_builder`].
#[derive(Debug, Clone, Default, Serialize, JsonHash)]
pub struct ReportsUserUsageReportGetArgs {
    /// Path parameter: `userKey`.
    pub user_key: String,
    /// Path parameter: `date`.
    pub date: String,
    /// Query parameter: `customerId`.
    pub customer_id: Option<String>,
    /// Query parameter: `filters`.
    pub filters: Option<String>,
    /// Query parameter: `groupIdFilter`.
    pub group_id_filter: Option<String>,
    /// Query parameter: `maxResults`.
    pub max_results: Option<String>,
    /// Query parameter: `orgUnitID`.
    pub org_unit_id: Option<String>,
    /// Query parameter: `pageToken`.
    pub page_token: Option<String>,
    /// Query parameter: `parameters`.
    pub parameters: Option<String>,
}

// =============================================================================
// CLIENT FUNCTIONS (per-endpoint)
// =============================================================================

// -----------------------------------------------------------------------------
// GET admin/reports/v1/activity/users/{userKey}/applications/{applicationName}
// -----------------------------------------------------------------------------

/// GET admin/reports/v1/activity/users/{userKey}/applications/{applicationName}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = reports_activities_list_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn reports_activities_list_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ReportsActivitiesListArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Activities>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://admin.googleapis.com/admin/reports/v1/activity/users/{}/applications/{}",
        args.user_key, args.application_name,
    );

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.actor_ip_address {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("actorIpAddress=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.application_info_filter {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("applicationInfoFilter=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.customer_id {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("customerId=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.end_time {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("endTime=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.event_name {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("eventName=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.filters {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("filters=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.group_id_filter {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("groupIdFilter=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.include_sensitive_data {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("includeSensitiveData=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.max_results {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("maxResults=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.network_info_filter {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("networkInfoFilter=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.org_unit_id {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("orgUnitID=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_token {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageToken=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.resource_details_filter {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("resourceDetailsFilter=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.start_time {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("startTime=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.status_filter {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("statusFilter=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Activities =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST admin/reports/v1/activity/users/{userKey}/applications/{applicationName}/watch
// -----------------------------------------------------------------------------

/// POST admin/reports/v1/activity/users/{userKey}/applications/{applicationName}/watch.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = reports_activities_watch_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn reports_activities_watch_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ReportsActivitiesWatchArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<Channel>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://admin.googleapis.com/admin/reports/v1/activity/users/{}/applications/{}/watch",
        args.user_key, args.application_name,
    );

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.actor_ip_address {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("actorIpAddress=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.customer_id {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("customerId=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.end_time {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("endTime=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.event_name {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("eventName=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.filters {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("filters=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.group_id_filter {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("groupIdFilter=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.max_results {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("maxResults=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.org_unit_id {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("orgUnitID=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_token {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageToken=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.start_time {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("startTime=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: Channel =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// POST admin/reports_v1/channels/stop
// -----------------------------------------------------------------------------

/// POST admin/reports_v1/channels/stop.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = admin_channels_stop_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn admin_channels_stop_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &AdminChannelsStopArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<()>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!("https://admin.googleapis.com/admin/reports_v1/channels/stop",);

    let mut builder = client
        .post(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    builder = builder
        .body_json(&args.body)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream: _,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: (),
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET admin/reports/v1/usage/dates/{date}
// -----------------------------------------------------------------------------

/// GET admin/reports/v1/usage/dates/{date}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = reports_customer_usage_reports_get_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn reports_customer_usage_reports_get_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ReportsCustomerUsageReportsGetArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<UsageReports>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://admin.googleapis.com/admin/reports/v1/usage/dates/{}",
        args.date,
    );

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.customer_id {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("customerId=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_token {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageToken=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.parameters {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("parameters=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: UsageReports =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET admin/reports/v1/usage/{entityType}/{entityKey}/dates/{date}
// -----------------------------------------------------------------------------

/// GET admin/reports/v1/usage/{entityType}/{entityKey}/dates/{date}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = reports_entity_usage_reports_get_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn reports_entity_usage_reports_get_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ReportsEntityUsageReportsGetArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<UsageReports>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://admin.googleapis.com/admin/reports/v1/usage/{}/{}/dates/{}",
        args.entity_type, args.entity_key, args.date,
    );

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.customer_id {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("customerId=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.filters {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("filters=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.max_results {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("maxResults=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_token {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageToken=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.parameters {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("parameters=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: UsageReports =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}

// -----------------------------------------------------------------------------
// GET admin/reports/v1/usage/users/{userKey}/dates/{date}
// -----------------------------------------------------------------------------

/// GET admin/reports/v1/usage/users/{userKey}/dates/{date}.
///
/// Takes client and args, builds the request, optionally applies modifications,
/// and returns a `TaskIterator` for execution.
///
/// # Arguments
///
/// * `client` - HTTP client for making the request
/// * `args` - Request arguments (path params, query params, body)
/// * `builder_mod` - Optional closure to modify the request builder (e.g., add headers)
///
/// # Example
///
/// ```ignore
/// let task = reports_user_usage_report_get_request(&client, &args, Some(|b| {
///     b.header("X-Custom-Header", "value")
/// }))?;
/// ```
#[inline]
pub fn reports_user_usage_report_get_request<R, F>(
    client: &SimpleHttpClient<R>,
    args: &ReportsUserUsageReportGetArgs,
    builder_mod: Option<F>,
) -> Result<
    impl TaskIterator<
            Ready = Result<ApiResponse<UsageReports>, super::shared::ApiError>,
            Pending = super::shared::ApiPending,
            Spawner = super::shared::BoxedSendExecutionAction,
        > + Send
        + 'static,
    super::shared::ApiError,
>
where
    R: foundation_core::wire::simple_http::client::DnsResolver + Clone + Default + 'static,
    F: FnOnce(&mut ClientRequestBuilder<R>),
{
    let endpoint_url = format!(
        "https://admin.googleapis.com/admin/reports/v1/usage/users/{}/dates/{}",
        args.user_key, args.date,
    );

    let endpoint_url = {
        let mut url = endpoint_url;
        let mut first = true;
        if let Some(ref v) = args.customer_id {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("customerId=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.filters {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("filters=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.group_id_filter {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("groupIdFilter=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.max_results {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("maxResults=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.org_unit_id {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("orgUnitID=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.page_token {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("pageToken=");
            url.push_str(&urlencoding::encode(v));
        }
        if let Some(ref v) = args.parameters {
            if first {
                url.push('?');
                first = false;
            } else {
                url.push('&');
            }
            url.push_str("parameters=");
            url.push_str(&urlencoding::encode(v));
        }
        url
    };

    let mut builder = client
        .get(&endpoint_url)
        .map_err(|e| super::shared::ApiError::RequestBuildFailed(e.to_string()))?;

    if let Some(f) = builder_mod {
        f(&mut builder);
    }

    Ok(builder
        .build_send_request()
        .map_err(|e: foundation_core::wire::simple_http::HttpClientError| {
            super::shared::ApiError::RequestBuildFailed(e.to_string())
        })?
        .map_ready(|intro| match intro {
            super::shared::RequestIntro::Success {
                stream,
                intro,
                headers,
                ..
            } => {
                let status: usize = intro.0.into();
                if status < 200 || status >= 300 {
                    return Err(super::shared::ApiError::HttpStatus {
                        code: status as u16,
                        headers: headers.clone(),
                        body: None,
                    });
                }
                let body =
                    foundation_core::wire::simple_http::client::body_reader::collect_string(stream);
                let parsed: UsageReports =
                    serde_json::from_str(&body).map_err(|e: serde_json::Error| {
                        super::shared::ApiError::ParseFailed(e.to_string())
                    })?;
                Ok(ApiResponse {
                    status: status as u16,
                    headers: headers.clone(),
                    body: parsed,
                })
            }
            super::shared::RequestIntro::Failed(e) => {
                Err(super::shared::ApiError::RequestSendFailed(e.to_string()))
            }
        })
        .map_pending(|_| super::shared::ApiPending::Sending))
}
