...
pub struct GoogleCloudPolicysimulatorV1betaCreateOrgPolicyViolationsPreviewOperationMetadata {
    ...
    pub resources_found: Option<i32>,
...
pub struct GoogleCloudPolicysimulatorV1betaOrgPolicyOverlay {
    ...
    #[serde(default, rename = "customConstraints")]
    pub custom_constraints:
        Option<Vec<GoogleCloudPolicysimulatorV1betaOrgPolicyOverlayCustomConstraintOverlay>>,
    /// Optional. The OrgPolicy changes to preview violations for. Any existing OrgPolicies with the same name will be overridden in the simulation. That is, violations will be determined as if all policies in the overlay were created or updated.
    #[serde(default)]
    pub policies: Option<Vec<GoogleCloudPolicysimulatorV1betaOrgPolicyOverlayPolicyOverlay>>,
}
...
pub struct GoogleCloudPolicysimulatorV1betaOrgPolicyViolationsPreview {
    ...
    /// Required. The proposed changes we are previewing violations for.
    #[serde(default)]
    pub overlay: Option<GoogleCloudPolicysimulatorV1betaOrgPolicyOverlay>,
    /// Output only. A summary of the state of all resources scanned for compliance with the changed OrgPolicy.
    #[serde(default, rename = "resourceCounts")]
    pub resource_counts:
        Option<GoogleCloudPolicysimulatorV1betaOrgPolicyViolationsPreviewResourceCounts>,
    /// Output only. The state of the OrgPolicyViolationsPreview. // TODO: enum values: ["PREVIEW_STATE_UNSPECIFIED", "PREVIEW_PENDING", "PREVIEW_RUNNING", "PREVIEW_SUCCEEDED", "PREVIEW_FAILED"]
    #[serde(default)]
    pub state: Option<String>,
    /// Output only. The number of OrgPolicyViolations in this OrgPolicyViolationsPreview. This count may differ from resource_summary.noncompliant_count because each OrgPolicyViolation is specific to a resource **and** constraint. If there are multiple constraints being evaluated (i.e. multiple policies in the overlay), a single resource may violate multiple constraints.
