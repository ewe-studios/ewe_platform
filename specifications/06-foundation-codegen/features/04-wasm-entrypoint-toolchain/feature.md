...
pub extern "C" fn main() {
    my_crate::handlers::auth_handler();
}
```

**Note:** The generated file uses the crate's library path to call the original function. This means:
- The crate must expose the function through its public API (`pub fn` + `pub mod`)
- The import path is the `qualified_path` from the codegen scanner minus the function name suffix, reconstructed as a proper use path
