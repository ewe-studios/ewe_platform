...
pub async fn run_server(config: KeychainConfig) -> Result<(), AppError> {
    ...
    // Start cron scheduler (foundation_cronjobs)
    let scheduler = CronScheduler::new(&ctx.db)?;
    scheduler.register("purge-expired-sends", "0 */6 * * *", || purge_expired_sends(&ctx.db, &ctx.blob)).await?;
    scheduler.register("purge-trashed-ciphers", "0 0 * * *", || purge_trashed_ciphers(&ctx.db)).await?;
