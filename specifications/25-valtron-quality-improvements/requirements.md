...
| # | Feature | Priority | Description | Tasks | Dependencies |
|---|---------|----------|-------------|-------|--------------|
| 00 | [pending-none-livelock](./features/00-pending-none-livelock/) | CRITICAL | Fix unbounded CPU spin when tasks return Pending(None) | 9 | None |
| 01 | [sleeper-lifecycle-safety](./features/01-sleeper-lifecycle-safety/) | CRITICAL | Fix stale sleeper entries that can panic executor on task combination | 6 | None |
| 02 | [linked-task-state-propagation](./features/02-linked-task-state-propagation/) | HIGH | Fix DualSequence silently discarding parent State signals | 5 | 01 |
| 03 | [global-queue-fairness](./features/03-global-queue-fairness/) | HIGH | Add fairness mechanism for global queue pickup | 4 | None |
| 04 | [notification-based-waiting](./features/04-notification-based-waiting/) | HIGH | Replace spin+sleep polling with CondVar notification | 6 | None |
| 05 | [iterator-semantics](./features/05-iterator-semantics/) | HIGH | Fix TaskIterator recursion trap, TransformIterator, CollectAllStream | 6 | None |
| 06 | [error-handling-api](./features/06-error-handling-api/) | MEDIUM | Fix error swallowing, executor panics, Drain bounds | 5 | None |
| 07 | [channel-backpressure](./features/07-channel-backpressure/) | MEDIUM | Add bounded queue option and EntryList slot reuse safety | 4 | 04 |
| 08 | [code-quality](./features/08-code-quality/) | LOW | WASM variable fix, BranchPath naming, alias cleanup | 3 | None |
...
1. **Phase 1 (Critical):** Features 00, 01 — fix the livelock and panic first
