...
`ewe-wasm build` CLI binary that orchestrates the full build pipeline: compiles WASM, collects JS runtimes, invokes `foundation_codegen` for bundling. Proc macro annotations (`#[wasm_bin]`, `#[wasm_worker]`, `#[wasm_service]`) mark entry points — the CLI reads them and generates output.
...
- Feature 00 (JS runtime assets to bundle)
