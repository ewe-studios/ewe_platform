...
| Part B — foundation_wasm_ui crate | DONE. `protocol/{arrow,custom_binary,json}.rs`, `instruction/receiver.rs`, `wasm/dom/{constants,element}.rs`, `embedded.rs` (feature `embedded-js`, forwards to `foundation_wasm/embedded-js`; embeds both runtime files). |
| Part C — three-layer protocols | DONE. Layer 1 encoders in `foundation_ui_traits`; Layer 2 `ProtocolHandler` + `WasmEnvelope` + `dispatch_message` in `foundation_wasm`; Layer 3 `ArrowV1`/`CustomBinaryV1`/`JsonV1` + `ProtocolMethods` in `foundation_wasm_ui`. |
