...
1. [ ] Define `SignalKind`, `SignalEvent`, `SignalError` in `signal/mod.rs`
2. [ ] Implement `SignalBus` (fan-out to subscribers via `ConcurrentQueue`)
...
6. [ ] Implement Windows backend (SetConsoleCtrlHandler + event + WaitForSingleObject)
7. [ ] Add `signal` feature flag to `Cargo.toml` (default)
8. [ ] Re-export from crate root when feature enabled
9. [ ] Write tests: signal delivery (Unix only — send SIGUSR1 to self)
10. [ ] Verify Windows compilation (no-op stubs for SIGINT/SIGTERM mapping)
