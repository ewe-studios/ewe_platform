...
feature_directory: "specifications/36-agentic-api/features/11-foundation-vectors-code-graph"
this_file: "specifications/36-agentic-api/features/11-foundation-vectors-code-graph/start.md"
created: 2026-06-14
---
...
3. **Stack:** Rust + tree-sitter. Read `.agents/skills/rust-clean-code/skill.md`. Confirm F08 landed.
4. Read `../../LEARNINGS.md`. Resolve OD-11-1..7 (esp. the 11a/b/c split + language scope) before coding.
...
7. Report; verify (native build + wasm query path); update `../../LEARNINGS.md`; move to Feature 12.

---

**Workflow:** feature.md → graphify SOURCE → tree-sitter + Rust skill → Resolve OD-11 → Compact → ONE LANG AT A TIME (fixtures) → Report → Verify → 12
