...
> `scan_from`/columns. See the new Part 0 and OD-04-4/5/6.
...
The Message API (F16) and memory hierarchy (F18/F19) need two capabilities the current
`DocumentStore` lacks:
...
3. **Caller-supplied ids.** Add `append_with_id(key, id, content)` (or `append(key, Option<&str> id,
   content)`) so the agentic message scru128 **is** the `doc_id` — required for F16/F18 to anchor
   `scan_from` on a message id they hold (OD-04-5). Plain `append` keeps generating one.

This is additive at the API level but **changes ordering semantics** of existing scans (now strictly
time-ordered by id rather than coarse `created_at`) — call out in the migration/changelog. OD-04-4.
...
- **`AsyncDocumentStore`:** add `scan_from_async` returning `StorageResult<Vec<V>>` (the async trait
  ...
  on wasm (`storage_provider.rs:27-33`). **No `AsyncDocumentStore` backend exists yet** — VFS is F05,
  CF KV/D1 is F06; `scan_from_async`'s backend implementations land there (OD-04-7).
...
- **Write path:** `append` gains an overload / companion that accepts the promoted fields, or
  ...
  implemented by F01's `SessionRecord`. See OD-04-1.
...
- **Read path (gap to close):** `scan`/`scan_all` currently `SELECT content` and deserialize to `V`
  ...
  `scan_documents_from(key, from_id, limit)`) that SELECTs the columns, so F16/F18 can filter/sort on
  them. OD-04-6.
- **Memory/Message search (F16/F18)** then `WHERE record_type = 'observation'` / `ORDER BY doc_id` /
  `LIKE` on `title`/`summary` without deserializing blobs.

> **OD-04-1:** how do promoted fields get populated — an extended `append_with(content, title,
...
graph LR
    ...
    F16[Message API] -->|scan_from id| SQL
    F18[Context recall] -->|WHERE record_type=observation| SQL
```
...
5. Add the write path for promoted fields (OD-04-1: `PromotableDocument` trait recommended).
