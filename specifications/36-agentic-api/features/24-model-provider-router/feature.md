...
# Feature 24: ModelProviderRouter
...
>    router needs an **object-safe boxed surface**. **OD-24-1 (load-bearing):** define a new
...
>    stream (`Box<dyn StreamIterator<D=Messages,P=ModelState>>`). (OD-24-2.)
...
>    `routes: model→provider` map. (OD-24-3.)
...
>    router data model supports N providers per model (a `Vec<provider_idx>` per route) but F24 ships
>    single-winner routing; the fallback *iteration* is wired by F30's circuit breaker later. Keep the
>    `Vec` so F30 can light it up without a reshape.
> 6. **F15 (Embedding) declared F24 as a hard dep** (`15-embedding-provider/feature.md` review #6:
>    "F24 ProviderRouter is an undeclared dep and UNWRITTEN"). F24 must therefore also route the
>    **embedding** model (which can differ from the chat model). The erased surface includes the
>    embedding generation path (the marker-message `ModelOutput::Embedding`, F15 review #2) so F15 routes
>    through F24. (OD-24-4.)
> 7. **This is the spec's one "(new)" feature** (requirements Feature Index F12 row: "(new)") — no
...
The agent loop (F27), memory generation (F19), embeddings (F15), and the circuit breaker (F30) all
...
### Object-safe erased surface (OD-24-1/2)
...
impl ProviderRouter {
    ...
    /// Used by F19/F30 to pick the memory/fallback model handle.
...
The `routes` value is a `Vec<usize>` so F30 can later iterate providers for **same-model fallback**
(design-aware now, F30 lights it up).

## Architecture

```mermaid
graph TD
    A[Agent loop F27 / memory F19 / embed F15] -->|generate by ModelId| R[ProviderRouter]
    R --> RES{resolve}
    ...
    AD2 --> GEN
    note[Vec provider indices per model -> F30 fallback future]
```
...
1. `RoutableProvider` (object-safe) + `ProviderAdapter<P: ModelProvider>` blanket impl (OD-24-1/2).
...
5. `memory_model` helper (chat≠memory model) for F19/F30; embedding routing path for F15 (OD-24-4).
6. Keep `Vec<usize>` routes for future F30 same-model fallback (design-aware).
7. Tests: single-provider pass-through; two-provider routing by declared support; explicit rule
   override; unresolved model errors cleanly; memory model resolves; a `MockModelProvider` (F32)
   round-trips `generate`/`stream` through the adapter; wasm build (with wasm-capable providers F00c).

## Open Decisions

- **OD-24-1 — erased surface (load-bearing):** `RoutableProvider` adapter (rec) — required because
...
- **OD-24-2 — boxed stream:** `Box<dyn StreamIterator<D=Messages,P=ModelState>>` to erase
  `Model::stream`'s `impl StreamIterator`. Rec: yes. Confirm the boxing is acceptable on the hot path.
- **OD-24-3 — support detection:** `get_one(model_id)` resolving vs `NotFound` as the `serves` signal,
  plus an optional explicit route map. Rec: both (rule first, then `get_one`).
- **OD-24-4 — embedding routing:** F15 routes embeddings through F24 (embedding model may differ).
  Rec: yes; `embedding_model` resolves like chat. Confirm F15↔F24 contract.
- **OD-24-5 — same-model fallback:** ship single-winner now, keep `Vec<usize>` for F30. Confirm the
  fallback iteration belongs to F30, not F24.

## Target Files

- `backends/foundation_ai/src/agentic/router.rs` (new) — `RoutableProvider`, `ProviderAdapter`,
  `ProviderRouter`
- coordinates the existing `ModelProvider`/`Model`/`ModelInteraction`/`ModelId` (`types/mod.rs`), F15
  (embedding routing), F19 (memory model), F30 (fallback/circuit breaker), F31 (builder takes a router)
...
- `ProviderRouter` fronts one or many providers behind an object-safe `RoutableProvider` adapter
  ...
  `Vec` routes retained for F30 same-model fallback; builds native + wasm.
- OD-24-1..5 resolved (OD-24-1 flagged for the user); fundamentals authored.
