...
>    PauseForPriority = 1, Abort = 2 }`. F13 ships the valid version. The signal itself is an
...
>    primitive. F13 does NOT build these — it consumes them and the `MessageRole` enum is from F01.
...
>    I have not opened `dependent_lift.rs`. (OD-13-4.)
> 5. **F13 is consumed by F11 (tool interruption) and F19 (the loop).** F13 owns the *primitives*
>    (queues, `CancelCode`, the readiness wiring, the sequenced-composition helper); F11 checks the
>    PriorityQueue between tool stages; F19 drains PriorityQueue at the FRONT of the inner loop and
>    FollowUpQueue at the outer-loop boundary (Decision 05 §Agent Loop Integration). Keep the boundary
>    crisp: F13 = mechanism, F11/F19 = policy.
...
>    boundary. On session end (F20/Decision 01), both queues are drained → persisted to the Message API
...
`Depends` + `QueueReadiness` exist precisely for this. This feature provides those primitives so F11
and F19 can implement interruption/continuation policy on top.
...
```rust
// In the agent loop (F19) when there is nothing to do but await steering/follow-up:
...
|-------|-----------------|-------------------|---------|--------------|
| **PriorityQueue** | yes | yes (F11) | each LLM/tool step | `PauseForPriority` |
| **FollowUpQueue** | no | no | outer-loop boundary (F19) | never |
...
5. Sequenced composition helper: compose an agent parent with an LLM child via valtron `sequenced`
   (verify the exact spawn API in `dependent_lift.rs`, OD-13-4).
...
- coordinates F01 (`Messages`/`MessageRole`), valtron (`ConcurrentQueue`, `QueueReadiness`,
  `EventReadiness`, `TaskStatus::Depends`, sequenced), F11 (tool interruption), F19 (loop policy),
  F20 (drain on end)
...
- OD-13-1..5 resolved (OD-13-4 flagged); fundamentals authored.
