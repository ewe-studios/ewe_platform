...
status: "pending"
priority: "high"
depends_on: ["20-toolimpl-registry", "18-context-provider-assembly"]
...
# Feature 22: Search Tools — `search()` vs `search_file()`

> **Review status (2026-06-14) — self-review against code (subagent unavailable):**
> 1. **The split is already half-built in F18.** `ContextProvider` (`18-context-provider-assembly/
>    feature.md:57-72`) already defines `search(query, SearchMode, k)` (Semantic/Memory/Graph/Hybrid)
>    and `search_file(query, FileSearchKind)` (fff, native; wasm unsupported). **F22 owns the *tool
>    surface* — the `SearchTool`/`SearchFileTool` `ToolImpl`s — that wrap F18's methods.** F22 does NOT
>    re-implement recall; it adapts the F18 Context API into two `ToolImpl`s the LLM calls. (OD-18-5
>    already assigns generation to F19; here the boundary is F18=capability, F22=tool wrapper.)
...
>    integration question (OD-22-1):** it is not a workspace member here and not on crates.io as a
...
>    TODO #6, echoed in F18): `search` is knowledge-only (NO fff), `search_file` is filesystem-only (fff,
>    no vector fallback). F22 implements the *split*, not the old two-phase. The two tools never call
>    each other.
> 4. **`search` modes map to existing subsystems:** Semantic → F16 `semantic_search` (message vectors,
>    ns=session); Memory → F12 query over observation/reflection vectors (F07/F19 content); Graph →
>    F11 code-graph (`find_entity`/`callers`/`neighborhood`); Hybrid → F10 fusion. All reached via F18's
>    `search`. On wasm, Graph degrades to a prebuilt-graph query or is unavailable (F18 OD-18-3).
> 5. **Both are `ToolImpl`s (F20), sync `execute`** (F20 OD-20-1). They are wired by F21's
...
>    `search_file` rides as a normal registered tool discoverable via `shed`). (OD-22-4.)
...
> `search` (knowledge — semantic/memory/graph/hybrid, wraps F18) and `search_file` (filesystem — fff,
> native-only, wasm-unsupported). Both are `ToolImpl`s (F20) wired into the `ToolShed` (F21).
...
F18 added the *capabilities*; this feature exposes them as two distinct, well-described tools.

## WHAT: Solution

### `search` — knowledge tool (wraps F18, all platforms)

```rust
// backends/foundation_ai/src/agentic/tools/search.rs
pub struct SearchTool { context: ContextProvider }   // F18

#[derive(Serialize, Deserialize)]
pub struct SearchArgs { pub query: String, pub mode: SearchMode, pub k: usize }
// SearchMode { Semantic, Memory, Graph, Hybrid } — re-exported from F18
...
impl ToolImpl for SearchTool {
    ...
    fn execute(&self, args: HashMap<String, ArgType>) -> Result<ToolCallResult, ToolError> {
        ...
        let res = self.context.search(&a.query, a.mode, a.k);   // F18
...
pub struct SearchFileTool {
    ...
    fff: FffSearch,    // F18's fff binding (grep/find/multi_grep) — native only
}
...
is defined in F18 / consumed here; the **fff dependency wiring is OD-22-1** (external workspace).
...
graph TD
    ...
    ST --> CTX[ContextProvider.search F18]
    CTX --> SEM[Semantic: F16 message vectors]
    CTX --> MEM[Memory: F12 obs/refl vectors]
    CTX --> GR[Graph: F11 code-graph]
    CTX --> HY[Hybrid: F10 fusion]
...
1. `SearchTool: ToolImpl` wrapping F18 `ContextProvider::search` (Semantic/Memory/Graph/Hybrid).
2. `SearchFileTool: ToolImpl`, target-gated execute (native fff vs wasm unsupported result).
3. Resolve fff consumption (OD-22-1: git/path dep vs vendor vs minimal reimpl) — wire `FffSearch`.
...
6. Register both in F21 `with_defaults` (`search` = ToolShed.search field; `search_file` = extra
   registered tool, shed-discoverable).
7. Tests: `search` each mode dispatches to the right F18 path; `search_file` grep/find/multi_grep
...
- **OD-22-1 — fff consumption (load-bearing):** fff is an external workspace, not a repo member / not a
...
- `backends/foundation_ai/Cargo.toml` — fff dep (target-gated, per OD-22-1)
- coordinates F18 (`ContextProvider::search`/`search_file`, `FffSearch`), F20 (`ToolImpl`), F21
  (`with_defaults` wiring), F10/F11/F12/F16 (the search backends behind F18)
...
- `search` (knowledge: semantic/memory/graph/hybrid via F18) and `search_file` (filesystem: fff,
  ...
  both registered via F21; the two tools never call each other (the split, not the old two-phase).
- OD-22-1..5 resolved (OD-22-1 flagged for the user); fundamentals authored.
