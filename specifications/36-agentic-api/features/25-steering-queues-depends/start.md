...
feature_directory: "specifications/36-agentic-api/features/25-steering-queues-depends"
this_file: "specifications/36-agentic-api/features/25-steering-queues-depends/start.md"
created: 2026-06-14
---
...
1. Read `feature.md` + Decision 05 + Decision 08. Read the REAL valtron primitives:
   ...
   `concurrent_queue::ConcurrentQueue`. Read `dependent_lift.rs` for the sequenced spawn API (OD-25-4).
...
3. Read `../../LEARNINGS.md`. Resolve OD-25-1..5. **OD-25-4 (confirm sequenced spawn API name)** —
...
7. Report; verify (native+wasm); update `../../LEARNINGS.md`; move to Feature 26.

---

**Workflow:** feature.md → Decision 05/08 → real Depends/QueueReadiness + dependent_lift → Resolve OD-25 (flag 4) → Compact → ONE ITEM → fundamentals → Report → Verify → 26
