...
Semantic recall over messages/observations (F18) needs both: "find where I mentioned `auth_check(`"
...
```rust
/// Classic BM25 over a tokenized corpus. Higher score = more relevant (matches F08 convention).
...
impl Bm25Index {
    ...
    pub fn to_bytes(&self)/from_bytes(...)            // persistence (F13)
}
```

- **Tokenization:** lowercase + simple unicode word-split now; optional `nlprule`/stemming later
  (consistent with F15's embedding tokenization note). OD-10-1.
...
- **Rerank (optional, native/feature):** a cross-encoder pass over the fused top-N for precision.
  ...
  -> Vec<VectorMatch> }`), not implemented here; F15/provider supplies one. OD-10-2.
...
Retrieve top-N from each (N≥k, e.g. 2k), fuse to k, optional rerank. Used by F18's `search()`
...
4. Tests: BM25 correctness vs known rankings; RRF fusion on synthetic lists; alpha bounds (0→BM25,
   1→vector); hybrid end-to-end with F08/F09 indexes; serialize round-trip; wasm32 build.

## Open Decisions

- **OD-10-1 — tokenization:** simple unicode split now vs `nlprule`/stemming. Rec: simple now,
  pluggable tokenizer trait so F15's nlprule can be shared.
- **OD-10-2 — reranker:** hook-only here (no model); a concrete cross-encoder is a later/provider
  feature. Rec: trait hook now.
- **OD-10-3 — RRF default k:** 60 (common default). Rec: 60, configurable.
- **OD-10-4 — BM25 persistence size:** the inverted index can be large; share the F13
  serialize/persist path. Rec: yes, `to_bytes`/`from_bytes` like `VectorIndex`.
- **OD-10-5 — where hybrid lives:** `foundation_vectors` (algorithms) vs F18 (orchestration). Rec:
  the `fuse`/BM25 primitives here; F18 wires the actual session search.

## Target Files

- `backends/foundation_vectors/src/{bm25.rs, fusion.rs, hybrid.rs}` (new)
- builds on F08 (`VectorMatch`), F09 (`VectorIndex`)
...
- `Bm25Index` (insert/search/remove/serialize) is correct; `fuse` (RRF default + alpha) and
  `hybrid_search` work with F08/F09 indexes; `Reranker` is a hook.
