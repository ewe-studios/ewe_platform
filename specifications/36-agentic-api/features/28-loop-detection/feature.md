...
status: "pending"
priority: "high"
depends_on: ["01-message-model", "19-memory-hierarchy"]
...
>    exclusive wirings and F26 OD-26-4 explicitly defers the choice here. **Recommendation: output
>    processor (Decision 11).** Rationale: detection is a cheap, synchronous `check(&ModelOutput)` over a
>    sliding window — it needs the assistant message the loop just produced, which the F26 output pipeline
...
>    A processor is simpler, deterministically ordered, and trivially testable. **Flag for the user; F26
...
>    order (sort keys) since `HashMap` iteration is unordered. (OD-28-2.)
> 3. **SimHash needs a hasher; `foundation_rng` has none** (F15 review #5 noted "no general hasher").
>    SimHash tokenizes the text, hashes each token (64-bit), and sums signed bit-vectors → Hamming
>    distance ≈ similarity. Pick a non-crypto 64-bit hasher (xxhash/ahash) — same dep F15 adds (OD-15-2).
>    F28 reuses it; do not add a second hasher. (OD-28-3.)
> 4. **Redirect is built from memory (F19), as `Messages::User { role: MessageRole::System }`** (Decision
...
>    F18 INCON-03). So F28 reads F19's latest snapshots (`latest_working`/`latest_reflection`). The
...
>    → switch model (F24 fallback) OR bump temperature (`ModelParams.temperature`, real field
>    `types/mod.rs:382`) by `temperature_delta`; 3rd → terminate with
>    `AgenticError::LoopDetected(LoopDetection)` (F30, Decision 16 line 61). `LoopDetection` must be
>    `Clone+PartialEq+Debug` so it embeds in `AgenticError` (F30 stream derives). (OD-28-4.)
> 6. **Semantic detection is DEFERRED** (Decision 09 §Alternatives "Deferred to optimization phase —
>    start with SimHash"). F28 ships exact + fuzzy(SimHash) + tool-call; the description includes a
...
> deferred), redirects the LLM using F19 memory, escalates (model switch / temperature) on persistence,
> and terminates after `max_redirects`. **Resolves the Decision 08-vs-11 execution-model question:
> recommended as an OUTPUT PROCESSOR (F26) — flagged for the user.**
...
give up cleanly rather than spin forever. F27 needs a detector it can run each turn; this feature is it.
...
#[derive(Debug, Clone, PartialEq)]              // Clone+PartialEq+Debug for AgenticError (F30)
...
impl LoopDetector {
    /// Cheap synchronous check over the latest assistant output. Called by F26's output pipeline.
    pub fn check(&mut self, output: &ModelOutput) -> LoopDetection;
    /// Build a redirect Messages::User{ System } from F19 memory (Decision 09 build_redirect).
...
pub enum Escalation {
    ...
    SwitchModelOrTemperature { delta: f32 }, // 2nd: F24 fallback or +temperature_delta
...
F19 `latest_working()` / `latest_reflection()`.
...
graph TD
    ...
    FZ --> ESC
    TC --> ESC
    ESC -->|1st| RD[redirect from F19 memory -> front-inject]
    ESC -->|2nd| SW[F24 model switch / +temperature]
    ESC -->|3rd| TERM[AgenticError::LoopDetected -> F30]
```
...
3. SimHash fuzzy detection (reuse F15's 64-bit hasher; tokenization → signed bit-vector → similarity).
4. Tool-call detection (`ToolCallSignature` with sorted-key argument hash; pattern repeat count).
5. `build_redirect` from F19 memory as `Messages::User{ System }`.
6. `escalate` ladder (redirect → switch/temperature → terminate) + `redirect_count`.
7. **Resolve execution model (OD-28-1): output processor (rec) — provide `LoopDetectorProcessor`
   implementing F26's `OutputProcessor`.** (Flag for the user.)
...
- **OD-28-1 — execution model (NEEDS USER RULING):** output processor (Decision 11, **rec**) vs sequenced
  parallel task (Decision 08). Rec: output processor — cheap sync check, gets the output directly from
  F26, no cross-task sync. **Flag for the user.**
- **OD-28-2 — tool-call arg hashing:** sort `HashMap` keys before hashing for determinism (rec) — raw
  iteration order is unstable.
- **OD-28-3 — SimHash hasher:** reuse F15's non-crypto 64-bit hasher (rec); don't add a second.
- **OD-28-4 — termination error:** `AgenticError::LoopDetected(LoopDetection)` (Decision 16) — requires
  `LoopDetection: Clone+PartialEq+Debug` (done). Confirm.
- **OD-28-5 — semantic detection:** deferred (Decision 09). Confirm v1 ships exact+fuzzy+tool-call only.

## Target Files

- `backends/foundation_ai/src/agentic/loop_detection.rs` (new) — `LoopDetector`, `LoopDetection`,
  `LoopDetectorProcessor` (if OD-28-1 = processor)
- coordinates F01 (`ModelOutput`/`Messages`/`MessageRole`), F19 (memory for redirect), F24 (model
  switch), F26 (output processor host), F27 (escalation handling), F30 (`AgenticError::LoopDetected`),
  F15 (shared hasher)
...
- Exact / fuzzy (SimHash) / tool-call detection over a sliding `ModelOutput` window; memory-grounded
  redirect (F19) as a `System` message; escalation ladder (redirect → model/temperature → terminate
...
- OD-28-1..5 resolved (OD-28-1 NEEDS USER RULING); fundamentals authored.
