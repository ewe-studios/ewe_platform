...
description: "New foundation_vectors crate — Vector type, cosine/L2/dot distance metrics, and a brute-force flat-scan top-k search. Pure Rust, WASM-safe; the algorithmic foundation the VectorStore backends (F12-14) call"
status: "pending"
priority: "high"
depends_on: []
...
# Feature 08: foundation_vectors — core + distance + flat scan
...
> or a squared-norm/normalized-dot formulation (revised OD-08-5); (3) cosine needs a **zero-vector
...
> and code-graph (F11) build on this. Pure Rust + WASM-safe (no native-only deps).
...
correct search (flat scan) — enough for the in-memory `VectorStore` (F12) and as the fallback path
for all backends.
...
```rust
/// A dense embedding. Dimension is carried for validation (cross-model mixing is a bug — F06).
...
- **Cosine** is primary for text embeddings; precompute norms where possible (OD-08-1: store
...
- Dimension mismatch is a `debug_assert!` + a checked `try_score` returning `Err` (OD-08-2).
...
- This is what the in-memory `VectorStore` (F12) uses, and the fallback for backends without native
  ANN (F13/F14).

### Numerics & determinism

- f32 throughout (embeddings are f32; matches `ModelOutput::Embedding.values: Vec<f32>`).
- NaN guard: treat NaN scores as -inf (never selected). Ties broken by id for determinism (OD-08-3).
...
graph TD
    ...
    F12[in-memory VectorStore] --> F
    F13[native backends fallback] --> F
```
...
3. Normalized-vector option for cosine (OD-08-1).
...
- **OD-08-1 — normalized storage:** store L2-normalized vectors so cosine == dot (faster queries) vs
  normalize per-query. Rec: normalized storage (the store normalizes on insert).
- **OD-08-2 — dimension mismatch:** `debug_assert!` + checked `try_score(-> Result)`; the VectorStore
...
- The higher-is-better score convention is uniform (ready for IVF/HNSW in F09).
- OD-08-1..5 resolved.
