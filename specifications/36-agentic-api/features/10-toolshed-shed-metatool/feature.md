...
> 1. **`ToolShed.others` removal is F01's job, not F10's.** The real struct
>    (`types/mod.rs:1067-1077`) still has `others: Option<Vec<Tool>>` (:1076); F01 removes it (F01
>    feature.md resolves CRIT-04). F10 *consumes* the post-F01 `ToolShed` and must NOT reintroduce a
>    catch-all — discovery is the `shed` tool's job (Decision 15 §"Why no others"). F10's contribution
...
>    - `ToolShed` gains a first-class **`search_files`** field (fff filesystem tool, F32) beside
...
> 3. **The `shed` tool's search uses the SAME VectorStore the registry indexed into** (F09 register
>    hook → F28 insert with a tool-description namespace). `shed` embeds the LLM's NL query via F31 and
>    calls `VectorStore::query(vec, k, Some("tools"))`. So F10 hard-depends on **F31 (Embedding) + F28
>    (VectorStore)** — the embedding the F09 register-hook deferred is realized here. (OD-10-2.)
> 4. **`shed` is a `ToolImpl` like any other** — it implements F09's trait; `definition()` returns the
...
>    immediately call it) OR a follow-up "activate this tool into the ToolShed" step. **OD-10-3:** rec —
...
> 6. **`ToolFormatter` (`types/mod.rs:1140`) flattens `ToolShed` → `Vec<Tool>` → provider JSON.** F10
>    must define the `ToolShed -> Vec<Tool>` flattening (today there's `build_toolshed` in F09 producing
>    the struct, but the struct→`[Tool]` flatten that feeds `format_tools(&[Tool])` (:1147) is
>    unimplemented). F10 owns `ToolShed::all_tools(&self) -> Vec<Tool>`.
...
> the **`ToolShed → Vec<Tool>` flatten** that feeds `ToolFormatter`. Depends on F09 (registry) for the
...
tool to parse YAML?" — via semantic search over tool descriptions. F09 built the registry; this
...
impl ToolCallManager {
    ...
    pub fn with_defaults(session_id: SessionId, vs: Arc<dyn VectorStore>, emb: Arc<dyn EmbeddingProvider>) -> Self {
        ...
        mgr.register(Arc::new(SearchTool::default()));           // F32 search() — knowledge
        mgr.register(Arc::new(SearchFilesTool::default()));      // F32 search_files() — fff filesystem
...
pub struct ToolSummary {
    ...
    pub category: String,
    pub schema: Option<serde_json::Value>,   // full Args schema for top hits (OD-10-3)
}
...
### Tool discovery index (the F09 register-hook, realized)
...
The tool-description vectors live in their **own namespace** (`"tools"`) in the same VectorStore (F28),
...
graph TD
    ...
    REG[register hook F09] -->|index description| DISC[ToolDiscovery]
    DISC -->|embed F31 + insert ns=tools| VS[(VectorStore F28)]
...
1. `ToolDiscovery` (embed via F31 + insert/query F28 under ns=`"tools"`); wire to F09's register hook.
2. `ShedTool: ToolImpl` (`ShedQuery`/`ShedResult`/`ToolSummary`, with optional full schema per OD-10-3).
...
5. Reconcile non-`Option` `read/edit/write/search` fields (OD-10-1): default-wire real tools.
...
- **OD-10-1 — ToolShed field optionality:** make `read/edit/write/search` `Option<Tool>` in F01 (rec)
  vs `default()` always installs real defaults. Rec: `default()` installs real defaults so the
  non-`Option` fields are honestly filled; coordinate with F01/F09 OD-09-4. Flag.
...
- **OD-10-2 — discovery deps:** `shed` hard-requires F31 + F28. On wasm both work (in-memory). Confirm
...
- **OD-10-3 — summary vs schema:** include full `Tool` schema for top-k hits in `ShedResult` (rec, one
...
- **OD-10-4 — re-index on update:** re-embed a tool's description on re-register (last-wins, F09
  OD-09-5). Rec: re-index.
        If we dont let re-registeration then we dont need to re-embed, see 20 point on not allowing it.

- **OD-10-5 — category source:** `ToolDefinition.category` is implementer-supplied; default `"general"`.
  Confirm.
...
- coordinates F01 (`ToolShed` no-`others`/optionality), F09 (registry + register hook), F28
  (VectorStore), F31 (EmbeddingProvider), F32 (the default `search`/`search_file` tools)
...
- OD-10-1..5 resolved (OD-10-1 + OD-10-3 flagged); fundamentals authored.
