...
feature_directory: "specifications/36-agentic-api/features/30-error-handling"
this_file: "specifications/36-agentic-api/features/30-error-handling/start.md"
created: 2026-06-14
---
...
3. Read `../../LEARNINGS.md`. Resolve OD-30-1..5. **OD-30-1 (flatten non-Clone sources) + OD-30-2
   (detect overflow/rate-limit) depart from Decision 16's `#[from]` — surface for a user ruling.**
   `AgenticError` MUST be `Clone+PartialEq+Debug` (F02).
...
7. Report; verify (native+wasm); update `../../LEARNINGS.md`; move to Feature 31.

---

**Workflow:** feature.md → Decision 16 → real GenerationError + is_context_overflow → Resolve OD-30 (flag 1+2) → Compact → ONE ITEM → fundamentals → Report → Verify → 31
