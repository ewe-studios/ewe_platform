---
feature: "Error Handling — AgenticError (Clone+PartialEq+Debug), Next(Err) propagation, circuit breaker"
description: "The unified AgenticError enum (MUST be Clone + PartialEq + Debug so the F02 stream keeps its derives), error propagation via Stream::Next(Err), retry/resilience owned by model+tool tasks, circuit-breaker model fallback, and resolution of the missing GenerationError::ContextOverflow/RateLimit via Messages::is_context_overflow + detection"
status: "pending"
priority: "high"
depends_on: ["01-message-model", "02-agent-stream-contract", "24-model-provider-router"]
...
# Feature 30: Error Handling

> **Review status (2026-06-14) — self-review against code (subagent unavailable):**
> 1. **`AgenticError` MUST be `Clone + PartialEq + Debug`** — F02's stream is
>    `D = Result<SessionRecord, AgenticError>` (`02-agent-stream-contract/feature.md:55`) and the
>    `Stream`/`TaskStatus` machinery + tests rely on those derives (F02 review: "`AgenticError` trait
>    bounds pinned"). This is the HARD constraint that reshapes Decision 16's design.
...
>    `#[from] GenerationError` into a `Clone+PartialEq` enum. **OD-30-1 (load-bearing):** `AgenticError`
...
>    overflow patterns; F30 classifies into its own `GenKind::{ContextOverflow, RateLimit, Other}` at the
>    boundary. (OD-30-2.)
> 4. **Errors propagate as `Stream::Next(Err(AgenticError))`** — NO new `Stream` variant
>    (`Stream<D,P>` has none, verified F02). The agent loop (F27) yields `TaskStatus::Ready(Err(e))`
>    which becomes `Stream::Next(Err(e))` (the `task.rs` mapping). Decision 16 §Error Propagation.
> 5. **Retry/resilience is owned by model + tool tasks, NOT the loop** (Decision 16 §Error Ownership):
>    model task retries rate-limit/network internally (backoff); tool retry is F23's `ToolRetryConfig`
>    (non-blocking `Delayed`, F23 OD-23-4). F30 owns the **taxonomy + classification + circuit-breaker
>    decision**, not the per-call retry loops.
> 6. **Circuit breaker switches model via F24** (Decision 16 §Circuit Breaker): after
>    `circuit_breaker_threshold` consecutive generation failures, pick the next `fallback_models` entry
>    through the router (F24). The `handle_error` → `AgentAction{ Continue / RetryWithReducedContext /
>    SwitchModel / Terminate }` dispatcher (Decision 16 line 170) lives here; F27 executes the action.
>    `AgentAction` carries no non-`Clone` payload (it references model ids / the error).
> 7. **Tool errors do NOT become `Next(Err)`** — they are `Messages::ToolResult{ error_detail }` back to
>    the LLM (Decision 16 §Error Surfacing; already handled in F23). Only generation/auth/loop/unexpected
>    surface as `Next(Err(AgenticError))`. The taxonomy still has an `AgenticError::ToolCall` variant for
...
> the `AgenticError` taxonomy (`Clone+PartialEq+Debug`, flattening non-`Clone` sources at the boundary),
> `Stream::Next(Err)` propagation, the missing-overflow/rate-limit resolution via detection, the
...
as `Next(Err(AgenticError))`, which forces `AgenticError: Clone + PartialEq + Debug` (F02). But the real
...
#[derive(Debug, Clone, PartialEq)]          // the F02 stream constraint
pub enum AgenticError {
    ...
    MessageStore(String),                   // flatten StorageError if non-Clone (OD-30-1)
    Memory(String),
    VectorStore(String),
    Embedding(String),
    Session(String),
    Queue(String),
    LoopDetected(LoopDetection),            // F28 — Clone+PartialEq (ensured there)
    Auth(AuthError),                        // F29 — Clone+PartialEq (ensured there)
    Budget { limit: u64 },
    Unexpected(String),
}
...
impl AgenticError {
    /// Boundary constructor: flatten a live GenerationError + classify overflow/rate-limit (OD-30-2).
...
impl CircuitBreaker {
    /// After `threshold` consecutive generation failures, return the next fallback model (via F24).
...
graph TD
    ...
    POL -->|SwitchModel| CB[CircuitBreaker -> F24 fallback model]
    POL -->|Continue| LOOP[keep going - tool error is a ToolResult]
    POL -->|Terminate| END[Stream::Next Err -> session ends]
```
...
variants); error propagation through valtron streams (`Next(Err)`, no new variant); ownership of
...
1. `AgenticError` (`Clone+PartialEq+Debug`) + `GenerationFailure`/`GenKind`.
2. Audit each wrapped error (`StorageError`/`VectorStoreError`/`EmbeddingError`/`ToolCallingError`); for
   any non-`Clone`/non-`PartialEq`, flatten to `String` (OD-30-1).
3. `from_generation` boundary constructor: classify via `Messages::is_context_overflow(context_window)`
   + rate-limit detection (OD-30-2).
4. `ErrorPolicy::classify` → `AgentAction` (Decision 16 dispatcher).
5. `CircuitBreaker` (threshold, fallback iteration via F24).
6. Ensure `LoopDetection` (F28) + `AuthError` (F29) are `Clone+PartialEq` so they embed.
7. Tests: `AgenticError` derives `Clone+PartialEq` (compile-asserted); overflow classified via
   `is_context_overflow`; rate-limit classified; non-`Clone` source flattened; `classify` returns the
   right `AgentAction` per variant; circuit breaker switches then exhausts → Terminate; error rides
   `Stream::Next(Err)`; wasm build.

## Open Decisions

- **OD-30-1 — flatten vs derive (load-bearing):** flatten non-`Clone` source errors to `String` at the
...
- **OD-30-2 — overflow/rate-limit (load-bearing):** detect via `is_context_overflow` + rate-limit string
...
- **OD-30-3 — context reduction:** on `ContextOverflow`, how to trim (drop oldest recall first, F18
  OD-18-2). Rec: reuse F18's budget-packing drop order.
- **OD-30-4 — retry ownership:** model/tool tasks retry; loop does not (Decision 16). Confirm no
  agent-level retry loop.
- **OD-30-5 — Unexpected catch-all:** `Unexpected(String)` for `ErrorTrace<T>` and anything unmapped
...
- coordinates the real `GenerationError`/`Messages::is_context_overflow` (`errors/`, `types/`), F02
  (stream `D` type), F24 (fallback models), F23 (tool errors), F28 (`LoopDetected`), F29 (`AuthError`),
  F27 (executes `AgentAction`)
...
- `AgenticError` is `Clone+PartialEq+Debug` (F02 stream stays derivable) with non-`Clone` sources
  flattened at the boundary; context-overflow + rate-limit resolved by detection
  (`is_context_overflow` + string match) rather than absent `GenerationError` variants; errors propagate
  via `Stream::Next(Err)` (no new variant); `ErrorPolicy::classify`→`AgentAction` + circuit-breaker
  fallback via F24; retry stays in model/tool tasks; builds native + wasm.
- OD-30-1..5 resolved (OD-30-1 + OD-30-2 flagged for the user); fundamentals authored.
