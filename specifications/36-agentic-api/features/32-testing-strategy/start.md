...
feature_directory: "specifications/36-agentic-api/features/32-testing-strategy"
this_file: "specifications/36-agentic-api/features/32-testing-strategy/start.md"
created: 2026-06-14
---
...
3. Read `../../LEARNINGS.md`. Resolve OD-32-1..5. **OD-32-1 (interaction matchers) + OD-32-2 (mock
   implements F24 RoutableProvider, not the assoc-type ModelProvider)** — surface for confirmation.
...
**Workflow:** feature.md → Decision 17 → real ModelInteraction + TestHarness + initialize_pool → Resolve OD-32 (flag 1+2) → Compact → ONE ITEM → fundamentals → Report → Verify → DONE
