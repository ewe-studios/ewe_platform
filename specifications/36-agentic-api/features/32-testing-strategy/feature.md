...
status: "pending"
priority: "high"
depends_on: ["24-model-provider-router", "20-toolimpl-registry", "27-agentic-loop", "31-agent-session-api"]
...
>    correction; requirements §F19 task "driven by `ModelInteraction` (not regex)"). Decision 17's
...
>    **`Fn(&ModelInteraction) -> bool`** matcher, returning a scripted `Vec<Messages>`. (OD-32-1.)
> 2. **Implementing `ModelProvider` for a mock is HARD** (same object-safety wall as F24): `ModelProvider`
...
>    `:1234`) and `MockModelProvider` a real `ModelProvider` — OR, simpler, the mock implements **F24's
>    object-safe `RoutableProvider`** directly and is fed to `ProviderRouter` (bypassing the assoc-type
>    trait). **Rec: implement `RoutableProvider`** — F27/F31 take a `ProviderRouter` anyway, so the mock
>    never needs the full `ModelProvider`. (OD-32-2, load-bearing.)
...
>    args)`, `mock_assistant_with_usage(..)`) so tests stay readable and `UsageReport` flows into F03's
>    ledger (so budget/memory-trigger tests work). (OD-32-3.)
...
>    parallel test infra.)
> 6. **Mock tools implement F20's `ToolImpl`** (sync `execute`, F20 OD-20-1): `MockTool { name,
>    behavior: ToolBehavior }` with `Returns / Fails / FailsThenSucceeds` (Decision 17 lines 116-128) —
>    drives F23 retry/DAG tests. `FailsThenSucceeds` validates F23's non-blocking `Delayed` backoff.
> 7. **wasm tests** use in-memory stores (no fjall, F05/F07), `MockModelProvider` (no native model load),
>    and the CF/Turso path where relevant (F06/F14). The whole agentic surface must
...
### `MockModelProvider` — `ModelInteraction`-driven (OD-32-1/2)
...
/// Object-safe — fed straight to ProviderRouter (bypasses the assoc-type ModelProvider). OD-32-2.
impl RoutableProvider for MockModelProvider {           // F24's trait
...
### Message builder helpers (OD-32-3)
...
### Mock tools (OD-32-6) — `ToolImpl` (F20)
...
pub enum ToolBehavior {
    ...
    Fails(ToolError),
    FailsThenSucceeds { failures: u32, error: ToolError, result: ToolCallResult },  // F23 retry tests
}
...
fn test_agent_turn() {
    ...
    let router = ProviderRouter::from_routable(Box::new(mock));   // F24
    // build session (F31), run_turn, assert SessionRecord stream
}
```
...
graph TD
    ...
    MMP -->|RoutableProvider| RT[ProviderRouter F24]
    RT --> SESS[AgentSession F31]
    SESS --> LOOP[AgentLoop F27]
    T --> MT[MockTool ToolImpl behaviors]
    MT --> TCM[ToolCallManager F20/F23]
...
1. `MockModelProvider` implementing **F24 `RoutableProvider`** (OD-32-2), `ModelInteraction`-matcher
   scripting + failures + call-count (OD-32-1).
...
8. Tests (the mock substrate is itself tested): mock returns scripted reply on interaction match;
   ...
   `FailsThenSucceeds` drives F23 retry; `UsageReport` from a mock reply increments F03's ledger;
   builds + runs on wasm.

## Open Decisions

- **OD-32-1 — match on ModelInteraction, not regex (load-bearing):** `Fn(&ModelInteraction) -> bool`
  matchers (rec, user requirement) replacing Decision 17's `Regex`. Confirm the matcher closure shape.
- **OD-32-2 — mock implements RoutableProvider (load-bearing):** implement F24's object-safe trait (rec)
  vs the full `ModelProvider` (assoc types, hard). Rec: `RoutableProvider` — F27/F31 take a router. Flag.
- **OD-32-3 — message builders:** provide `mock_text`/`mock_tool_call`/`mock_text_usage` (rec) — raw
  `Messages::Assistant` is too verbose and `UsageReport` must flow for ledger tests.
- **OD-32-4 — mock location:** `foundation_ai::agentic::testing` (behind a `testing`/`dev` feature) vs
...
- **OD-32-5 — wasm test runner:** `wasm-bindgen-test` for the deterministic tier vs build-only. Rec: at
...
- coordinates F24 (`RoutableProvider`/`ProviderRouter`), F20 (`ToolImpl`), F03 (`UsageReport`→ledger),
  F27/F31 (the systems under test), `foundation_testing::huggingface::TestHarness`, valtron
  `initialize_pool`
...
- `MockModelProvider` is driven by `ModelInteraction` matchers (not regex), implements F24's object-safe
  `RoutableProvider`, scripts replies + failures + nth-call, and carries real `UsageReport`; `MockTool`
  (`Returns/Fails/FailsThenSucceeds`) drives F23 retry/DAG tests; the four tiers + the exact valtron pool
...
- OD-32-1..5 resolved (OD-32-1 + OD-32-2 flagged); fundamentals authored.
