...
status: "pending"
priority: "low"
depends_on: ["12-vectorstore-trait-inmemory"]
...
>    `#[async_trait(?Send)]` signature**, or F14 owns defining it.
> 2. **External vector DBs — RESOLVED (user, 2026-06-15):** they ARE supported, **behind the existing
>    `VectorStore`/`AsyncVectorStore` trait** (F12) — Decision 07's blanket rejection is reversed (a
...
>    OD-14-1 resolves to **"absent — greenfield `bindgen/cf/vectorize.rs`"**; size accordingly.
> 4. **CF D1/KV inherit F06's hard limits**: KV `list` can't paginate past 1000 yet (binding fix is a
...
>    TurboPuffer *namespaces* (partitions). Map F12's `namespace: String` per provider; define index/
>    collection provisioning + who creates/deletes (OD-14-7). Dimension is server-side/async-fail for
>    external providers (deviates from F12's insert-time check).
> 6. **Commit the 14a (CF, wasm) / 14b (external HTTP, native) split** (OD-14-2). Client-side flat
>    search over fetched D1/KV vectors needs a **hard cap + truncation policy** (OD-14-8). Credentials
>    via `SessionAccessProvider` makes **F16 a dependency** (declare it) (OD-14-9).
...
> the F12 **`AsyncVectorStore`** trait. Completes the VectorStore backend set (in-memory F12, native
> F13, CF+external F14). Lowest priority — the in-memory + native backends cover the core use cases;
...
All implement **`AsyncVectorStore`** (`#[async_trait(?Send)]`, F12) — these are network/Promise-based.
...
- **D1 / KV fallback:** store vectors as BLOB (D1) / JSON (KV); **fetch namespace-scoped set +
  ...
  eventual consistency — inherits F06's caveats). Best-effort for large sets.

### External managed (native HTTP) — scoped per user ruling (2026-06-15)

The `VectorStore`/`AsyncVectorStore` trait (F12) is the well-structured seam; external providers are
...
- Namespace = the provider's native namespace concept; dimension = provider index config (server-side,
  async-fail — deviates from F12's local insert-time check). Native-only (HTTP transport; 00c). API
  keys via config / `SessionAccessProvider` (F16), never committed.
...
```mermaid
graph TD
    A[AsyncVectorStore F12] --> VEC[CF Vectorize: native ANN]
...
2. CF D1/KV `AsyncVectorStore` (fetch + flat fallback) — reuse F06 D1/KV bindings + caveats.
...
- **OD-14-1 — CF Vectorize availability:** is the binding in our worker stack? If yes, prefer it over
  D1/KV fetch-then-search. Research.
- **OD-14-2 — external scope:** **Resolved (user, 2026-06-15) → TurboPuffer REQUIRED** (native REST
...
- **OD-14-3 — D1/KV best-effort:** explicitly document KV's eventual-consistency + 1000-cap mean
  KV-vector is best-effort; D1 fetch is bounded by row count. (Inherits F06.)
- **OD-14-4 — async transport on wasm:** external HTTP providers are native-only (wasm has no HTTP —
  00c). CF backends are wasm. State the split.
- **OD-14-5 — credentials:** provider API keys via config/`SessionAccessProvider` (F16), never committed.
...
- Best-effort caveats documented; fundamentals authored. OD-14-1..5 resolved (incl. 14a/14b split).
