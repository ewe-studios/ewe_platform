...
- **Append (single writer + batch):** `DocumentStore::append` is `&self`, but a collection has **exactly
  ...
  index inserts) on size/time threshold or explicit flush. doc_id is the authoritative scru128 (F06
  Part 0), so the fjall key order **is** chronological. (OD-22-6.)
...
- **OD-22-2 — delete strategy:** tombstone-in-index + periodic file compaction vs immediate rewrite.
  Rec: tombstone + compaction at a threshold (append-only friendly).
      Sounds good

- **OD-22-3 — index value contents:** offset+len only, or also `record_type`/`title` for index-only
...
- **OD-22-5 — which `VfsFileSystem` impl:** `native_fs` (real disk) + the **real fjall-backed** VFS
...
- **OD-22-6 — append serialization:** **Resolved (user, 2026-06-15) → single writer + batch**, NOT a
...
- **OD-22-7 — collection-key → filename encoding:** keys like `session:{id}:messages` contain `:`.
  Encode via the already-pulled `hex` (or base32/percent). Rec: hex.
        Sure, if it does not add issues, i am good

- **OD-22-8 — `scan_from` materialization:** eager-materialize (matches all existing backends:
...
- **OD-22-9 — tail-rebuild high-water-mark:** persist a `__hwm` key (last indexed offset) vs derive
