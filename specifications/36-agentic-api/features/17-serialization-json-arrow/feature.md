...
# Feature 17: Serialization — JSON + Arrow
...
DocumentStore F04, `jq`/`fff`-friendly) and **Arrow** (columnar, for batch export, analytics — token
...
file format (F04). No new work beyond confirming round-trip + the F04 promoted-column extraction.
...
- The promoted columns mirror F04's promoted DocumentStore columns (consistency) — same `record_type`/
...
```mermaid
graph LR
    R[SessionRecord] -->|serde| J[JSON / NDJSON → DocumentStore F04]
...
1. Confirm `SessionRecord` JSON round-trip (F01) + NDJSON for F04.
2. `ArrowSerializable` + the promoted-column schema (mirror F04 columns).
...
- coordinates F01 (records), F04 (column parity), F16 (export)
...
- Analytics (token sums, type filter) work columnar. OD-17-1..4 resolved; fundamentals authored.
