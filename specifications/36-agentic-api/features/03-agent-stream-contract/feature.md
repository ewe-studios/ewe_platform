...
> *produces* this stream is F19. Defines what every caller of `run_turn_stream` observes.
...
- **Errors** flow as a **record, not a `Result`** (user, 2026-06-15). Rather than wrap `D` in
  ...
  `SessionRecord::FailedAction`; F08 skips it on flush. (Supersedes the earlier CRIT-05 `Result`-in-`D`
  resolution; F01 adds the variant, F02 defines `AgenticError`.)
...
- **`Stream::Next(SessionRecord)`** — the rich record: `Conversation { message: Messages }` for
  ...
  layer produces.
- **`Stream::Next(SessionRecord::FailedAction)`** — an error record (the `AgenticError` kind + `trace: StructuredErrorTrace`, errstack's serializable chain); NOT a `Result`, NOT persisted. (`AgenticError` is F02; see F01 OD-1-FA.)
...
> **OD-03-1 — RESOLVED (user):** `D = SessionRecord` (pure — conversation, memory, AND `FailedAction`
...
### Mapping to valtron `TaskStatus` — **valtron does this; F03 doesn't**

> **RESOLVED (user, 2026-06-15):** the `TaskStatus → Stream` conversion is **fully abstracted by
> valtron** (`From<TaskStatus> for Stream`) — F03 does **not** map it. We only declare the loop's
...
So all F03 must state: the agent loop (F19) is a `TaskIterator` with
...
> **`AgenticError` trait-bound contract (pin now, F02 owns the type):** `Stream<D,P>`/`TaskStatus`
...
> `Clone + PartialEq + Debug`** or the whole agent stream loses those derives. F02 must honor this.
...
> **Streaming partials (OD-03-5) — RESOLVED (user, 2026-06-15; CONFIRMED against code).** The model
...
2. Define the loop's associated types: `type Ready = SessionRecord;   // errors are SessionRecord::FailedAction, not a Result
   type Pending = AgentProgress;` (the `AgenticError` shell may be a forward-declared stub until F02).
...
4. Provide a `From<ModelState> for AgentProgress` and a helper lifting model
   `Stream<Messages, ModelState>` items into the agent stream (used by F19).
...
- **OD-03-6:** add `SessionRecord::Summary { message_count, usage }` (to F01) for the session-final
...
- **OD-03-7 (dependency):** `AgenticError` (F02) is a forward-ref; until F02 lands, F03 uses a local
  stub with the pinned bounds (`Clone + PartialEq + Debug`). Track F02 as a soft dependency.
...
- references F01 (`SessionRecord`), F02 (`AgenticError` — stub/forward until then)
