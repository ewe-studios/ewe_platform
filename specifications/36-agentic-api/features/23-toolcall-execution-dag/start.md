...
feature_directory: "specifications/36-agentic-api/features/23-toolcall-execution-dag"
this_file: "specifications/36-agentic-api/features/23-toolcall-execution-dag/start.md"
created: 2026-06-14
---
...
1. Read `feature.md` + Decision 04 + Decision 16. Confirm F01 added `depends_on`/`execution_hint` to
   `ModelOutput::ToolCall` (real enum `types/mod.rs:870` had neither). Read F25 (PriorityQueue/
   CancelCode/Depends) + F16 (`append`).
...
3. Read `../../LEARNINGS.md`. Resolve OD-23-1..5. **OD-23-3 (persist durability) + OD-23-4 (backoff)
...
7. Report; verify (native+wasm); update `../../LEARNINGS.md`; move to Feature 24.

---

**Workflow:** feature.md → Decision 04/16 → F01 fields + F25 + F16 → Resolve OD-23 (flag 3+4) → Compact → ONE ITEM → fundamentals → Report → Verify → 24
