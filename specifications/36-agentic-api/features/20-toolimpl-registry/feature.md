...
status: "pending"
priority: "high"
depends_on: ["01-message-model", "16-message-api", "12-vectorstore-trait-inmemory"]
...
>    :1140, `Args` :633, `ArgType` :590, `ToolChoice` :1023). F20 does NOT redefine them. The
...
>    Decision 15 line 71) would not compile. **OD-20-1 (load-bearing):** make `execute` either (a) a
...
>    a `SessionRecord::Conversation { message: Messages::ToolResult { .. } }` for F16 (F23 owns the
>    persist-before-deliver; F20 owns the *shape*).
> 6. **VectorStore registration is for the `shed` meta-tool (F21), not F20's core.** Decision 15 line 81
>    inserts the tool description into the vector store on `register`. The *insertion* belongs here (so
>    every registered tool is discoverable), but the **`shed` tool + ShedQuery/ShedResult are F21**. F20
>    exposes a `register`-time hook; F21 implements the search tool. Embedding the description requires
>    F15 (EmbeddingProvider) — added as a soft dep via F21, not a hard F20 dep (registration can defer
>    indexing). `register` mutation: Decision 15 uses `&mut self`; behind `Arc` that fails — use **`&self`
>    + interior mutability** (`RwLock<HashMap>`), matching Decision 08. (OD-20-3.)
> 7. **`build_toolshed` must drop `others`.** The real `ToolShed` (:1076) still has
>    `others: Option<Vec<Tool>>`; F01 removes it. F20's `build_toolshed` populates the post-F01 struct
...
>    needs *some* `Tool` there or F01 must make them `Option` — **flag: F01/F21 must reconcile the
>    ToolShed field optionality** with "zero tools registered" (Decision 15 §Always Present). (OD-20-4.)

> Implements Decision 15 (the `ToolImpl` → registry → schema pipeline, minus the `shed` meta-tool
> which is F21). Owns the **tool contract** every tool implements and the **registry** that holds them,
...
### The `ToolImpl` contract (object-safe, sync — OD-20-1)
...
pub trait ToolImpl: Send + Sync {
    ...
    /// Run the tool with validated arguments. SYNC + object-safe (valtron owns concurrency, F23).
...
pub struct ToolDefinition {
    ...
    pub category: String,          // for shed discovery (F21)
}
...
`ToolError` is `Clone + PartialEq + Debug` so F30's `AgenticError` (which must stay `Clone+PartialEq`
for the F02 stream derives) can embed it.
...
struct ToolCallManagerInner {
    tools: RwLock<HashMap<String, Arc<dyn ToolImpl>>>,  // &self register (OD-20-3)
    session_id: SessionId,
    // F23 adds: execution_queue, cancellation, message_api (persist), retry config.
    // F21 adds:  vector_store + embedder for shed discovery indexing.
}
...
impl ToolCallManager {
    ...
    /// (Pure execution; F23 owns scheduling/persistence/retry — this is the inner call.)
...
ToolImpl::definition() -> ToolDefinition { name, description, arguments: Args, category }
    ...
    ▼
ToolCallManager::build_toolshed() -> ToolShed { shed(F21), read, edit, write, search, bash, .. }   // NO others
    ▼
...
declare them) — F23 reads them back off `ModelOutput::ToolCall`.
...
graph TD
    ...
    REG -.register hook.-> IDX[(VectorStore index for shed F21)]
```
...
1. `ToolImpl` trait (sync `execute`, OD-20-1) + `ToolDefinition`/`ToolCallResult`/`ToolError`
...
6. `register`-time discovery hook (description → vector index) — interface only; F21 fills it.
...
- coordinates F01 (`ModelOutput::ToolCall` DAG fields, `ToolShed` no-`others`), existing
  `types/mod.rs` (`Tool`/`Args`/`ToolFormatter`), F16 (persist results), F21 (shed tool), F23
  (execution DAG)
...
- OD-20-1..5 resolved (OD-20-1 + OD-20-4 flagged for the user); fundamentals authored.
