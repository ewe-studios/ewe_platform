...
- [ ] `DigitalOceanClient::from_env()` declares both vendor spellings (`DIGITALOCEAN_TOKEN`, `DIGITALOCEAN_ACCESS_TOKEN`); resolution is `EWE_*` for each, then vendor-native for each, and it **errors naming every name it looked for**
