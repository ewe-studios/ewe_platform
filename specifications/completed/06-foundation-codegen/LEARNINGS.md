...
- **Generated code must include imports**: When generating Rust source files that call functions from another module, always include a `use` statement at the top. The generated `main.rs` should import the entrypoint function (`use crate_name::module::function;`) then call it by name in `main()`, rather than calling the fully qualified path directly.
