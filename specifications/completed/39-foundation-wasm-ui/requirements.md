...
| [10-build-pipeline](features/10-build-pipeline/) | wasm_bin/worker/service macros + WasmBundleGenerator + ewe-wasm CLI | 3 | **COMPLETE** (2026-06-12) |
| [11-request-batching](features/11-request-batching/) | probe + id'd /primal/messages batches, WS/Worker coalescing, batch-response routing | 3 | **COMPLETE** (2026-06-12) |
...
- [x] foundation_wasm compiles with DOM concepts removed, no_std preserved
- [x] foundation_wasm_ui compiles, depends on foundation_wasm without circular deps
- [x] foundation-wasm.js rewritten — clean standard communication only
- [x] foundation-wasm-ui.js created — all DOM operations, animations, web components
- [x] Signal<T> supports Rust-side reactivity with fine-grained subscriptions
- [x] html! macro compiles to template with Parts for dynamic expressions
- [x] Arrow batch encoding sends DOM mutations in single host_batch_apply call
- [x] JS runtime parses Arrow batches and applies to real DOM (no vDOM)
- [x] Web component base class supports shadow DOM, lifecycle, attribute observation
- [x] Bidirectional signal bridge: Rust signal change -> DOM update, DOM event -> Rust handler
- [x] foundation_auth_ui login form connects to foundation_auth server endpoints
- [x] Headless UI components pass accessibility tests (keyboard nav, ARIA, focus management)
- [x] Server SSE stream delivers DOM patches that WASM applies incrementally
- [x] Template caching by identity — same template string parsed only once
