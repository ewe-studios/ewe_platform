...
- [x] `Serve` trait enables synchronous request handling with `ConnectionResult` (Keep/Take/Close)
- [x] `ContextBag` provides type-erased, thread-safe dependency storage
- [x] Router is simplified to `Router<'a>` with no generic Request/Response types — just `dispatch(method, path) -> Option<ArcServe>`
- [x] Zero `tokio`/`tower`/`axum`/`async-trait`/`http` crate dependencies in `foundation_http`
- [x] `HttpApp` is not generic — `HttpApp` with `route::<H: Serve>(method, path)`
- [x] `HttpServer` accepts connections on `std::net::TcpListener`, dispatches via `BackgroundJobRunner`
- [x] Keep-alive connections work correctly across multiple requests
- [x] WebSocket upgrade returns `ConnectionResult::Take` and handler owns the stream
- [x] SSE streaming works with proper headers and `EventSourceWriter`
- [x] Middleware chain executes synchronously in registration order
- [x] Graceful shutdown drains active connections and stops accepting new ones
- [x] TLS support via rustls and openssl feature flags
