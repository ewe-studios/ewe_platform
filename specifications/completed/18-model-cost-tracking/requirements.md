...
- [x] Create `backends/foundation_ai/src/costing.rs` with `CostStatus`, `CostResult`, and `calculate_cost()` function
- [x] Add `total` and `status` fields to `UsageCosting` in `types/mod.rs`
- [x] Export costing module from `lib.rs`
- [x] Write unit tests for `calculate_cost()` with known token/pricing inputs

### Feature 02: Token Estimation

- [x] Add `estimate_tokens(messages: &[Message]) -> UsageReport` to costing module
- [x] Add `estimate_cost(&self, interaction: &ModelInteraction)` to `ModelProvider` trait
- [x] Implement default trait method in trait definition
- [x] Write unit tests for token estimation with text-only and multipart messages

### Feature 03: Provider Cost Tracking

- [x] Add `get_cost(&self) -> CostResult` to `ModelProvider` trait
- [x] Update `anthropic_messages_provider.rs` — populate cost on UsageReport, implement trait methods
- [x] Update `openai_provider.rs` — populate cost on UsageReport, implement trait methods
- [x] Update `openai_responses_provider.rs` — populate cost on UsageReport, implement trait methods
- [x] Update `llamacpp.rs` — implement estimation-based cost (zero USD)
- [x] Update `candle.rs` — implement estimation-based cost (zero USD)
- [x] Update `huggingface_gguf_provider.rs` — delegates to llamacpp (cost handled by underlying backend)
- [x] Update `huggingface_candle_provider.rs` — delegates to candle (cost handled by underlying backend)
- [x] Write integration tests verifying cost flows through at least one provider

### Feature 04: Provider Prompt Assembly

- [x] Update `anthropic_messages_provider.rs` — system prompt + soul as combined system, tool_shed tools as tools array, messages mapped
- [x] Update `openai_provider.rs` — system prompt message, soul as second system message, tool_shed tools as function definitions, messages mapped
- [x] Update `openai_responses_provider.rs` — system prompt + soul as combined instructions, tool_shed tools, messages mapped
- [x] Update `llamacpp.rs` — system prompt + soul as system message in chat template, tool_shed instructions injected, messages through chat template
- [x] Update `candle.rs` — system prompt, soul, tool_shed instructions as `System:` prefixes, messages as `User:/Assistant:` lines
- [x] Remove all references to `interaction.tools` (replaced by `tools_shed` flattening)
- [x] Verify compilation across all providers after changes

### Feature 05: Tool Schema Builder

- [x] Replace `Tool.arguments: Option<Vec<Args>>` with `Tool.arguments: Option<Args>` (single struct)
- [x] `Args` struct holds `schema: serde_json::Value` + `validator: ValidationOptions`
- [x] Manual `Clone`, `PartialEq`, `Serialize`, `Deserialize` on `Args`
- [x] Providers use `args.schema` directly for `input_schema`/`parameters` — no per-provider ArgType mapping
- [x] llama.cpp/Candle extract property names from `args.schema["properties"]` for text-based tool listing
