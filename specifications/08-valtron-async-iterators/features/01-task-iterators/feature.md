...
```rust
// Current synchronous pattern - BLOCKS
let models: Vec<ModelEntry> = fetch_models_dev(&client);  // Blocks until complete

// Desired async builder pattern - chain operations, then execute
...
let stream = execute_stream(task)?;  // Execute and consume results
```

## WHAT: Solution Overview

### TaskStatusIterator Trait (Conversion to Stream)

```rust
/// Conversion trait: Turn any TaskIterator into a StreamIterator via execute_stream()
///
/// This is the boundary method that hands off task execution to Valtron.
pub trait TaskStatusIterator: Iterator<Item = TaskStatus<Self::Ready, Self::Pending, Self::Spawner>> {
    type Ready;
    type Pending;
    type Spawner: ExecutionAction;

    /// Convert into StreamIterator via execute_stream() for execution engine integration
    ///
    /// This uses the existing execute_stream() function from unified.rs,
    /// which handles platform detection and returns DrivenStreamIterator.
    fn into_stream(self) -> GenericResult<DrivenStreamIterator<Self>>
    where
...
///
/// // Then execute
/// let stream = execute_stream(task)?;
/// ```
...
where
    ...
    I::Ready: Clone,
{
    type Item = TaskStatus<Vec<I::Ready>, StreamCollectStatus<I::Pending>, I::Spawner>;
...
1. Define `TaskStatusIterator` trait for conversion to streams (uses `execute_stream()`)
...
1. **TaskStatusIterator trait** - Core trait with `into_stream()` method using `execute_stream()`
...
6. **Integration test** - Chain multiple combinators, execute via `execute_stream()`

## Tasks

1. [ ] Define `TaskStatusIterator` trait with `into_stream()` method
...
7. [ ] Implement `StreamCollect<I>` combinator - yields StreamCollectStatus while gathering (stream_collect method)
8. [ ] Write unit tests for each combinator type
9. [ ] Write integration test chaining multiple combinators
...
- Integration test demonstrates chaining multiple combinators before execution
- Zero clippy warnings

---

_Created: 2026-03-20_
_Updated: 2026-03-20 (Builder-style TaskIteratorExt with forward/transform pattern)_
