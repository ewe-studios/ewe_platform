...
pub struct PlatformAssetManager {
    ...
    /// `RwLock` because `activate()` swaps in a rebuilt overlay.
    fs: RwLock<DynFs>,
    /// Rebuilds the VFS backend for a different delta root. Captured at
    /// `initialize()` so the manager stays non-generic over `R: Runtime`
    /// (PlatformSession is non-generic and must own one).
    rebuild: VfsFactory,
...
impl PlatformAssetManager {
    ...
    /// 2. Validate manifest_domain + base_url against the baked domain
    /// 3. Anti-replay: check sequence > last accepted
    /// 4. Validate app_id / file paths (no `..`, `\`, NUL, absolute)
    /// 5. Enforce file size / count limits
    /// 6. Return the download plan; rollback_to / delete_after ride along
    ...
    /// This is the constructor the test suites use — `MemoryFs` or a
    /// `NativeFs` over a temp dir gives full version-management coverage
    /// on the host, with no Android device and no Tauri `App`.
    pub fn from_vfs(fs: DynFs, rebuild: VfsFactory, base_root: PathBuf,
        bundle_version: &str, layout: AssetLayout,
