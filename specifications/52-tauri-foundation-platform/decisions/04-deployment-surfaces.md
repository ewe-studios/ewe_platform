...
The `#[platform_bin]` source parser runs at build time and writes generated
code directly into `src/generated/`. For each `#[wasm_app]` function it finds,
it produces a module that uses `PackageDirectorate` to locate the compiled
`.wasm` bytes.
...
User's crate/
  ...
  │   ├── generated/
  │   │   ├── mod.rs           ← auto-generated: pub mod business_logic;
  │   │   └── business_logic.rs← generated wasmtime wrapper (see below)
  │   └── app/
  │       ├── mod.rs
  ...
  └── webview_assets/          ← #[wasm_bin] / worker / service bundles
      └── ... (generated by WasmBundleGenerator)
```

The crate's main module tree includes it with `mod generated;`. The source
parser overwrites these files on each build. They're checked into git so CI
and other developers don't need to run the parser (but `#[platform_bin]` also
regenerates them as a build step to keep them in sync).

**Generated `src/generated/business_logic.rs`:**

```rust
// Auto-generated by #[platform_bin] source parser.
...
pub fn builder() -> WasmtimeBuilder {
    // PackageDirectorate locates "shell_wasm/business_logic.wasm"
    // In release: bytes embedded via rust-embed / EmbeddableDirectory
    // In debug: reads from shell_wasm/ directory on disk
    ...
    // Session-standard imports (platform, session handle) are wired by
    // the session when .build(&session) is called. User adds custom
    // imports via .with_import(...) before calling .build().
}

// Type alias for the embedded asset directory — generated per crate.
// The embed macro (rust-embed or equivalent) bakes shell_wasm/ into the binary.
#[derive(Default)]
...
**What `WasmtimeBuilder` does:**

```rust
// foundation_wasmtime::WasmtimeBuilder (conceptual)
impl WasmtimeBuilder {
    /// Create a builder with WASM bytes loaded (via PackageDirectorate).
    ...
    /// The session wires its standard imports here.
...
|---|---|
| **Debug** | `PackageDirectorate` reads from the filesystem: `shell_wasm/business_logic.wasm` in the crate directory. No recompile needed after WASM changes — just rebuild the `.wasm` and restart. |
| **Release** | `EmbeddableDirectory` bakes the bytes into the binary at compile time. `get_file()` returns them from static memory. No filesystem reads. |

This is the same pattern used by `foundation_packager` for templates and static
assets — `PackageDirectorate` abstracts over embed-vs-filesystem so the
generated code never cares whether it's debug or release.
