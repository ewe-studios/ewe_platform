...
The `#[platform_bin]` source parser produces a `generated/` directory in the
user's crate. For each `#[wasm_app]` function it finds, it generates a module
that uses `PackageDirectorate` to locate the compiled `.wasm` bytes.
...
User's crate/
  ...
  │
  ├── generated/               ← created by #[platform_bin] source parser
...
**Generated `generated/business_logic.rs`:**
