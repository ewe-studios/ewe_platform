...
MON_OPTS="-monitor $MONITOR -qmp unix:${QEMU_DIR}/qmp.sock,server=on,wait=off,nodelay=on -name $PROCESS,process=$PROCESS,debug-threads=on -pidfile $QEMU_PID"
...
configureClipboard() {
  ...
  # Clipboard sharing via qemu-vdagent bridges the noVNC browser client
  # clipboard with the VM guest. Requires SPICE vdagent in the guest:
  #   Windows: SPICE Guest Tools (spice-guest-tools)
  #   Linux:   spice-vdagent package
  #   macOS:   no native agent — VNC clipboard buffer (paste-only)
  disabled "${CLIPBOARD:-N}" && return 0
