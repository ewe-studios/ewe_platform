...
const LLAMA_SPLIT_MODE_NONE: i8 = infrastructure_llama_bindings::LLAMA_SPLIT_MODE_NONE as i8;
#[allow(clippy::cast_possible_wrap)]
#[allow(clippy::cast_possible_truncation)]
const LLAMA_SPLIT_MODE_LAYER: i8 = infrastructure_llama_bindings::LLAMA_SPLIT_MODE_LAYER as i8;
#[allow(clippy::cast_possible_wrap)]
#[allow(clippy::cast_possible_truncation)]
const LLAMA_SPLIT_MODE_ROW: i8 = infrastructure_llama_bindings::LLAMA_SPLIT_MODE_ROW as i8;
...
/// `infrastructure_llama_cpp::max_devices()`.
...
impl LlamaModelParams {
    ...
    pub fn with_devices(mut self, devices: &[usize]) -> Result<Self, LlamaCppError> {
        ...
        for (i, &dev) in devices.iter().enumerate() {
            if dev >= unsafe { infrastructure_llama_bindings::ggml_backend_dev_count() } {
                return Err(LlamaCppError::BackendDeviceNotFound(dev));
            }
            let backend_dev = unsafe { infrastructure_llama_bindings::ggml_backend_dev_get(dev) };
