...
///
/// ```
/// use infrastructure_llama_cpp::mtmd::MtmdInputChunkType;
///
...
/// assert_eq!(text_chunk, infrastructure_llama_bindings::MTMD_INPUT_CHUNK_TYPE_TEXT.into());
...
pub enum MtmdInputChunkType {
    /// Text input chunk
    Text = infrastructure_llama_bindings::MTMD_INPUT_CHUNK_TYPE_TEXT as _,
    /// Image input chunk
    Image = infrastructure_llama_bindings::MTMD_INPUT_CHUNK_TYPE_IMAGE as _,
    /// Audio input chunk
    Audio = infrastructure_llama_bindings::MTMD_INPUT_CHUNK_TYPE_AUDIO as _,
}

impl From<infrastructure_llama_bindings::mtmd_input_chunk_type> for MtmdInputChunkType {
    fn from(chunk_type: infrastructure_llama_bindings::mtmd_input_chunk_type) -> Self {
        match chunk_type {
            infrastructure_llama_bindings::MTMD_INPUT_CHUNK_TYPE_TEXT => MtmdInputChunkType::Text,
            infrastructure_llama_bindings::MTMD_INPUT_CHUNK_TYPE_IMAGE => MtmdInputChunkType::Image,
            infrastructure_llama_bindings::MTMD_INPUT_CHUNK_TYPE_AUDIO => MtmdInputChunkType::Audio,
...
///
/// ```
/// use infrastructure_llama_cpp::mtmd::{MtmdContextParams, mtmd_default_marker};
/// use std::ffi::CString;
///
...
impl Default for MtmdContextParams {
    fn default() -> Self {
        unsafe { infrastructure_llama_bindings::mtmd_context_params_default() }.into()
    }
}

impl From<&MtmdContextParams> for infrastructure_llama_bindings::mtmd_context_params {
    fn from(params: &MtmdContextParams) -> Self {
        let mut context = unsafe { infrastructure_llama_bindings::mtmd_context_params_default() };
...
impl From<infrastructure_llama_bindings::mtmd_context_params> for MtmdContextParams {
    fn from(params: infrastructure_llama_bindings::mtmd_context_params) -> Self {
        Self {
...
///
/// ```
/// use infrastructure_llama_cpp::mtmd::MtmdInputText;
///
...
impl MtmdContext {
    ...
    pub fn init_from_file(
        ...
        let ctx_params = infrastructure_llama_bindings::mtmd_context_params::from(params);

        let context = unsafe {
            infrastructure_llama_bindings::mtmd_init_from_file(
                path_cstr.as_ptr(),
    ...
    pub fn decode_use_mrope(&self) -> bool {
        unsafe { infrastructure_llama_bindings::mtmd_decode_use_mrope(self.context.as_ptr()) }
    }

    /// Check whether the current model supports vision input.
    #[must_use]
    pub fn support_vision(&self) -> bool {
        unsafe { infrastructure_llama_bindings::mtmd_support_vision(self.context.as_ptr()) }
    }

    /// Check whether the current model supports audio input.
    #[must_use]
    pub fn support_audio(&self) -> bool {
        unsafe { infrastructure_llama_bindings::mtmd_support_audio(self.context.as_ptr()) }
    }
    ...
    pub fn tokenize(
        ...
        let input_text = infrastructure_llama_bindings::mtmd_input_text {
        ...
        let bitmap_ptrs: Vec<*const infrastructure_llama_bindings::mtmd_bitmap> = bitmaps
            .iter()
            .map(|b| b.bitmap.as_ptr().cast_const())
            .collect();

        let result = unsafe {
            infrastructure_llama_bindings::mtmd_tokenize(
                self.context.as_ptr(),
...
impl Drop for MtmdContext {
    fn drop(&mut self) {
        unsafe { infrastructure_llama_bindings::mtmd_free(self.context.as_ptr()) }
    }
}
...
impl MtmdBitmap {
    ...
    ///
    /// ```
    /// use infrastructure_llama_cpp::mtmd::MtmdBitmap;
    ///
    ...
    pub fn from_file(ctx: &MtmdContext, path: &str) -> Result<Self, MtmdBitmapError> {
        ...
        let bitmap = unsafe {
            infrastructure_llama_bindings::mtmd_helper_bitmap_init_from_file(
                ctx.context.as_ptr(),
                path_cstr.as_ptr(),
            )
        };
    ...
    pub fn from_buffer(ctx: &MtmdContext, data: &[u8]) -> Result<Self, MtmdBitmapError> {
        let bitmap = unsafe {
            infrastructure_llama_bindings::mtmd_helper_bitmap_init_from_buf(
                ctx.context.as_ptr(),
                data.as_ptr(),
                data.len(),
            )
    ...
    #[must_use]
    pub fn nx(&self) -> u32 {
        unsafe { infrastructure_llama_bindings::mtmd_bitmap_get_nx(self.bitmap.as_ptr()) }
    }

    /// Get bitmap height in pixels.
    #[must_use]
    pub fn ny(&self) -> u32 {
        unsafe { infrastructure_llama_bindings::mtmd_bitmap_get_ny(self.bitmap.as_ptr()) }
    }
    ...
    pub fn is_audio(&self) -> bool {
        unsafe { infrastructure_llama_bindings::mtmd_bitmap_is_audio(self.bitmap.as_ptr()) }
    }
...
impl Drop for MtmdBitmap {
    fn drop(&mut self) {
        unsafe { infrastructure_llama_bindings::mtmd_bitmap_free(self.bitmap.as_ptr()) }
    }
}
...
impl MtmdInputChunks {
    ...
    ///
    /// ```
    /// use infrastructure_llama_cpp::mtmd::MtmdInputChunks;
    ///
    /// let chunks = MtmdInputChunks::new();
    ...
    #[must_use]
    pub fn new() -> Self {
        let chunks = unsafe { infrastructure_llama_bindings::mtmd_input_chunks_init() };
    ...
    pub fn len(&self) -> usize {
        unsafe { infrastructure_llama_bindings::mtmd_input_chunks_size(self.chunks.as_ptr()) }
    }
    ...
    pub fn total_tokens(&self) -> usize {
        unsafe { infrastructure_llama_bindings::mtmd_helper_get_n_tokens(self.chunks.as_ptr()) }
    }
    ...
    pub fn total_positions(&self) -> i32 {
        unsafe { infrastructure_llama_bindings::mtmd_helper_get_n_pos(self.chunks.as_ptr()) }
    }
    ...
    pub fn eval_chunks(
        ...
        n_batch: i32,
        logits_last: bool,
    ) -> Result<infrastructure_llama_bindings::llama_pos, MtmdEvalError> {
        let mut new_n_past: infrastructure_llama_bindings::llama_pos = 0;

        let result = unsafe {
            infrastructure_llama_bindings::mtmd_helper_eval_chunks(
...
impl Drop for MtmdInputChunks {
    fn drop(&mut self) {
        unsafe { infrastructure_llama_bindings::mtmd_input_chunks_free(self.chunks.as_ptr()) }
    }
}
...
impl MtmdInputChunk {
    ...
    pub fn text_tokens(&self) -> Option<&[LlamaToken]> {
        ...
        let tokens_ptr = unsafe {
            infrastructure_llama_bindings::mtmd_input_chunk_get_tokens_text(
                self.chunk.as_ptr(),
                &raw mut n_tokens,
            )
        };
    ...
    pub fn n_tokens(&self) -> usize {
        unsafe { infrastructure_llama_bindings::mtmd_input_chunk_get_n_tokens(self.chunk.as_ptr()) }
    }
    ...
    pub fn n_positions(&self) -> i32 {
        unsafe { infrastructure_llama_bindings::mtmd_input_chunk_get_n_pos(self.chunk.as_ptr()) }
    }
...
impl Drop for MtmdInputChunk {
    fn drop(&mut self) {
        if self.owned {
            unsafe { infrastructure_llama_bindings::mtmd_input_chunk_free(self.chunk.as_ptr()) }
        }
    }
}
...
///
/// ```
/// use infrastructure_llama_cpp::mtmd::mtmd_default_marker;
///
...
pub fn mtmd_default_marker() -> &'static str {
    unsafe {
        let c_str = infrastructure_llama_bindings::mtmd_default_marker();
