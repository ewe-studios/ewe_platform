...
impl LlamaModel {
    ...
    pub fn token_bos(&self) -> LlamaToken {
        let token = unsafe { infrastructure_llama_bindings::llama_token_bos(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn token_eos(&self) -> LlamaToken {
        let token = unsafe { infrastructure_llama_bindings::llama_token_eos(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn token_nl(&self) -> LlamaToken {
        let token = unsafe { infrastructure_llama_bindings::llama_token_nl(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn is_eog_token(&self, token: LlamaToken) -> bool {
        unsafe { infrastructure_llama_bindings::llama_token_is_eog(self.vocab_ptr(), token.0) }
    }
    ...
    pub fn token_sep(&self) -> LlamaToken {
        let token = unsafe { infrastructure_llama_bindings::llama_vocab_sep(self.vocab_ptr()) };
        LlamaToken(token)
    }
    ...
    pub fn str_to_token(
        ...
        let size = unsafe {
            infrastructure_llama_bindings::llama_tokenize(
                self.vocab_ptr(),
                c_string.as_ptr(),
    ...
    pub fn token_to_bytes_with_size(
        ...
        let size = unsafe {
            infrastructure_llama_bindings::llama_token_to_piece(
                self.vocab_ptr(),
                token.0,
                buf,
                len,
    ...
    pub fn n_vocab(&self) -> i32 {
        unsafe { infrastructure_llama_bindings::llama_n_vocab(self.vocab_ptr()) }
    }
    ...
    pub fn n_embd(&self) -> c_int {
        unsafe { infrastructure_llama_bindings::llama_n_embd(self.model.as_ptr()) }
    }
    ...
    pub fn meta_val_str(&self, key: &str) -> Result<String, MetaValError> {
        ...
        extract_meta_string(
            |buf_ptr, buf_len| unsafe {
                infrastructure_llama_bindings::llama_model_meta_val_str(
                    self.model.as_ptr(),
                    key_ptr,
                    buf_ptr,
                    buf_len,
    ...
    pub fn meta_key_by_index(&self, index: i32) -> Result<String, MetaValError> {
        extract_meta_string(
            |buf_ptr, buf_len| unsafe {
                infrastructure_llama_bindings::llama_model_meta_key_by_index(
                    self.model.as_ptr(),
                    index,
                    buf_ptr,
                    buf_len,
    ...
    pub fn meta_val_str_by_index(&self, index: i32) -> Result<String, MetaValError> {
        extract_meta_string(
            |buf_ptr, buf_len| unsafe {
                infrastructure_llama_bindings::llama_model_meta_val_str_by_index(
                    self.model.as_ptr(),
                    index,
                    buf_ptr,
                    buf_len,
    ...
    pub fn rope_type(&self) -> Option<RopeType> {
        match unsafe { infrastructure_llama_bindings::llama_model_rope_type(self.model.as_ptr()) } {
            infrastructure_llama_bindings::LLAMA_ROPE_TYPE_NONE => None,
            infrastructure_llama_bindings::LLAMA_ROPE_TYPE_NORM => Some(RopeType::Norm),
            infrastructure_llama_bindings::LLAMA_ROPE_TYPE_NEOX => Some(RopeType::NeoX),
            infrastructure_llama_bindings::LLAMA_ROPE_TYPE_MROPE => Some(RopeType::MRope),
            infrastructure_llama_bindings::LLAMA_ROPE_TYPE_VISION => Some(RopeType::Vision),
            rope_type => {
    ...
    pub fn apply_chat_template(
        ...
        let chat: Vec<infrastructure_llama_bindings::llama_chat_message> = chat
            .iter()
            .map(|c| infrastructure_llama_bindings::llama_chat_message {
                role: c.role.as_ptr(),
...
impl Drop for LlamaModel {
    fn drop(&mut self) {
        unsafe { infrastructure_llama_bindings::llama_free_model(self.model.as_ptr()) }
    }
}
...
pub enum VocabType {
    /// Byte Pair Encoding
    BPE = infrastructure_llama_bindings::LLAMA_VOCAB_TYPE_BPE as _,
    /// Sentence Piece Tokenizer
    SPM = infrastructure_llama_bindings::LLAMA_VOCAB_TYPE_SPM as _,
}
...
pub enum LlamaTokenTypeFromIntError {
    ...
    UnknownValue(infrastructure_llama_bindings::llama_vocab_type),
}
