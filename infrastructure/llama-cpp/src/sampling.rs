...
impl LlamaSampler {
    ...
    pub fn accept(&mut self, token: LlamaToken) {
        unsafe { infrastructure_llama_bindings::llama_sampler_accept(self.sampler, token.0) }
    }
    ...
    pub fn reset(&mut self) {
        unsafe {
            infrastructure_llama_bindings::llama_sampler_reset(self.sampler);
        }
    }
    ...
    pub fn get_seed(&self) -> u32 {
        unsafe { infrastructure_llama_bindings::llama_sampler_get_seed(self.sampler) }
    }
    ...
    pub fn chain(samplers: impl IntoIterator<Item = Self>, no_perf: bool) -> Self {
        unsafe {
            let chain = infrastructure_llama_bindings::llama_sampler_chain_init(
                infrastructure_llama_bindings::llama_sampler_chain_params { no_perf },
            );

            for sampler in samplers {
                infrastructure_llama_bindings::llama_sampler_chain_add(chain, sampler.sampler);
    ...
    /// # Example
    /// ```rust
    /// use infrastructure_llama_cpp::token::{
    ///    LlamaToken,
    ///    data::LlamaTokenData,
    ...
    pub fn temp(t: f32) -> Self {
        let sampler = unsafe { infrastructure_llama_bindings::llama_sampler_init_temp(t) };
        Self { sampler }
    }
    ...
    pub fn top_k(k: i32) -> Self {
        let sampler = unsafe { infrastructure_llama_bindings::llama_sampler_init_top_k(k) };
        Self { sampler }
    }
    ...
    pub fn top_n_sigma(n: f32) -> Self {
        let sampler = unsafe { infrastructure_llama_bindings::llama_sampler_init_top_n_sigma(n) };
        Self { sampler }
    }
    ...
    pub fn grammar(
        ...
        let sampler = unsafe {
            infrastructure_llama_bindings::llama_sampler_init_grammar(
                model.vocab_ptr(),
                grammar_str.as_ptr(),
                grammar_root.as_ptr(),
            )
    ...
    pub fn grammar_lazy(
        ...
        let sampler = unsafe {
            infrastructure_llama_bindings::llama_sampler_init_grammar_lazy(
                model.vocab_ptr(),
                grammar_str.as_ptr(),
                grammar_root.as_ptr(),
    ...
    pub fn dry(
        ...
        let sampler = unsafe {
            infrastructure_llama_bindings::llama_sampler_init_dry(
                model.vocab_ptr(),
                model
                    .n_ctx_train()
                    .try_into()
    ...
    pub fn penalties(
        ...
        let sampler = unsafe {
            infrastructure_llama_bindings::llama_sampler_init_penalties(
                penalty_last_n,
                penalty_repeat,
                penalty_freq,
                penalty_present,
    ...
    pub fn dist(seed: u32) -> Self {
        let sampler = unsafe { infrastructure_llama_bindings::llama_sampler_init_dist(seed) };
        Self { sampler }
    }
    ...
    #[must_use]
    pub fn greedy() -> Self {
        let sampler = unsafe { infrastructure_llama_bindings::llama_sampler_init_greedy() };
        Self { sampler }
    }
    ...
    /// # Example
    /// ```rust
    /// use infrastructure_llama_cpp::token::{LlamaToken, logit_bias::LlamaLogitBias};
    /// use infrastructure_llama_cpp::sampling::LlamaSampler;
    ///
    /// let biases = vec![
...
impl Drop for LlamaSampler {
    fn drop(&mut self) {
        unsafe {
            infrastructure_llama_bindings::llama_sampler_free(self.sampler);
        }
    }
}
